    ...
    def run(self):
        ...
        for i in range(N):
            ...
            depthmap_utils.depth_visual_save(
                pano_depth, f"{save_dir_now}/depth_rgb.png")
            pointcloud_utils.depthmap2pointcloud_erp(
                pano_depth, pano_rgb, f"{save_dir_now}/pcd_rgb.ply")

            rgb_pil = Image.fromarray(pano_rgb)
            rgb_pil.save(rgb_path)

            pano_depth = (pano_depth - pano_depth.min()) / \
                (pano_depth.max() - pano_depth.min())
