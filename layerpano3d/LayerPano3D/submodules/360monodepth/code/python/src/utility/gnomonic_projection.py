...
def inside_polygon_2d(points_list, polygon_points, on_line=False, eps=1e-4):
    ...
    :rtype: numpy
    """
    point_inside = np.full(
        np.shape(points_list)[0],
        False,
        dtype=bool)  # the point in the polygon
    online_index = np.full(
        np.shape(points_list)[0],
        False,
        dtype=bool)  # the point on the polygon lines
    ...
    for index in range(np.shape(polygon_points)[0]):
        ...
        test_result = np.logical_and(
            GREATER(
                points_y, min(
                    polygon_1_y, polygon_2_y)), LESS(
                points_y, max(
                    polygon_1_y, polygon_2_y)))
        test_result = np.logical_and(
            test_result, LESS(
                points_x, max(
                    polygon_1_x, polygon_2_x)))
        ...
        on_line_list = LESS(
            abs(points_x[test_result] - intersect_points_x), eps)
        if on_line_list.any():
            online_index[test_result] = np.logical_or(
                online_index[test_result], on_line_list)

        # the point on the left of the line
        if LESS(points_x[test_result], intersect_points_x).any():
            test_result[test_result] = np.logical_and(
                test_result[test_result], LESS(
                    points_x[test_result], intersect_points_x))
            point_inside[test_result] = np.logical_not(
                point_inside[test_result])

    if on_line:
        return np.logical_or(point_inside, online_index).reshape(
            np.shape(points_list[:, 0]))
    else:
        return np.logical_and(point_inside, np.logical_not(
            online_index)).reshape(np.shape(points_list[:, 0]))
...
def gnomonic_projection(theta, phi, theta_0, phi_0):
    ...
    cos_c = np.sin(phi_0) * np.sin(phi) + np.cos(phi_0) * \
        np.cos(phi) * np.cos(theta - theta_0)
    ...
    y = (np.cos(phi_0) * np.sin(phi) - np.sin(phi_0) *
         np.cos(phi) * np.cos(theta - theta_0)) / cos_c
...
def reverse_gnomonic_projection(x, y, lambda_0, phi_1):
    ...
    c = np.arctan2(rho, 1)
    phi_ = np.arcsin(np.cos(c) * np.sin(phi_1) +
                     (y * np.sin(c) * np.cos(phi_1)) / rho)
    lambda_ = lambda_0 + \
        np.arctan2(x * np.sin(c), rho * np.cos(phi_1) * np.cos(c) - y * np.sin(phi_1) * np.sin(c))
...
def gnomonic2pixel(
    ...
    if padding_size != 0.0 and coord_gnom_xy_range is not None:
        log.warning(
            "set the padding size and gnomonic range at same time! Please double check!")

    # normailzed tangent image space --> tangent image space
    # TODO check add the padding whether necessary
    gnomonic2image_width_ratio = (
        tangent_image_width - 1.0) / (x_max - x_min + padding_size * 2.0)
    coord_pixel_x = (coord_gnom_x - x_min + padding_size) * \
        gnomonic2image_width_ratio
    coord_pixel_x = (coord_pixel_x + 0.5).astype(int)

    gnomonic2image_height_ratio = (
        tangent_image_height - 1.0) / (y_max - y_min + padding_size * 2.0)
    coord_pixel_y = -(coord_gnom_y - y_max - padding_size) * \
        gnomonic2image_height_ratio
...
def pixel2gnomonic(
    ...
    gnomonic2image_ratio_width = (
        tangent_image_width - 1.0) / (gnomonic_size_x + padding_size * 2.0)
    coord_gnom_x = coord_pixel_x / gnomonic2image_ratio_width + x_min - padding_size

    gnomonic_size_y = abs(y_max - y_min)
    gnomonic2image_ratio_height = (
        tangent_image_height - 1.0) / (gnomonic_size_y + padding_size * 2.0)
