...
def great_circle_distance_uv(
    ...
    a = np.sin(delta_phi * 0.5) ** 2 + np.cos(points_1_phi) * \
        np.cos(points_2_phi) * np.sin(delta_theta * 0.5) ** 2
...
def erp2sph(erp_points, erp_image_height=None, sph_modulo=False):
    ...
    if erp_image_height is None:
        ...
        if (height * 2) != width:
            log.error(
                "the ERP image width {} is not two time of height {}".format(
                    width, height))
    else:
    ...
    points_phi = -(erp_points_y * (np.pi / height) +
                   np.pi / height * 0.5) + 0.5 * np.pi
...
def car2sph(points_car, min_radius=1e-10):
    ...
    theta[valid_list] = np.arctan2(
        points_car[:, 0][valid_list], points_car[:, 2][valid_list])

    phi = np.zeros((points_car.shape[0]), float)
    phi[valid_list] = - \
        np.arcsin(np.divide(points_car[:, 1][valid_list], radius[valid_list]))
