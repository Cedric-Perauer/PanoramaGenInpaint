...
def world2cam_slow(point3D, cam_model):
    ...
    zero_flag = np.logical_and.reduce(
        (x_ocamcalib == 0, y_ocamcalib == 0, z_ocamcalib == 0))
...
def world2cam(matrix_3d, cam_model):
    ...
    # distorsion_c s inverted because the image is now rotated (3840-height,
    # 2880-width)
...
def cam2world(matrix_2d, cam_model):
    ...
    # all parameter store in OCamClib coordinate system, because it computed
    # by OCamCablib tool-box
    ...
    u_ocam = invdet * ((matrix_2d_ocamclib[:,
                                           0] - distorsion_c[0]) - d * (matrix_2d_ocamclib[:,
                                                                                           1] - distorsion_c[1]))
    v_ocam = invdet * (-e * (matrix_2d_ocamclib[:, 0] - distorsion_c[0]) + c * (
        matrix_2d_ocamclib[:, 1] - distorsion_c[1]))
    # point2D = matrix_2d @ A.I - distorsion_c
...
def create_perspective_undistortion_LUT(
        img_shape, cam_model, sf=4, patch=None):
    """
...
def point3d2obj(points_3d, obj_path):
    ...
    with open(obj_path, "w") as f:
        for index in range(0, points_3d.shape[0]):
            f.write("v {} {} {}\n".format(
                points_3d[index, 0], points_3d[index, 1], points_3d[index, 2]))


def stitch_rgb_image(
        image_data_list,
        image_param_list,
        fisheye_model,
        subimage_fov=60,
    ...
    fisheye_image = np.zeros(
        (fisheye_image_height,
         fisheye_image_width,
         channel_number),
        float)
    fisheye_image_weight = np.zeros(
        (fisheye_image_height, fisheye_image_width), float)
    for index in range(0, len(image_data_list)):
        fisheye_image_weight_subimg = np.zeros(
            (fisheye_image_height, fisheye_image_width), float)
        fisheye_image_subimage = np.zeros(
            (fisheye_image_height, fisheye_image_width, channel_number), float)
        ...
        pinhole_3d_points = image_param["rotation"] @ fisheye_3d_points.T + \
            image_param["translation"][:, np.newaxis]

        # check the pixel in the available hfov and vfov range of fisheye image
        hfov = subimage_fov
        vfov = subimage_fov
        radius = np.linalg.norm(pinhole_3d_points.T, axis=1)
        points_azimuth = np.degrees(np.arctan2(
            pinhole_3d_points[0, :], pinhole_3d_points[2, :]))
        points_altitude = np.degrees(
            np.arcsin(np.divide(-pinhole_3d_points[1, :], radius)))

        points_azimuth_inhfov = np.logical_and(
            points_azimuth > -0.5 * hfov,
            points_azimuth < 0.5 * hfov)
        points_altitude_invfov = np.logical_and(
            points_altitude > -0.5 * vfov,
            points_altitude < 0.5 * vfov)
        available_pixels_list_fov = np.logical_and(
            points_azimuth_inhfov, points_altitude_invfov)
        available_pixels_list_fov_mat = available_pixels_list_fov.reshape(
            fisheye_image_height, fisheye_image_width)

        fisheye_2d_points_subimage = fisheye_2d_points[available_pixels_list_fov].astype(
            int)

        # projection to pin-hole image
        pinhole_3d_points = np.divide(
            pinhole_3d_points, pinhole_3d_points[2, :])
        pinhole_2d_points = (
            image_param["intrinsics"]["matrix"] @ pinhole_3d_points)[:2, :]
        ...
        available_pixels_list = np.logical_and.reduce(
            (
                ...
                pinhole_2d_points[0, :] >= 0,
                pinhole_2d_points[0, :] <= pinhole_image_width - 1,
                pinhole_2d_points[1, :] >= 0,
                pinhole_2d_points[1, :] <= pinhole_image_height - 1,
            )
        )

        for channel in range(0, channel_number):
            fisheye_image_subimage[fisheye_2d_points_subimage[:,
                                                              1][available_pixels_list],
                                   fisheye_2d_points_subimage[:,
                                                              0][available_pixels_list],
                                   channel,
                                   ] = ndimage.map_coordinates(image_data[:,
                                                                          :,
                                                                          channel],
                                                               [pinhole_2d_points[1,
    ...
    for channel in range(0, channel_number):
        fisheye_image[:,
                      :,
                      channel][available_weight_list] = np.divide(fisheye_image[:,
                                                                                :,
                                                                                channel][available_weight_list],
                                                                  fisheye_image_weight[available_weight_list])
...
def sample_rgb_image(
    ...
    xyz_rotation_array = generate_camera_orientation(
        hfov_fisheye,
        vfov_fisheye,
        hfov_pinhole,
        vfov_pinhole,
    ...
    sub_image_param_list = get_perspective_camera_parameters(
        ...
        vfov_pinhole,
        image_width_pinhole,
        image_height_pinhole,
        xyz_rotation_array.T)

    # 1) generate the perspective images
    # TODO perspective image size is base on fov ane original fish-eye image
    # size
    canvas_yy, canvas_xx = np.mgrid[0:image_height_pinhole,
                                    0:image_width_pinhole]
    canvas_2d = np.squeeze(
        np.dstack(
            (canvas_xx.flatten(), canvas_yy.flatten())))
    # canvas_2d each column is 3D point [x,y,1]
    canvas_2d = np.hstack(
        (canvas_2d, np.ones((image_height_pinhole * image_width_pinhole, 1))))
    sub_images = []
    ...
    for index in range(0, len(sub_image_param_list)):
        ...
        pinhole_cs = np.linalg.inv(
            sub_image_param["intrinsics"]["matrix"]) @ canvas_2d.T
        ...
        tangential_img = np.zeros(
            tuple(canvas_size) + (channel_number,), dtype=float)

        for channel in range(0, channel_number):
            tangential_img[:, :, channel] = ndimage.map_coordinates(
                img[:, :, channel],
                [fetch_from[:, 1].reshape(canvas_size), fetch_from[:, 0].reshape(canvas_size)],
                order=1,
                mode="constant",
            )
...
def sample_img(img, cam_model, fov=53, run_midas=False):
    ...
    canvas_2d = np.squeeze(
        np.dstack(
            (canvas_xx.flatten(), canvas_yy.flatten())))
    canvas_2d = np.hstack(
        (canvas_2d, np.ones((canvas_size[0] * canvas_size[1], 1))))
    ...
    equirect_3D_points_rgb = np.zeros(
        (7, equirect_3D_points.shape[-1]), dtype=float)
    ...
    nan_boolean = np.bitwise_not(
        np.isnan(
            np.sum(
                equirect_3D_points.T,
                axis=1)))
    ...
    fisheye2equirec[:, nan_boolean] = img[fetch_from[1,
                                                     nan_boolean], fetch_from[0, nan_boolean]].T
    fisheye2equirec = np.moveaxis(
        fisheye2equirec.ravel().reshape(equirect_size), 0, -1)
    RGB_sub_images = []
    depth_sub_images = []
    ...
    for i in range(0, angle_mesh.shape[0]):
        for j in range(0, angle_mesh.shape[1]):
            ...
            # Get perspective camera model with angle_pair as parameter for
            # extrinsic params
            pinhole_camera, params_2json = getVirtualCameraMatrix(
                fov, canvas_size, cam_model, x_angle=angle_pair[1], y_angle=angle_pair[0])
            virtual_pinhole_params.append(params_2json)

            #   Transform pixel coords to world coords
            pinhole_cs = np.linalg.inv(
                pinhole_camera["intrinsics"]["matrix"]) @ canvas_2d.T
            ...
            fetch_from[:, 0] = np.clip(fetch_from[:, 0], 0, width - 1)
            fetch_from[:, 1] = np.clip(fetch_from[:, 1], 0, height - 1)
            virtual2fisheye_idxs = np.dstack((fetch_from[:, 0].reshape(
                canvas_size), fetch_from[:, 1].reshape(canvas_size)))

            tangential_img = img[virtual2fisheye_idxs[...,
                                                      1], virtual2fisheye_idxs[..., 0]]

            if not os.path.isfile(
                "../../../../color_subviews/{:06}_{}_{}.png".format(
                    idx, int(
                        angle_pair[1]), int(
                    angle_pair[0]))):
                os.makedirs("../../../../color_subviews/", exist_ok=True)
                cv2.imwrite(
                    "../../../../color_subviews/frame_{:06}.png".format(idx),
                    tangential_img)

            if run_midas:
                tangential_depth = run_depth(
                    tangential_img[None],
                    "MiDaS/model.pt",
                    MonoDepthNet,
                    MiDaS_utils)[0]
                os.makedirs("../../../../depth_subviews/", exist_ok=True)
                np.save(
                    "../../../../depth_subviews/frame_{:06}.npy".format(idx),
                    tangential_depth)
            else:
                tangential_depth = np.load(
                    "../../../../depth_subviews/frame_{:06}.npy".format(idx))

            # tangential_depth = (tangential_depth - np.min(tangential_depth)) / np.ptp(tangential_depth)
            print("frame {}: {}---{}".format(idx,
                                             np.max(tangential_depth),
                                             np.min(tangential_depth)))
            RGB_sub_images.append(tangential_img)
            depth_sub_images.append(tangential_depth)

            # Which equirect 3D points fall in the current perspective subview
            # camera plane?
            equi2tangential_3d = (
                pinhole_camera["intrinsics"]["matrix"] @ pinhole_camera["rotation"] @ equirect_3D_points)
            equi2tangential = np.divide(equi2tangential_3d, np.tile(
                equi2tangential_3d[2, :], (3, 1)))[:-1].astype(int)
            inside_fov = (
                (equi2tangential[0, :] >= 0)
                & (equi2tangential[0, :] < canvas_size[1])
                & (equi2tangential[1, :] >= 0)
                & (equi2tangential[1, :] < canvas_size[0])
            ...
            equirect_3D_points_rgb[3:6,
                                   inside_fov] = tangential_img[fetch_pixel[1],
                                                                fetch_pixel[0]].T
            equirect_3D_points_rgb[6,
                                   inside_fov] = tangential_depth[fetch_pixel[1],
                                                                  fetch_pixel[0]].T
    ...
    equirect_rgb = np.moveaxis(
        equirect_3D_points_rgb[3:6].ravel().reshape(equirect_size), 0, -1)
...
def get_fisheyeFOV(img, model):
    # Get (top center vs bottom center) and (left center vs right center)
    # pixels to calculate FOV
    ...
    v_fov = np.degrees(
        np.arccos(
            np.clip(
                np.dot(
                    bot_center_world, top_center_world), -1.0, 1.0)))
    h_fov = np.degrees(
        np.arccos(
            np.clip(
                np.dot(
                    right_center_world, left_center_world), -1.0, 1.0)))
...
def plot_img_array(camera_array_size, sub_images):
    fig, axes = plt.subplots(
        nrows=camera_array_size[0], ncols=camera_array_size[1])
...
def cam2world_single(point2D, model):
    ...
    invnorm = 1.0 / np.sqrt(xp * xp + yp * yp + zp * zp)
...
def equirect_world2cam(matrix3d, im_size=(1000, 1000)):
    ...
    # implemented from https://ttic.uchicago.edu/~rurtasun/courses/CV/lecture0
    ...
    lat = np.arctan2(matrix3d[2, :], np.linalg.norm(
        matrix3d[[0, 1], :], axis=0))
...
def equirect_cam2world(im_size=(1000, 1000)):
    ...
    canvas_2d = np.squeeze(
        np.dstack((canvas_xx.flatten(), canvas_yy.flatten()))).reshape(-1, 2).T
    ...
    unit_sphere = np.divide(
        points_3D, np.tile(
            np.linalg.norm(
                points_3D, axis=0), (3, 1)))
...
def generate_camera_orientation(
    ...
    :type padding: float
    """
    hfov_interval = (hfov_fisheye - hfov_pinhole -
                     padding * 2) / (horizontal_size - 1)
    vfov_interval = (vfov_fisheye - vfov_pinhole -
                     padding * 2) / (vertical_size - 1)

    h_index = (np.linspace(0, horizontal_size, horizontal_size,
                           endpoint=False) - (horizontal_size - 1) / 2.0) * hfov_interval
    v_index = (np.linspace(0, vertical_size, vertical_size,
               endpoint=False) - (vertical_size - 1) / 2.0) * vfov_interval
    x_rotation, y_rotation = np.meshgrid(
        h_index, v_index)  # the camere orientation

    # compute the overlap of perspective images.
    overlap_area_h = h_index[0] + hfov_pinhole / \
        2.0 - (h_index[1] - hfov_pinhole / 2.0)
    log.debug("the horizontal overlap angle is {}".format(overlap_area_h))
    overlap_area_v = v_index[0] + vfov_pinhole / \
        2.0 - (v_index[1] - vfov_pinhole / 2.0)
    ...
    xyz_rotation_array = xyz_rotation_array.reshape(
        [3, horizontal_size * vertical_size])
    return xyz_rotation_array


def get_perspective_camera_parameters(
        hfov,
        vfov,
        image_width,
        image_height,
    ...
    rotation = R.from_euler(
        "zyx", camera_direction[:, [2, 1, 0]], degrees=True)
...
def getVirtualCameraMatrix(
    ...
    intrinsic_matrix = np.array(
        [[f, 0, size[1] / 2], [0, f, size[0] / 2], [0, 0, 1]])
    ...
    params_json = {
        "rotation": (rmat_z @ rmat_y @ rmat_x).tolist(),
        "translation": [0, 0, 0],
        "intrinsics": {
