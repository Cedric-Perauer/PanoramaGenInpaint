...
def create_alignment_data(
    erp_depthmap,
    subimage_size=500,
    padding_size=0.3,
    coeff_scale_list=None,
    coeff_offset_list=None,
    grid_size=[
        5,
        5]):
    ...
    # 1) erp to 20 images
    subimage_list, _, _ = proj_ico.erp2ico_image(
        erp_depthmap, subimage_size, padding_size, full_face_image=True)

    # 2) scale and offset the subimage disparity map
    subimage_list_new = []
    for idx in range(len(subimage_list)):
        temp = depth_stitch.depthmap_deform(
            subimage_list[idx],
            coeff_scale_list[idx],
            coeff_offset_list[idx])
...
def data_visualizing(mock_data_root_dir, frame_number):
    ...
    for idx in range(frame_number):
        depthmap_filename = pathlib.Path(
            filenameConv.subimage_depthmap_filename_expression.format(idx))
    ...
    for src_idx in range(frame_number):
        for tar_idx in range(frame_number):
            if src_idx == tar_idx:
                continue
            filename_src2tar = filenameConv.subimage_pixelcorr_filename_expression.format(
                src_idx, tar_idx)
            pixels_corresponding = serialization.pixel_corresponding_load(
                filename_src2tar)
            ...
            # load subimage
            if os.path.isabs(src_image_filepath) and os.path.isabs(
                    tar_image_filepath):
                # src_image_data = image_io.image_read(src_image_filepath + ".jpg")
                # tar_image_data = image_io.image_read(tar_image_filepath+ ".jpg")
                image1_output_path = src_image_filepath + \
                    "_{}_{}.jpg".format(src_idx, tar_idx)
                image2_output_path = tar_image_filepath + \
                    "_{}_{}.jpg".format(tar_idx, src_idx)
            else:
                ...
                image1_output_path = mock_data_root_dir + \
                    src_image_filepath + "_{}_{}.jpg".format(src_idx, tar_idx)
                image2_output_path = mock_data_root_dir + \
                    tar_image_filepath + "_{}_{}.jpg".format(tar_idx, src_idx)

            src_image_data = np.full(
                (image_height, image_width, 3), 255, dtype=np.uint8)
            tar_image_data = np.full(
                (image_height, image_width, 3), 255, dtype=np.uint8)
            ...
            src_image_data_image_np, tar_image_data_image_np, _ = subimage.draw_corresponding(
                src_image_data, tar_image_data, pixel_corresponding_array)
