...
def abs_rel_error(pred, gt, mask):
    ...
    return np.mean(np.abs(pred[mask > 0] - gt[mask > 0]) /
                   np.maximum(np.abs(gt[mask > 0]), np.full_like(gt[mask > 0], eps)))
...
def log_rms_sq_error(pred, gt, mask):
    ...
    mask = (
        mask > 0) & (
        pred > eps) & (
            gt > eps)  # Compute a mask of valid values
...
def log_rms_sq_error_map(pred, gt, mask):
    ...
    log_rms_map[mask > 0] = (np.log10(pred[mask > 0]) -
                             np.log10(gt[mask > 0])) ** 2 / gt[mask > 0]
...
def log_rms_scale_invariant(pred, gt, mask):
    ...
    if np.any(pred[mask] < 0) or np.any(gt[mask] < 0):
        log.error(
            "The disparity map has negative value! The metric log will generate NaN")

    alpha_depth = np.mean(np.log(pred[mask > 0]) - np.log(gt[mask > 0]))
    log_rms_scale_inv = np.mean(
        np.log(pred[mask > 0]) - np.log(gt[mask > 0]) + alpha_depth)
...
def log_rms_scale_invariant_map(pred, gt, mask):
    ...
    log_rms_map = np.zeros_like(pred)
    alpha_depth = np.mean(np.log(pred[mask > 0]) - np.log(gt[mask > 0]))
    log_rms_map[mask > 0] = np.log(
        pred[mask > 0]) - np.log(gt[mask > 0]) + alpha_depth
    log_rms_map[mask <= 0] = np.nan
    return log_rms_map
...
def delta_inlier_ratio(pred, gt, mask, degree=1):
    ...
    return np.mean(np.maximum(pred[mask > 0] /
                              gt[mask > 0], gt[mask > 0] /
                              pred[mask > 0]) < (1.25**degree))
...
def delta_inlier_ratio_map(pred, gt, mask, degree=1):
    ...
    :rtype: numpy
    """
    delta_max = np.maximum(pred[mask > 0] /
                           gt[mask > 0], gt[mask > 0] /
                           pred[mask > 0])
...
def report_error(gt, pred, max_depth=10.0):
    ...
    metrics_res = {
        "AbsRel": abs_rel_error(pred, gt, mask),
...
def visualize_error_maps(
    ...
    error_map_filename = (
        os.path.join(output_path, "{:04}_{}_error.png".format(idx, filename))
        if filename != ""
        else os.path.join(output_path, "{:04}_error.png".format(idx))
    )
    i = 0
