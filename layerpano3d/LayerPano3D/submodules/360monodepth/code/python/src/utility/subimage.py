...
def draw_corresponding(
        src_image_data,
        tar_image_data,
        pixel_corresponding_array):
    """
    ...
    if pixel_corresponding_array.size == 0:
        ...
        return np.asarray(src_image_data_image), np.asarray(
            tar_image_data_image), np.asarray(src_warp_image)
    ...
    for match_pair in pixel_corresponding_array:
        ...
        # Note: hsv_to_rgb() function expects h to be in the range 0..1 not
        # 0..360
        r, g, b = (int(i * 255) for i in hsv_to_rgb(h / 360.0, 1.0, 1.0))
    ...
    for channel in range(0, 3):
        src_warp_image[tar_y, tar_x, channel] = ndimage.map_coordinates(
            src_image_data[:, :, channel], [src_y, src_x], order=1, mode="constant"
        )

    return src_image_data_image_np, tar_image_data_image_np, src_warp_image.astype(
        np.uint8)
...
def erp_ico_cam_intrparams(image_width, padding_size=0):
    ...
    fov_v = np.abs(np.arctan2(tangent_points_y_min, 1.0)) + \
        np.abs(np.arctan2(tangent_points_y_max, 1.0))
    fov_h = np.abs(2 * np.arctan2(tangent_points_x_min, 1.0))

    log.debug(
        "Pin-hole camera fov_h: {}, fov_v: {}".format(np.degrees(fov_h), np.degrees(fov_v)))
    ...
    for index in range(0, 20):
        ...
        # rotation
        ico_param_list = proj_ico.get_icosahedron_parameters(
            index, padding_size)
        tangent_point = ico_param_list["tangent_point"]
        # print(tangent_point)
...
def erp_ico_pixel_corr(
    subimage_sphcoor,
    next_tangent_point,
    padding_size,
    tangent_image_width,
    tangent_image_height,
    tangent_triangle_vertices_gnom,
):
    """
    ...
    """
    gnom_x, gnom_y = gp.gnomonic_projection(
        subimage_sphcoor[0, :], subimage_sphcoor[1, :], next_tangent_point[0], next_tangent_point[1]
    )

    # 1)remove the pixels on the another hemisphere, just use the pixel in the
    # same hemisphere.
    ...
    tangent_gnomonic_range = [
        ...
        gnomonic_y_min,
        gnomonic_y_max]
    gnom_image_x, gnom_image_y = gp.gnomonic2pixel(
        gnom_x, gnom_y, 0.0, tangent_image_width, tangent_image_height, tangent_gnomonic_range)

    points_2_theta = np.full(
        subimage_sphcoor[0, :].shape, next_tangent_point[0], np.float64)
    points_2_phi = np.full(
        subimage_sphcoor[0, :].shape, next_tangent_point[1], np.float64)
    central_angle_delta = spherical_coordinates.great_circle_distance_uv(
        subimage_sphcoor[0, :], subimage_sphcoor[1, :], points_2_theta, points_2_phi, radius=1
    )

    valid_pixel_index = np.logical_and.reduce(
        (
            gnom_image_x >= 0,
            gnom_image_x < tangent_image_width,
            gnom_image_y >= 0,
            gnom_image_y < tangent_image_height,
            np.abs(central_angle_delta) < 0.5 * np.pi,
        )
    )

    # 2) get the src ant tar's subimage pixel coordinate
    src_subimage_x = np.linspace(
        0,
        tangent_image_width,
        tangent_image_width,
        endpoint=False)
    src_subimage_y = np.linspace(
        0,
        tangent_image_height,
        tangent_image_height,
        endpoint=False)
    src_subimage_xv, src_subimage_yv = np.meshgrid(
        src_subimage_x, src_subimage_y)
    pixel_index_src = np.stack(
        (src_subimage_yv[valid_pixel_index],
         src_subimage_xv[valid_pixel_index])).T
    pixel_index_tar = np.stack(
        (gnom_image_y[valid_pixel_index],
         gnom_image_x[valid_pixel_index])).T
...
def erp_ico_proj(
    ...
    if corr_downsample_factor != 1.0:
        log.info(
            "Down sample the pixels corresponding, keep {}%.".format(
                corr_downsample_factor * 100))

    # 0) generate subimage
    subimage_list, subimage_sphcoor_list, _ = proj_ico.erp2ico_image(
        erp_image, tangent_image_width, padding_size, full_face_image=True
    )
    ...
    for index in range(0, len(subimage_list)):
        ico_param_list.append(
            proj_ico.get_icosahedron_parameters(
                index, padding_size))
    ...
    if opt is None:
        ...
        matterport_circle_phi = np.deg2rad(
            opt.dataset_matterport_blur_area_height * (180.0 / erp_image_height))
        matterport_hexagon_circumradius = np.tan(matterport_circle_phi)
        log.info(
            f"The image height is {erp_image_height}, margin height is {opt.dataset_matterport_blur_area_height}, circumradius is {matterport_hexagon_circumradius}"
        )
        matterport_blurarea_shape = opt.dataset_matterport_blurarea_shape  # "hexagon",  "circle"

    # source subimage index
    def subimage_corr_fun(
            ico_param_list,
            pixels_corr_dict,
            subimage_index_src):
        subimage_sphcoor = subimage_sphcoor_list[subimage_index_src]

        # target subimage index, and compute the corresponding computing with
        # multi-threads
        ...
        for subimage_index_tar in range(0, len(subimage_list)):
            ...
            if subimage_index_src == subimage_index_tar:
                pixels_corr_dict_subimage[subimage_index_tar] = np.empty(
                    shape=(0, 0))
                continue
            # compute the corresponding
            ico_param = ico_param_list[subimage_index_tar]
            heightbout_tangent_point = ico_param["tangent_point"]
            tangent_triangle_vertices_gnom = np.array(
                ico_param["triangle_points_tangent"])
            pixels_corr_src2tar, pixels_sph = erp_ico_pixel_corr(
                subimage_sphcoor,
                heightbout_tangent_point,
                padding_size,
                tangent_image_width,
                tangent_image_height,
                tangent_triangle_vertices_gnom,
            ...
            if matterport_hexagon_mask_enable and (
                (0 <= subimage_index_src <= 4 and 0 <= subimage_index_tar <= 4) or (
                    15 <= subimage_index_src <= 19 and 15 <= subimage_index_tar <= 19)):

                # 1) get the src and tar pixel coordinate on the top/bottom
                # tangent image
                ...
                if matterport_blurarea_shape == "hexagon":
                    ...
                    pixels_gnom_xv, pixels_gnom_yv = gp.gnomonic_projection(
                        pixels_sph[0, :],
                        pixels_sph[1, :],
                        hexagon_subimage_tangent_point_theta,
                        hexagon_subimage_tangent_point_phi,
                    )
                    pixels_corr_gnom = np.stack(
                        (pixels_gnom_xv, pixels_gnom_yv), axis=1)

                    # check if the pixels in the hexagon
                    hexagon_points_list = polygon.generate_hexagon(
                        matterport_hexagon_circumradius)
                    inside_hexagon = gp.inside_polygon_2d(
                        pixels_corr_gnom, hexagon_points_list)
                    ...
                    outside_circle = np.logical_and(
                        pixels_sph[1, :] <= np.pi * 0.5 - matterport_circle_phi,
                        pixels_sph[1, :] >= -np.pi * 0.5 + matterport_circle_phi,
                    )
            ...
            if corr_downsample_factor != 1.0 and pixels_corr_dict_subimage[
                    subimage_index_tar] is not None:
                corr_number = pixels_corr_dict_subimage[subimage_index_tar].shape[0]
                corr_index = np.linspace(
                    0,
                    corr_number -
                    1,
                    num=int(
                        corr_number *
                        corr_downsample_factor)).astype(int)
                corr_index = np.unique(corr_index)
                pixels_corr_dict_subimage[subimage_index_tar] = pixels_corr_dict_subimage[subimage_index_tar][
                    corr_index, :
                ]
    ...
    with ThreadPoolExecutor(max_workers=opt.dispalign_corr_thread_number) as executor:
        ...
        for future in as_completed(feature_list):
            try:
                ...
                print(
                    "Computing subimage {} pixels corresponding error! Exception: {}" %
                    (result, exc))
            else:
                log.debug(
                    "Generate image {} pixels corresponding: done ".format(result))

    # 2) camera parameters
    subimage_cam_param_list = erp_ico_cam_intrparams(
        tangent_image_width, padding_size)
...
def erp_ico_stitch(subimage_list, erp_image_height, padding_size):
    ...
    erp_image = proj_ico.ico2erp_image(
        subimage_list, erp_image_height, padding_size, "mean")
    return erp_image


def erp_ico_draw_corresponding(
        src_image_data,
        tar_image_data,
        pixel_corresponding_array,
        src_image_output_path,
        tar_image_output_path):
    ...
    for channel in range(0, 3):
        src_warp[tar_y, tar_x, channel] = ndimage.map_coordinates(
            src_image_data_np[:, :, channel], [src_y, src_x], order=1, mode="constant"
        )
