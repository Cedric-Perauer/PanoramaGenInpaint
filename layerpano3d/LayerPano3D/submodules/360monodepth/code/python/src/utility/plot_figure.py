...
class PlotFigure:
    ...
    def draw_ico_tangent_planes_texture_stitch(
            self, root_dir, subimage_filename_list, texture_filename):
        ...
        texture_data = np.zeros(
            (texture_height, texture_width, 3), dtype=np.uint8)
    ...
    def draw_ico_tangent_planes(
        self,
        radius,
        padding_size,
        obj_file_path,
        subimage_shift_ratio=0.0,
        obj_file_texture_enable=False,
        texture_image_filename="ico_rgb_image.png",
    ):
        ...
        for tangent_image_idx in range(0, 20, 1):
            tangent_image_param = pro_ico.get_icosahedron_parameters(
                tangent_image_idx, padding_size)

            triangle_points_sph = tangent_image_param["triangle_points_sph"]
            triangle_points_sph = np.array(
                triangle_points_sph, dtype=np.float32)

            triangle_points_sph_sort_idx = np.argsort(
                triangle_points_sph[:, 0])
            if (triangle_points_sph_sort_idx != np.array((0, 1, 2))).any():
                triangle_points_sph_sort_idx = np.tile(
                    triangle_points_sph_sort_idx, 2).reshape((2, 3))
                triangle_points_sph = np.take_along_axis(
                    triangle_points_sph, triangle_points_sph_sort_idx.T, axis=0)

            triangle_points_3d = sc.sph2car(
                triangle_points_sph[:, 0], triangle_points_sph[:, 1], radius=radius)
            ...
            if subimage_shift_ratio != 0.0:
                ...
                triangle_normal_norm_3d = sc.sph2car(
                    triangle_normal_norm_sph[0], triangle_normal_norm_sph[1], radius=radius)
            ...
            # 2-0) 3D points
            rectangle_points_3d = polygon.triangle_bounding_rectangle_3D(
                triangle_points_3d_head, triangle_points_3d_edge
            )
            # write vertex
            ...
            if obj_file_texture_enable:
                ...
                texture_y_offset_start = 1.0 - 1.0 / \
                    4.0 * int(tangent_image_idx / 5)
                texture_y_offset_end = texture_y_offset_start - 1.0 / 4.0
                obj_file_texture_coor_str += "vt {} {}\n".format(
                    texture_x_offset_start, texture_y_offset_start)
                obj_file_texture_coor_str += "vt {} {}\n".format(
                    texture_x_offset_start, texture_y_offset_end)
                obj_file_texture_coor_str += "vt {} {}\n".format(
                    texture_x_offset_end, texture_y_offset_end)
                obj_file_texture_coor_str += "vt {} {}\n".format(
                    texture_x_offset_end, texture_y_offset_start)
            ...
            if obj_file_texture_enable:
                ...
                obj_file_face_str += "f {} {} {}\n".format(
                    idx_offset + 1, idx_offset + 2, idx_offset + 3)
                obj_file_face_str += "f {} {} {}\n".format(
                    idx_offset + 1, idx_offset + 3, idx_offset + 4)

            obj_file_line_str += "f {} {} {} {}\n".format(
                idx_offset + 1, idx_offset + 2, idx_offset + 3, idx_offset + 4
            )
