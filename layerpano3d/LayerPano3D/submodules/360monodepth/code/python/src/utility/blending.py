...
class BlendIt:
    def __init__(self, padding, n_subimages, blending_method):
        ...
        # point where slope starts to affect the radial blendweights
        self.inflection_point = 10
        # Percentage of diagonal until weights start decaying in frustum
        # weights
        self.diagonal_percentage = 48.5
        self.n_subimages = n_subimages
        self.blending_method = blending_method
        self.padding = padding
        # Pixel coordinates of the triangular tangent face in equirect image
        self.triangle_coordinates_erp = []
        self.triangle_coordinates_tangent = []
        # Pixel coordinates of the squared (plane) tangent face in equirect
        # image
        self.squared_coordinates_erp = []
    ...
    def blend(self, subimage_dispmap, erp_image_height):
        ...
        if isinstance(subimage_dispmap, str):
            for index in range(0, 20):
                tangent_disp_imgs.append(
                    depthmap_utils.read_pfm(
                        subimage_dispmap.format(index))[0])
        elif isinstance(subimage_dispmap, list):
            tangent_disp_imgs = subimage_dispmap
        else:
            log.error(
                "Disparity map type error. {}".format(
                    type(subimage_dispmap)))
        ...
        equirect_depth_tensor, nn_blending = self.misc_data(
            tangent_disp_imgs, erp_size)

        # Normalize
        norm_radial_blendweights = np.divide(
            self.radial_blendweights,
            np.sum(self.radial_blendweights, axis=2)[..., None],
            where=(np.sum(self.radial_blendweights, axis=2) > 0)[..., None],
        )

        norm_frustum_blendweights = np.divide(
            self.frustum_blendweights,
            np.sum(self.frustum_blendweights, axis=2)[..., None],
            where=(np.sum(self.frustum_blendweights, axis=2) > 0)[..., None],
        )

        radial_blended = np.nansum(
            norm_radial_blendweights *
            equirect_depth_tensor,
            axis=2)
        frustum_blended = np.nansum(
            norm_frustum_blendweights *
            equirect_depth_tensor,
            axis=2)
        ...
        if self.blending_method == "poisson":
            blended_img[self.blending_method] = self.gradient_blending(
                equirect_depth_tensor, self.frustum_blendweights, nn_blending
            )
        ...
        if self.blending_method == "all":
            blended_img["poisson"] = self.gradient_blending(
                equirect_depth_tensor, self.frustum_blendweights, nn_blending
            )
    ...
    def tangent_images_coordinates(self, erp_image_height, tangent_img_size):
        ...
        if erp_image_width != erp_image_height * 2:
            raise Exception(
                "the ERP image dimession is {}x{}".format(
                    erp_image_height, erp_image_width))
        ...
        for triangle_index in range(0, 20):
            ...
            triangle_param = proj_ico.get_icosahedron_parameters(
                triangle_index, self.padding)
            ...
            # Tangent img coordinates normalize to the [0,1]?[-1,1]? tangent
            # plane
            ...
            triangle_points_tangent_nopad = np.array(
                triangle_param["triangle_points_tangent_nopad"])
            ...
            tangent_gnomonic_range = [
                ...
                gnomonic_y_max]
            pixel_eps = abs(
                tangent_xv[0, 0] - tangent_xv[0, 1]) / (2 * tangent_image_width)

            square_points_tangent = [
            ...
            tangent_sq_xv, tangent_sq_yv = gp.gnomonic2pixel(
                tangent_xv[inside_square_pixels_list],
            ...
            tangent_tri_xv, tangent_tri_yv = gp.gnomonic2pixel(
                tangent_xv[inside_tri_pixels_list],
            ...
            self.triangle_coordinates_tangent.append(
                [tangent_tri_xv, tangent_tri_yv])
            self.squared_coordinates_tangent.append(
                [tangent_sq_xv, tangent_sq_yv])
            self.triangle_coordinates_erp.append(
                [triangle_xv[inside_tri_pixels_list], triangle_yv[inside_tri_pixels_list]]
            )
            self.squared_coordinates_erp.append(
                [triangle_xv[inside_square_pixels_list], triangle_yv[inside_square_pixels_list]]
            )

    def erp_blendweights(
            self,
            sub_image_param_expression,
            erp_image_height,
            tangent_img_size,
            n_images=20):
        erp_image_width = 2 * erp_image_height
        ...
        if isinstance(sub_image_param_expression, str):
            tangent_cam_params = json.load(
                open(sub_image_param_expression.format(0)))
        elif isinstance(sub_image_param_expression, list):
            tangent_cam_params = sub_image_param_expression[0]
        else:
            log.error(
                "Camera parameter type error. {}".format(
                    type(sub_image_param_expression)))

        erp_radial_weights = np.full(
            [erp_image_height, erp_image_width, n_images], 0, np.float64)
        erp_frustum_weights = np.full(
            [erp_image_height, erp_image_width, n_images], 0, np.float64)

        tangent_img_blend_radial_weights = self.get_radial_blendweights(
            tangent_cam_params, tangent_img_size)

        # mx + b normalization. The slope is -1/(fov-inflection_point)
        # The bias is calculated knowing that in the inflection point we want a
        # weight of 1.
        focal_lengths = np.array(
            [
                tangent_cam_params["intrinsics"]["focal_length_y"],
                tangent_cam_params["intrinsics"]["focal_length_x"]])
        min_f_length = np.argmax(focal_lengths)
        fov = np.degrees(
            np.arctan(
                tangent_img_size[min_f_length] *
                0.5 /
                focal_lengths[min_f_length]))
        slope = -1 / (fov - self.inflection_point)
        bias = -self.inflection_point * slope + 1

        # Clip values out of bounds. Weights belong to [0,1]
        tangent_img_blend_radial_weights = np.clip(
            tangent_img_blend_radial_weights * slope + bias, 0, 1)

        tangent_img_blend_frustum_weights = self.get_frustum_blendweights(
            tangent_img_size)
        ...
        for triangle_index in range(0, n_images):
            ...
            erp_face_radial_weights = ndimage.map_coordinates(
                tangent_img_blend_radial_weights, [
                    tangent_sq_yv, tangent_sq_xv], order=1, mode="constant", cval=0.0)

            erp_face_frustum_weights = ndimage.map_coordinates(
                tangent_img_blend_frustum_weights, [
                    tangent_sq_yv, tangent_sq_xv], order=1, mode="constant", cval=0.0)

            erp_radial_weights[erp_sq_yv.astype(int), erp_sq_xv.astype(
                int), triangle_index] = erp_face_radial_weights

            erp_frustum_weights[erp_sq_yv.astype(int), erp_sq_xv.astype(
                int), triangle_index] = erp_face_frustum_weights
    ...
    def misc_data(self, tangent_images, erp_size):
        ...
        erp_depth_tensor = np.full(
            [erp_image_height, erp_image_width, len(tangent_images)], np.nan, np.float64)
        nn_blending = np.zeros(erp_size)

        if erp_image_width != erp_image_height * 2:
            raise Exception(
                "the ERP image dimession is {}".format(
                    np.shape(erp_depth_tensor)))
        ...
        for triangle_index in range(0, 20):
            ...
            erp_face_image = ndimage.map_coordinates(
                tangent_images[triangle_index], [
                    tangent_sq_yv, tangent_sq_xv], order=1, mode="constant", cval=0.0)

            nn_blending[erp_tri_yv.astype(int), erp_tri_xv.astype(int)] = ndimage.map_coordinates(
                tangent_images[triangle_index], [tangent_tri_yv, tangent_tri_xv], order=1, mode="constant", cval=0.0
            )

            erp_depth_tensor[erp_sq_yv.astype(int), erp_sq_xv.astype(
                int), triangle_index] = erp_face_image.astype(np.float64)

        return erp_depth_tensor, nn_blending

    def get_radial_blendweights(self, img_params, size):
        # Weights for each tangent image. Angular distance wrt to the principal
        # point
        height, width = size
        ...
        principal_point = np.array(
            [
                img_params["intrinsics"]["principal_point"][0],
                img_params["intrinsics"]["principal_point"][1],
                1.0])
        principal_point_vec = np.linalg.inv(
            img_params["intrinsics"]["matrix"]) @ principal_point[..., None]
        principal_point_vec = principal_point_vec / \
            np.linalg.norm(principal_point_vec)
        angles = np.degrees(
            np.arccos(
                np.clip(
                    np.dot(
                        points3d, principal_point_vec.squeeze()), -1.0, 1.0)))
        return angles
    ...
    def get_frustum_blendweights(self, size):
        ...
        # Distances to y=0, y=height, x=0, x=width lines. They are the 4
        ...
        total_dist = np.dstack(
            (dist_to_right, dist_to_left, dist_to_top, dist_to_bottom))
        ...
        peak_top_left = np.array(
            [np.min(peak_coors[0]), np.min(peak_coors[1])])
        peak_bottom_right = np.array(
            [np.max(peak_coors[0]), np.max(peak_coors[1])])

        unit_dir = np.array([1 / np.sqrt(2), 1 / np.sqrt(2)])
        top_left = (
            peak_top_left -
            2 *
            self.diagonal_percentage *
            unit_dir).astype(int)
        bottom_right = (
            peak_bottom_right +
            2 *
            self.diagonal_percentage *
            unit_dir).astype(int)
        total_dist[top_left[0]: bottom_right[0] +
                   1, top_left[1]: bottom_right[1] + 1] = 0
        total_dist = (total_dist - np.min(total_dist)) / np.ptp(total_dist)
        total_dist[top_left[0]: bottom_right[0] +
                   1, top_left[1]: bottom_right[1] + 1] = 1
        return total_dist
    ...
    def gradient_blending(
            self,
            equirect_tangent_imgs,
            equirect_weights,
            color_blended,
            eigen_solver=None):
        ...
        for i in range(0, n_images):
            ...
            grad_y = np.diff(img, axis=0, append=np.zeros_like(
                img[None, 0])) * weights
        ...
        if self.eigen_solver is not None:
            ...
            x, _ = scipy.sparse.linalg.cg(
                self.A.transpose().dot(
                    self.A), self.A.transpose().dot(b))
    ...
    def compute_linear_system_matrices(self, rows, cols, equirect_weights):
        ...
        # Mat A. [x_ffd_1, y_ffd_1, x_ffd_2, y_ffd_2, ... , x_ffd_n, y_ffd_n,
        # fidelity]
        blocks = []
