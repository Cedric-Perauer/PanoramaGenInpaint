...
def depth_ico_visual_save(
        depth_data_list_,
        output_path,
        subimage_idx_list=None):
    ...
    if (
        len(depth_data_list_) != 20
        and subimage_idx_list is not None
        and len(depth_data_list_) == len(subimage_idx_list)
    ):
        log.debug(
            "The ico's sub-image size is {}, fill blank sub-images.".format(len(depth_data_list_)))
        # depth_data_list = [np.zeros_like(depth_data_list_[0])] * 20
        # for subimage_index in range(len(subimage_idx_list)):
        #     subimage_face_idx = subimage_idx_list[subimage_index]
        #     depth_data_list[subimage_face_idx] = depth_data_list_[subimage_index]
        depth_data_list = fill_ico_subimage(
            depth_data_list_, subimage_idx_list)
    ...
    for row_index in range(0, 4):
        for col_index in range(0, 5):
            ...
            im = axes[row_index, col_index].imshow(
                depth_data_list[dispmap_index], cmap=cm.jet, vmin=vmin_, vmax=vmax_)
...
def rgb2dispmap(image_filepath, pytorch_hub=True):
    ...
    if pytorch_hub:
        ...
        if os.path.exists(MiDaS_module_filepath):
            log.error("MiDaS local module {} does not exist.".format(
                MiDaS_module_filepath))

        depthmap_data = run_depth(
            image_data,
            MiDaS_module_filepath,
            MonoDepthNet,
            MiDaS_utils)[0]
...
def Zoedepth_prediction(rgb_image_data_list):
    ...
    for index in range(0, len(rgb_image_data_list)):
        ...
        # print("zoedepth output original:", output.min(), output.max())
        # #1.6375176 5.4559774

        # print("zoedepth output processed:", output.min(), output.max())
        # #0.18328522 0.61068046
        ...
        if index % 10 == 0:
            log.debug(
                "ZoeDepth estimate {} rgb image's disparity map.".format(index))
    del model
    gc.collect()
    torch.cuda.empty_cache()
...
def Depth_Anything_prediction(rgb_image_data_list):
    model_configs = {
        "vits": {
            "encoder": "vits", "features": 64, "out_channels": [
                48, 96, 192, 384]}, "vitb": {
            "encoder": "vitb", "features": 128, "out_channels": [
                96, 192, 384, 768]}, "vitl": {
            "encoder": "vitl", "features": 256, "out_channels": [
                256, 512, 1024, 1024]}, "vitg": {
            "encoder": "vitg", "features": 384, "out_channels": [
                1536, 1536, 1536, 1536]}, }
...
def MiDaS_torch_hub_data(
    ...
    for index in range(0, len(rgb_image_data_list)):
        ...
        # [413,512] float32 maxeg=14 mineg=0.37
        ...
        if index % 10 == 0:
            log.debug(
                "MiDaS estimate {} rgb image's disparity map.".format(index))
...
def boosting_monodepth(rgb_image_data_list):
    ...
    # This import fixes relative imports in subfiles within
    # BoostingMonocularDepth project
    sys.path.append(
        os.path.abspath(
            os.path.join(
                boost_path,
    ...
    BoostingMonocularDepth.run.pix2pixmodel.save_dir = os.path.join(
        boost_path, "pix2pix", "checkpoints", "mergemodel")
    ...
    if option.depthNet == 0:
        ...
        BoostingMonocularDepth.run.midasmodel = MidasNet(
            midas_model_path, non_negative=True)
        BoostingMonocularDepth.run.midasmodel.to(device)
        BoostingMonocularDepth.run.midasmodel.eval()

    # Generate mask used to smoothly blend the local pathc estimations to the base estimate.
    # It is arbitrarily large to avoid artifacts during rescaling for each
    # crop.
    ...
    for image_ind, image in enumerate(rgb_image_data_list):
        ...
        whole_estimate = BoostingMonocularDepth.run.doubleestimate(
            img,
        ...
        BoostingMonocularDepth.run.factor = max(
        ...
        # Enumerate through all patches, generate their estimations and
        # refining the base estimate.
        for patch_ind in range(len(imageandpatchs)):
            ...
            patch_estimation = BoostingMonocularDepth.run.doubleestimate(
                patch_rgb,
                option.net_receptive_field_size,
            ...
            BoostingMonocularDepth.run.pix2pixmodel.set_input(
                patch_whole_estimate_base, patch_estimation)
...
def read_dpt(dpt_file_path):
    ...
    assert ext == ".dpt", exit(
        "readFlowFile: fname %s should have extension "
        ".flo"
        "" %
        dpt_file_path)
...
def write_pfm(path, image, scale=1):
    ...
    if image.dtype.name != "float32":
        ...
        log.warn(
            "The depth map data is {}, convert to float32 and save to pfm format.".format(
                image.dtype.name))
    ...
    if len(image_.shape) == 3 and image_.shape[2] == 3:  # color image
        color = True
    # greyscale
    elif len(image_.shape) == 2 or len(image_.shape) == 3 and image_.shape[2] == 1:
        color = False
    else:
...
def depth2disparity(depth_map, baseline=1.0, focal=1.0):
    ...
    disparity_map[no_zeros_index] = (
        baseline * focal) / depth_map[no_zeros_index]
...
def disparity2depth(disparity_map, baseline=1.0, focal=1.0):
    ...
    depth_map = np.full(disparity_map.shape, 0, np.float64)
    depth_map[no_zeros_index] = (
        baseline * focal) / disparity_map[no_zeros_index]
...
def dispmap_normalize(dispmap, method="", mask=None):
    ...
    if method == "naive":
        ...
        dev_dispmap = np.sum(
            np.abs(dispmap[mask] - median_dispmap)) / np.sum(mask)
...
def subdepthmap_erp2tang(subimage_depthmap_erp, gnomonic_coord_xy):
    ...
    center2pixel_length = np.sqrt(
        ...
        np.square(gnomonic_coord_y) +
        np.ones_like(gnomonic_coord_y))
    subimage_depthmap_persp = np.divide(
        subimage_depthmap_erp, center2pixel_length)
...
def depthmap_pyramid(depthmap_list, pyramid_layer_number, pyramid_downscale):
    ...
    depthmap_pryamid = [
        [0] * depthmap_number for i in range(pyramid_layer_number)]
    for index in range(0, depthmap_number):
        if pyramid_layer_number == 1:
            depthmap_pryamid[0][index] = depthmap_list[index].astype(
                np.float64)
        else:
            ...
            pyramid = tuple(
                pyramid_gaussian(
                    depthmap,
                    max_layer=pyramid_layer_number - 1,
                    downscale=pyramid_downscale,
                    multichannel=False))
            for layer_index in range(0, pyramid_layer_number):
                depthmap_pryamid[pyramid_layer_number - layer_index -
                                 1][index] = pyramid[layer_index].astype(np.float64)
