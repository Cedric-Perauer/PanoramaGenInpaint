...
def depthmap_deform(depthmap, scale_array, offset_array):
    ...
    scale_pixelwise = ndimage.map_coordinates(
        scale_array, [corr_yv_grid, corr_xv_grid], order=1, mode="constant", cval=0.0
    )
    offset_pixelwise = ndimage.map_coordinates(
        offset_array, [corr_yv_grid, corr_xv_grid], order=1, mode="constant", cval=0.0
    )
...
def stitch_depth_subimage(depth_data_list, image_param_list, fisheye_model):
    ...
    for index in range(0, len(depth_data_list)):
        ...
        pinhole_3d_points_original = (
            image_param["rotation"] @ fisheye_3d_points.T
        )  # + image_param['translation'][:, np.newaxis]
        pinhole_3d_points = np.divide(
            pinhole_3d_points_original, pinhole_3d_points_original[2, :])
        pinhole_2d_points = (
            image_param["intrinsics"]["matrix"] @ pinhole_3d_points)[:2, :]

        # just use  the available 3D point, in +z and have corresponding 2D
        # image points
        available_pixels_list = np.logical_and.reduce(
            (
                pinhole_3d_points_original[2, :] > 0,
                pinhole_2d_points[0, :] >= 0,
                pinhole_2d_points[0, :] < pinhole_image_width,
                pinhole_2d_points[1, :] >= 0,
                pinhole_2d_points[1, :] < pinhole_image_height,
            )
        )

        available_pixels_list_mat = available_pixels_list.reshape(
            (fisheye_image_height, fisheye_image_width))
        fisheye_depth[:,
                      :][available_pixels_list_mat] += ndimage.map_coordinates(depth_data[:,
                                                                                          :],
                                                                               [pinhole_2d_points[1,
    ...
    fisheye_depth[:,
                  :][available_weight_list] = np.divide(fisheye_depth[:,
...
def find_corresponding(
    ...
    src_image_u_list = np.linspace(
        0,
        src_image_width,
        src_image_width,
        endpoint=False)  # x
    src_image_v_list = np.linspace(
        0,
        src_image_height,
        src_image_height,
        endpoint=False)  # y
    src_image_grid_u, src_image_grid_v = np.meshgrid(
        src_image_u_list, src_image_v_list)
    src_image_grid_z = np.ones(src_image_grid_u.shape, float)
    src_image_2d_points = np.stack(
        (src_image_grid_u.ravel(),
         src_image_grid_v.ravel(),
         src_image_grid_z.ravel()),
        axis=1)

    # project the pinhole image to world coords (spherical 3D points)
    src_image_3d_points = np.linalg.inv(
        src_param["intrinsics"]["matrix"]) @ src_image_2d_points.T
    src_image_3d_points_world = np.linalg.inv(src_param["rotation"]) @ (
        src_image_3d_points - src_param["translation"][:, np.newaxis]
    )
    fisheye_2d_points = cam_models.world2cam_slow(
        src_image_3d_points_world, fisheye_model).T

    # 2) from fisheye image to target image
    fisheye_3d_points = cam_models.cam2world(fisheye_2d_points, fisheye_model)
    # pinhole_3d_points = (np.divide(fisheye_3d_points, fisheye_3d_points[2, :]))
    tar_image_3d_points = tar_param["rotation"] @ fisheye_3d_points.T + \
        tar_param["translation"][:, np.newaxis]

    # projection to pin-hole image
    tar_image_3d_points = np.divide(
        tar_image_3d_points, tar_image_3d_points[2, :])
    tar_image_2d_points = (
        tar_param["intrinsics"]["matrix"] @ tar_image_3d_points)[:2, :]

    # get the available pixels index
    available_pixels_list = np.logical_and.reduce(
        (
            tar_image_2d_points[0, :] >= 0,
            tar_image_2d_points[0, :] <= tar_image_width - 1,
            tar_image_2d_points[1, :] >= 0,
            tar_image_2d_points[1, :] <= tar_image_height - 1,
        )
    )
    ...
    if src_image_2d_points_available.shape[0] == 0:
        ...
        src_image_avail_pixel_data = src_image[src_image_2d_points_available.astype(
            int)]
        tar_image_avail_pixel_data = tar_image[tar_image_2d_points_available.astype(
            int)]
        rms = np.sqrt(
            np.mean(
                (src_image_avail_pixel_data -
                 tar_image_avail_pixel_data) ** 2))
    ...
    pixel_matching = np.hstack(
        (src_image_2d_points_available,
         tar_image_2d_points_available))
