...
class Options:
    ...
    def __init__(self) -> None:
        ...
        self.dispalign_weight_smooth = 40.0
    ...
    def parser_arguments(self, parser):
        ...
        parser.add_argument(
            "--data",
            type=str,
            default="data/pano/panorama_1.png",
        ...
        parser.add_argument(
            ...
            type=int,
            default=1,
            help="Levels of multi-resolution pyramid. If > 1"
        ...
        parser.add_argument(
            ...
            default=False,
            action="store_true",
            help="save intermediate data" "generated during the " "pipeline",
        )
        parser.add_argument(
            "--grid_search",
            default=False,
        ...
        parser.add_argument(
            ...
            type=int,
            default=0,
            help="Sample a subset from --data")
        ...
        self.dispalign_align_coeff_grid_width = int(
            opt_arguments.grid_size.lower().split("x")[0])
        self.dispalign_align_coeff_grid_height = int(
            opt_arguments.grid_size.lower().split("x")[1])
...
def depthmap_estimation(erp_rgb_image_data, fnc, opt, blendIt, idx=1):
    ...
    if 1 in opt.available_steps:
        ...
        subimage_rgb_list, _, points_gnomocoord = proj_ico.erp2ico_image(
            erp_rgb_image_data, opt.subimage_tangent_image_width, opt.subimage_padding_size, full_face_image=True)
        tangent_image_gnomo_xy = points_gnomocoord[1]

        if opt.debug_enable:
            log.debug(
                "Debug enable, Step 1 output subimage rgb to {}".format(
                    fnc.root_dir))
            for index in range(0, len(subimage_rgb_list)):
                src_image_output_path = fnc.subimage_rgb_filename_expression.format(
                    index)
                Image.fromarray(
                    subimage_rgb_list[index].astype(
                        np.uint8)).save(src_image_output_path)
                if index % 4 == 0:
                    print("Output subimage to {}.".format(
                        src_image_output_path))

            # output subimage's rgb to image array
            subimage_rgb_array_filepath = fnc.subimage_rgb_filename_expression.format(
                999)
            image_io.subimage_save_ico(
                subimage_rgb_list, subimage_rgb_array_filepath)
    ...
    if 2 in opt.available_steps:
        ...
        if not subimage_rgb_list or not tangent_image_gnomo_xy:
            ...
            for index in list(range(0, 20)):
                src_image_output_path = fnc.subimage_rgb_filename_expression.format(
                    index)
                subimage_rgb_list.append(np.asarray(
                    Image.open(src_image_output_path)))
            log.info("generate face gnomonic coordinate")
            _, _, points_gnomocoord = proj_ico.erp2ico_image(
                erp_rgb_image_data, opt.subimage_tangent_image_width, opt.subimage_padding_size, full_face_image=True)
            tangent_image_gnomo_xy = points_gnomocoord[1]
        ...
        subimage_dispmap_persp_list = depthmap_utils.run_persp_monodepth(
            subimage_rgb_list, opt.persp_monodepth)
        # convert disparity map to depth map
        for dispmap_persp in subimage_dispmap_persp_list:
            subimage_depthmap_persp_list.append(
                depthmap_utils.disparity2depth(dispmap_persp))
        ...
        for depthmap_persp in subimage_depthmap_persp_list:
            subimage_depthmap_erp_list.append(
                depthmap_utils.subdepthmap_tang2erp(
                    depthmap_persp, tangent_image_gnomo_xy))
        # convert each subimage's from ERP depth map to perspective map.
        for depthmap_erp in subimage_depthmap_erp_list:
            subimage_dispmap_erp_list.append(
                depthmap_utils.depth2disparity(depthmap_erp).astype(
                    np.float32))

        # output disparity map and visualized result.
        if opt.debug_enable:
            # output disparity map array
            dispmap_array_filename = fnc.subimage_dispmap_persp_filename_expression.format(
                999)
            depthmap_utils.depth_ico_visual_save(
                subimage_dispmap_persp_list,
                dispmap_array_filename + ".jpg")
            depthmap_persp_array_filename = fnc.subimage_depthmap_persp_filename_expression.format(
                999)
            depthmap_utils.depth_ico_visual_save(
                subimage_depthmap_persp_list,
                depthmap_persp_array_filename + ".jpg")
            depthmap_erp_array_filename = fnc.subimage_depthmap_erp_filename_expression.format(
                999)
            depthmap_utils.depth_ico_visual_save(
                subimage_depthmap_erp_list,
                depthmap_erp_array_filename + ".jpg")
            subimage_dispmap_erp_filepath = fnc.subimage_dispmap_erp_filename_expression.format(
                999)
            depthmap_utils.depth_ico_visual_save(
                subimage_dispmap_erp_list,
                subimage_dispmap_erp_filepath + ".jpg")

            # output disparity map
            for index in range(0, len(subimage_rgb_list)):
                depth_filename = fnc.subimage_dispmap_erp_filename_expression.format(
                    index)
                depthmap_utils.write_pfm(
                    depth_filename, subimage_dispmap_erp_list[index], scale=1)
                depthmap_utils.depth_visual_save(
                    subimage_dispmap_erp_list[index], depth_filename + ".jpg")
                if index % 4 == 0:
                    print(
                        "Output subimages disparity map to {}".format(depth_filename))
        toc = time.perf_counter()
    ...
    if 3 in opt.available_steps:
        ...
        if not subimage_dispmap_erp_list or not subimage_rgb_list:
            ...
            for index in list(range(0, 20)):
                ...
                depth_filename = fnc.subimage_dispmap_erp_filename_expression.format(
                    index)
                subimage_dispmap_erp_list.append(
                    depthmap_utils.read_pfm(depth_filename)[0])
        ...
        if opt.dispalign_output_dir is not None:
            # output cpp module alignment coefficient
            depthmap_aligner.output_dir = opt.dispalign_output_dir
        else:
        ...
        subimage_dispmap_list_sub = [
            subimage_dispmap_erp_list[i] for i in opt.subimage_available_list]
        dispmap_aligned_list, coeffs_scale, coeffs_offset, subimage_cam_param_list = depthmap_aligner.align_multi_res(
            erp_rgb_image_data, subimage_dispmap_list_sub, opt.subimage_padding_size, opt.subimage_available_list)
        ...
        if opt.debug_enable:
            ...
            if opt.dispalign_debug_enable:
                for subimage_index in opt.subimage_available_list:
                    subimage_filepath = fnc.subimage_dispmap_cpp_aligned_filename_expression.format(
                        subimage_index)
                    dispmap, _ = depthmap_utils.read_pfm(
                        str(subimage_filepath))
                    dispmap_vis_filepath = subimage_filepath + ".jpg"
                    depthmap_utils.depth_visual_save(
                        dispmap, dispmap_vis_filepath)

            # output the scale ans offset to json
            serialization.subimage_alignment_params(
                fnc.subimage_dispmap_aligned_coeffs_filename_expression,
                coeffs_scale,
                coeffs_offset,
            ...
            serialization.save_cam_params(
                fnc.subimage_camsparams_list_filename_expression,
                opt.subimage_available_list,
                subimage_cam_param_list)

            # output alignment disparity map array image
            depth_array_filename = fnc.subimage_dispmap_aligned_filename_expression.format(
                999)
            depthmap_utils.depth_ico_visual_save(
                dispmap_aligned_list,
                depth_array_filename + ".jpg",
                opt.subimage_available_list)
            ...
            for index in range(0, len(dispmap_aligned_list)):
                ...
                depthmap_utils.write_pfm(
                    depth_filename, dispmap_aligned_list[index].astype(
                        np.float32), scale=1)
                depthmap_utils.depth_visual_save(
                    dispmap_aligned_list[index], depth_filename + ".jpg")
                if index % 4 == 0:
                    print(
                        "Output aligned subimages disparity map to {}".format(depth_filename))
    ...
    if 4 in opt.available_steps:
        if not dispmap_aligned_list or not subimage_cam_param_list:
            ...
            for index in opt.subimage_available_list:
                ...
                if os.path.isfile(depth_filename):
                    dispmap_aligned_list.append(
                        depthmap_utils.read_pfm(depth_filename)[0])
                else:
                    dispmap_aligned_list.append(
                        subimage_dispmap_erp_list[index])
        ...
        if len(opt.subimage_available_list) == 20:
            if idx == 0:
                blendIt.tangent_images_coordinates(
                    erp_image_height, dispmap_aligned_list[0].shape)
                blendIt.erp_blendweights(
                    subimage_cam_param_list,
                    erp_image_height,
                    dispmap_aligned_list[0].shape)
                blendIt.compute_linear_system_matrices(
                    erp_image_height, erp_image_height * 2, blendIt.frustum_blendweights)

            erp_dispmap_blend = blendIt.blend(
                dispmap_aligned_list, erp_image_height)
        else:
            # available faces number is not 20, output subimages linear blend
            # result
            log.warn("Linear blend {} subimages.".format(
                len(opt.subimage_available_list)))
            # file with blank depth
            dispmap_aligned_list_filled = depthmap_utils.fill_ico_subimage(
                dispmap_aligned_list, opt.subimage_available_list
            )
            erp_dispmap_blend = proj_ico.ico2erp_image(
                dispmap_aligned_list_filled,
                erp_image_height,
...
def monodepth_360(opt):
    ...
    output_results_file = os.path.join(
        output_folder, "{}.txt".format(opt.expname))
    ...
    if opt.grid_search:
        ...
        energy_weights = np.array(
            [opt.dispalign_weight_smooth, opt.dispalign_weight_scale])[None, ...]

    # BlendIt object. Equation 7 of the paper
    blend_it = blending.BlendIt(
        opt.subimage_padding_size, len(
            opt.subimage_available_list), opt.blending_method)
    ...
    for weights in energy_weights:
        ...
        filename_base, _ = os.path.splitext(
            os.path.basename(erp_image_filename))
        ...
        estimated_depthmap, times = depthmap_estimation(
            erp_rgb_image_data, fnc, opt, blend_it, iter)

        serialization.save_predictions(
            output_folder,
            erp_rgb_image_data,
            estimated_depthmap,
            opt.persp_monodepth)
        ...
        if opt.grid_search:
            ...
            with open(output_results_file, "a") as f:
                np.savetxt(
                    f, metrics_list.reshape(
                        1, -1), delimiter=",", fmt="%1.11f")
...
def grid_search(fidelity_term=False):
    if not fidelity_term:
        ...
        exhaustive_pairs = np.array(np.meshgrid(
            weight_smooth, weight_scale)).T.reshape(-1, 2)
...
def check_weights_processed(file, weights):
    ...
    if exist:
        log.info("PASS on weights: " + ",".join(str(weight)
                 for weight in weights))
    else:
        log.info("USING weights: " + ",".join(str(weight)
                 for weight in weights))
