...
def run(dataset, option):
    ...
    for image_ind, images in enumerate(dataset):
        ...
        imageandpatchs = ImageandPatchs(
            option.data_dir, images.name, patchset, img, mergein_scale)
        ...
        for patch_ind in range(len(imageandpatchs)):
            ...
            if option.savepatchs:
                path = os.path.join(
                    patchped_est_outputpath,
                    imageandpatchs.name +
                    "_{:04}".format(patch_id))
                midas.utils.write_depth(
                    path,
                    patch_estimation,
                    bits=2,
...
# Generating local patches to perform the local refinement described in
...
def generatepatchs(img, base_size):
    ...
    # We use the integral image to speed-up the evaluation of the amount of
    # gradients for each patch.
    ...
    # each patch size to ensure that there will be enough depth cues for the
    # network to generate a consistent depth map.
...
def adaptiveselection(integral_grad, patch_bound_list, gf):
    ...
    for c in range(len(patch_bound_list)):
        ...
        # Compute the amount of gradients present in the patch from the
        # integral image.
...
def estimatesrl(img, msize):
    # SGRNet forward pass script adapted from
    # https://github.com/KexianHust/Structure-Guided-Ranking-Loss
    img_transform = transforms.Compose([transforms.ToTensor(), transforms.Normalize([
...
def estimatemidas(img, msize):
    # MiDas -v2 forward pass script adapted from
    # https://github.com/intel-isl/MiDaS/tree/v2
...
def estimateleres(img, msize):
    # LeReS forward pass script adapted from
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        default=1024,
        required=False)  # Do not change it
    parser.add_argument(
        "--depthNet",
        type=int,
        default=0,
    ...
    # field size to speed up the local refinement as described in the section
    # 6 of the main paper.
