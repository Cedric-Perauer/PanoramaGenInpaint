...
class DepthMergeDataset(BaseDataset):
    def __init__(self, opt):
        ...
        self.outer_paths = sorted(
            make_dataset(
                self.dir_outer,
                opt.max_dataset_size))
        self.inner_paths = sorted(
            make_dataset(
                self.dir_inner,
                opt.max_dataset_size))
        self.gtfake_paths = sorted(
            make_dataset(
                self.dir_gtfake,
                opt.max_dataset_size))
    ...
    def __getitem__(self, index):
        ...
        data_outer = Image.open(self.outer_paths[index % self.dataset_size])
        data_outer = np.array(data_outer, dtype=np.float32)
        data_outer = data_outer / normalize_coef

        # needs to be a tensor
        data_inner = Image.open(self.inner_paths[index % self.dataset_size])
        data_inner = np.array(data_inner, dtype=np.float32)
        data_inner = data_inner / normalize_coef

        if self.isTrain:
            # needs to be a tensor
            data_gtfake = Image.open(
                self.gtfake_paths[index % self.dataset_size])
            data_gtfake = np.array(data_gtfake, dtype=np.float32)
            data_gtfake = data_gtfake / normalize_coef

            data_inner = GuidedFilter(
                data_gtfake,
                data_inner,
                64,
                0.00000001).smooth.astype("float32")
            data_outer = GuidedFilter(
                data_outer,
                data_gtfake,
                64,
                0.00000001).smooth.astype("float32")
