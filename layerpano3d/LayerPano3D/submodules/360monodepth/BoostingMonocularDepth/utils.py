...
# miscellaneous function for reading, writing and processing rgb and depth
# images.
...
def getGF_fromintegral(integralimage, rect):
    # Computes the gradient density of a given patch from the gradient
    # integral image.
    x1 = rect[1]
    x2 = rect[1] + rect[3]
    y1 = rect[0]
...
def calculateprocessingres(
    ...
    # whole_size_threshold: maximum allowed resolution. (R_max from section 6
    # of the main paper)
    ...
    # speed scale parameter is to process every image in a smaller size to
    # accelerate the R_x resolution search
    speed_scale = 32
    ...
    # thresholding the gradient map to generate the edge-map as a proxy of the
    # contextual cues
    m = grad.min()
    M = grad.max()
    middle = m + (0.4 * (M - m))
    ...
    # Output resolution limit set by the whole_size_threshold and
    # scale_threshold.
...
class ImageandPatchs:
    ...
    def __getitem__(self, index):
        ...
        if self.do_have_estimate:
            patch_whole_estimate_base = impatch(
