...
def get_kernel(p): return torch.ones(1, 1, p * 2 + 1, p * 2 + 1).to("cuda")
...
def np2t(x): return (torch.as_tensor(x).to(
    torch.float32).permute(2, 0, 1) / 255.0)[None, ...].to("cuda")


def pad_mask(x, padamount=1): return t2np(F.conv2d(np2t(
    x[..., None]), get_kernel(padamount), padding=padamount))[..., 0].astype(bool)
...
class LayerPano:
    ...
    def create(
        ...
        for layer_idx in range(n_layer):
            self.traindata = self.load_pcd_and_perspectives(
                input_dir, layer_idx)
            if layer_idx == 0:
                n_iterations = 3001
            else:
            ...
            self.gaussians = LayerGaussian(
                self.opt.sh_degree, outlier_thresh=self.outlier_thresh)
            self.scene = Scene(
                self.traindata,
                gaussians_prev,
                self.gaussians,
    ...
    def render_video(self, preset, phi=0):
        ...
        if preset == "360":
            preset = "pers2pano"
            poses, theta_list, phi_list = get_pcdGenPoses(
                preset, {"n_views": 80, "phi": phi})
        else:
    ...
    def training(self, layer_idx, n_iterations):
        ...
        for iteration in tqdm(iterable_gauss):
            ...
            # Every 1000 its we increase the levels of SH up to a maximum
            # degree
            if iteration % 1000 == 0:
            ...
            # [c,h,w]
            image, mask, depth = render_pkg["render"], render_pkg["mask"], render_pkg["depth"]
            ...
            loss = (1.0 - self.opt.lambda_dssim) * Ll1 + self.opt.lambda_dssim * \
                (1.0 - ssim(image, gt_image))  # + 0.5 * Ldepth
            loss.backward()
            ...
            with torch.no_grad():
                ...
                if iteration < self.opt.densify_until_iter:
                    ...
                    self.gaussians.max_radii2D[visibility_filter_part] = torch.max(
                        self.gaussians.max_radii2D[visibility_filter_part],
                        radii[self.gaussians.identity_mask][visibility_filter_part],
                    )
                    self.gaussians.add_densification_stats(
                        viewspace_point_tensor, visibility_filter_part, self.gaussians.identity_mask)
                    ...
                    if iteration % self.opt.opacity_reset_interval == 0 or (
                            self.opt.white_background and iteration == self.opt.densify_from_iter):
    ...
    def compose_pano(self):
        ...
        for phi in phi_all:
            ...
            persdata, theta_list, phi_list = get_pcdGenPoses(
                "pers2pano", {"n_views": 10, "phi": phi})
            n_pers = len(persdata)

            path = os.path.join(self.pers_path, f"pers_phi{phi}")
            ...
            for i in range(n_pers):
                ...
                image.save(os.path.join(path, f"pers_{i}.jpg"))
                ...
                F_T_P_tmp.append(
                    [cam_fov90.fov_deg, theta_list[i], phi_list[i]])
    ...
    def pano2pers(self, pano, viewangle, N, time=None, name=None):
        ...
        for i in range(N):
            ...
            img = equ.GetPerspective(
                self.cam.fov_deg, theta, 0, self.cam.H, self.cam.W)
            ...
            if time:
                ...
                pil_img = Image.fromarray(img)
                pil_img.save(os.path.join(self.save_dir, f"{name}_{i}.jpg"))
    ...
    def load_pcd_and_perspectives(self, parent_dir, idx):
        ...
        pcd_points, pcd_colors = self.load_pcd(
            os.path.join(load_dir, f"pcd_rgb_layer{idx}.ply"))
        _, pcd_masks = self.load_pcd(os.path.join(
            load_dir, f"pcd_mask_layer{idx}.ply"))
        ...
        if len(pcd_points) > 3000000:
            ...
            print(
                "Warning: PointCloud is too large {}, downsampling by ratio of {}".format(
                    len(pcd_points), ratio))
            pcd_points = pcd_points[::ratio]
            pcd_colors = pcd_colors[::ratio]
            pcd_masks = pcd_masks[::ratio]

        print(
            "[INFO] !!! Loaded {} points from Layer {}.".format(
                pcd_points.shape, idx))
        num_frames = 24
        frames = []
        for frame_idx in range(num_frames):
            pers_rgb = Image.open(
                os.path.join(
                    load_dir,
                    f"frames/rgb_{frame_idx}.png"))
            pose_gs = np.load(
                os.path.join(
                    load_dir,
                    f"frames/transform_matrix_{frame_idx}.npy"))
