...
class RealESRGAN_degradation(object):
    ...
    def random_augment(self, img_gt):
        ...
        img_gt, status = augment(
            img_gt, hflip=True, rotation=False, return_status=True)
        """
        # random color jitter
    ...
    def degrade_process(self, img_gt, resize_bak=False):
        ...
        out = F.interpolate(out,
                            size=(int(ori_h / scale_final * scale),
                                  int(ori_w / scale_final * scale)),
                            mode=mode)
        # noise
