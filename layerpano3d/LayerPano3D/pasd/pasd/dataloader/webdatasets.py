...
def group_by_keys_nothrow(
    ...
    for filesample in data:
        ...
        # begins, rare, but can happen since prefix aren't unique across tar
        # files in that dataset
...
def tarfile_to_samples_nothrow(src, handler=wds.warn_and_continue):
    # NOTE this is a re-impl of the webdataset impl with group_by_keys that
    # doesn't throw
...
class Text2ImageDataset:
    def __init__(
        ...
        degradation = RealESRGAN_degradation(
            "params_realesrgan.yml", device="cpu")
        ...
        crop_preproc = transforms.Compose(
            [
                ...
                transforms.RandomHorizontalFlip() if random_flip else transforms.Lambda(
                    lambda x: x),
            ])
        ...
        def transform(example):
            if crop_preproc is not None:
                ...
                if control_type is not None:
                    if control_type == "realisr":
                        GT_image_t, LR_image_t = degradation.degrade_process(
                            np.asarray(image) / 255.0, resize_bak=resize_bak
                        )
                        example["conditioning_pixel_values"] = LR_image_t.squeeze(
                            0)
                        example["image"] = GT_image_t.squeeze(0) * 2.0 - 1.0
                    elif control_type == "grayscale":
                        image = np.asarray(
                            image.convert("L").convert("RGB")) / 255.0
                        example["conditioning_pixel_values"] = torch.from_numpy(
                            image).permute(2, 0, 1)
                    else:
            ...
            if tokenizer is not None:
                example["input_ids"] = tokenize_caption(
                    caption, tokenizer, null_text_ratio).squeeze(0)
...
class WebImageDataset(wds.DataPipeline, wds.compat.FluidInterface):
    def __init__(
        ...
        self.crop_preproc = transforms.Compose(
            [
                ...
                transforms.CenterCrop(
                    image_size) if center_crop else transforms.RandomCrop(image_size),
        ...
        # self.append(tarfile_to_samples(handler=handler)) # in case of
        # duplicated filename
    ...
    def preproc(self, example):
            ...
            if self.control_type is not None:
                if self.control_type == "realisr":
                    GT_image_t, LR_image_t = self.degradation.degrade_process(
                        np.asarray(image) / 255.0, resize_bak=self.resize_bak
                    )
                    example["conditioning_pixel_values"] = LR_image_t.squeeze(
                        0)
        ...
        if self.tokenizer is not None:
            example["input_ids"] = tokenize_caption(
                caption, self.tokenizer, self.null_text_ratio).squeeze(0)
