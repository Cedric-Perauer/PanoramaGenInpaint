...
def load_dreambooth_lora(
    ...
    if base_state_dict:
        ...
        for key in converted_unet_checkpoint:
            if key in unet_state_dict:
                converted_unet_checkpoint[key] = converted_unet_checkpoint[key] * \
                    blending_alpha + unet_state_dict[key] * (1.0 - blending_alpha)
            else:
                print(key)
    ...
    if is_lora:
        unet, text_encoder = convert_lora(
            unet, text_encoder, state_dict, multiplier=multiplier)
