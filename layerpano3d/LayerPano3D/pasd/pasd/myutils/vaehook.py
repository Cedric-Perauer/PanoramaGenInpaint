...
def attn_forward_new_xformers(self, hidden_states):
    ...
    if attention_mask is not None:
        ...
        #   [batch*heads, query_tokens, key_tokens]
        # we do this explicitly because xformers doesn't broadcast the
        # singleton dimension for us.
    ...
    if False:  # isinstance(net, AttnBlock):
        ...
        if is_xformers_available:
            task_queue.append(
                ("attn",
                 lambda x,
                 net=net: attn_forward_new_xformers(
                     net,
                     x)))
            ...
            task_queue.append(
                ("attn",
                 lambda x,
                 net=net: attn_forward_new_pt2_0(
                     net,
                     x)))
        else:
            task_queue.append(
                ("attn", lambda x, net=net: attn_forward_new(net, x)))
        task_queue.append(["add_res", None])
    ...
    for i_level in resolution_iter:
        ...
        if i_level != condition:
            if sd_flag:
                task_queue.append(
                    (func_name, getattr(
                        module[i_level], func_name)))
            else:
                if is_decoder:
                    task_queue.append(
                        (func_name, module[i_level].upsamplers[0]))
                else:
                    task_queue.append(
                        (func_name, module[i_level].downsamplers[0]))
...
def build_task_queue(net, is_decoder):
    ...
    # because encoder and decoder share the same architecture, we extract the
    # sampling part
...
def crop_valid_region(x, input_bbox, target_bbox, is_decoder):
    ...
    return x[:, :, margin[2]: x.size(
        2) + margin[3], margin[0]: x.size(3) + margin[1]]
...
class VAEHook:
    def __init__(
        ...
        self.fast_mode = (
            fast_encoder and not is_decoder) or (
    ...
    def split_tiles(self, h, w):
        ...
        # Suggestions from https://github.com/Kahsolt: auto shrink the tile
        # size
        ...
        for i in range(num_height_tiles):
            for j in range(num_width_tiles):
                # bbox: [x1, x2, y1, y2]
                # the padding is is unnessary for image borders. So we directly
                # start from (32, 32)
                input_bbox = [
                ...
                # if the output bbox is close to the image boundary, we extend
                # it to the image boundary
                output_bbox = [
                ...
                output_bbox = [
                    x *
                    8 if self.is_decoder else x //
                    8 for x in output_bbox]
    ...
    def vae_tile_forward(self, z):
        ...
        if self.fast_mode:
            ...
            # The downsampling will heavily distort its mean and std, so we
            # need to recover it.
        ...
        pbar.close()
        return result.to(
            dtype) if result is not None else result_approx.to(device)
