...
def load_pasd_pipeline(
    ...
    if args.control_type == "grayscale":
        scheduler = UniPCMultistepScheduler.from_pretrained(
            "/".join(args.pasd_model_path.split("/")[:-1]), subfolder="scheduler"
        )
    else:
        scheduler = UniPCMultistepScheduler.from_pretrained(
    ...
    unet = UNet2DConditionModel.from_pretrained(
        args.pasd_model_path, subfolder="unet")
    controlnet = ControlNetModel.from_pretrained(
        args.pasd_model_path, subfolder="controlnet")
    ...
    if args.use_personalized_model and args.personalized_model_path is not None:
        if os.path.isfile(
                f"{personalized_model_root}/{args.personalized_model_path}"):
            ...
            unet = UNet2DConditionModel.from_pretrained_orig(
                personalized_model_root,
                subfolder=f"{args.personalized_model_path}")  # unet_disney
    ...
    if args.use_lcm_lora:
        ...
        validation_pipeline.scheduler = LCMScheduler.from_config(
...
def get_validation_prompt(
    ...
    if args.high_level_info == "classification":
        ...
        if args.use_blip:
            ...
            caption = open_clip.decode(
                generated[0]).split("<end_of_text>")[0].replace(
                "<start_of_text>", "")
            caption = caption.replace(
                "blurry", "clear").replace(
                "noisy", "clean")  #
        ...
        for image_name in image_names[:]:
            ...
            if args.control_type == "realisr":
                validation_prompt = get_validation_prompt(
                    args, validation_image, model, preprocess, category)
                ...
                validation_prompt = get_validation_prompt(
                    args, validation_image, model, preprocess, category, accelerator.device)
            ...
            if min(
                    validation_image.size) < args.process_size or args.control_type == "grayscale":
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default="checkpoints/pasd/stable-diffusion-v1-5",
        help="path of base SD model",
    )
    parser.add_argument(
        "--lcm_lora_path",
        type=str,
        default="checkpoints/pasd/lcm-lora-sdv1-5",
        help="path of LCM lora model")
    parser.add_argument(
        "--pasd_model_path",
        type=str,
        default="checkpoints/pasd/checkpoint-100000",
        help="path of PASD model")
    parser.add_argument(
        "--personalized_model_path",
        type=str,
        default="majicmixRealistic_v7.safetensors",
        help="name of personalized dreambooth model, path is 'checkpoints/personalized_models'",
    )  # toonyou_beta3.safetensors, majicmixRealistic_v6.safetensors, unet_disney
    parser.add_argument(
        "--control_type",
        choices=[
            "realisr",
    ...
    parser.add_argument(
        "--high_level_info",
        choices=["classification", "detection", "caption"],
        nargs="?",
        default="caption",
        help="high level information for prompt generation",
    )
    ...
    parser.add_argument(
        "--negative_prompt",
        type=str,
        default="blurry, dotted, noise, raster lines, unclear, lowres, over-smoothed",
    ...
    parser.add_argument(
        "--inputs_dir",
        type=str,
        default="outputs_nature_inpaint",
        help="layerdata dir")
    parser.add_argument(
        "--mixed_precision",
        type=str,
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.0,
        help="blending alpha for personalized model")
    parser.add_argument(
        "--multiplier",
        type=float,
        default=0.6,
        help="multiplier for personalized lora model")
    parser.add_argument(
        "--num_inference_steps",
        type=int,
        default=20,
    ...
    parser.add_argument(
        ...
        type=int,
        default=768,
        help="minimal input size for processing")  # 512?
    parser.add_argument(
        "--decoder_tiled_size",
        type=int,
        default=224,
        help="decoder tile size for saving GPU memory")  # for 24G
    parser.add_argument(
        "--encoder_tiled_size",
        type=int,
        default=1024,
        help="encoder tile size for saving GPU memory")  # for 24G
    parser.add_argument(
        "--latent_tiled_size",
        type=int,
        default=320,
        help="unet latent tile size for saving GPU memory")  # for 24G
    parser.add_argument(
        "--latent_tiled_overlap",
        type=int,
        default=8,
        help="unet lantent overlap size for saving GPU memory")  # for 24G
    parser.add_argument(
        "--upscale",
        type=int,
        default=1,
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="use personalized model or not")
    parser.add_argument(
        "--use_pasd_light",
        action="store_true",
        help="use pasd or pasd_light")
    parser.add_argument(
        "--use_lcm_lora",
        action="store_true",
        help="use lcm-lora or not")
    parser.add_argument(
        "--use_blip",
        action="store_true",
        help="use lcm-lora or not")
    parser.add_argument(
        "--init_latent_with_noise",
        action="store_true",
        help="initial latent with pure noise or not")
    parser.add_argument(
        "--added_noise_level",
        type=int,
        default=900,
        help="additional noise level")
    parser.add_argument(
        "--offset_noise_scale",
        type=float,
        default=0.0,
        help="offset noise scale, not used")
