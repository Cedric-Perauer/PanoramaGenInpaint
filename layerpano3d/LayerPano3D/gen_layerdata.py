...
parser.add_argument(
    "--lora_path",
    type=str,
    default="checkpoints/pano_lora_720*1440_v1.safetensors",
...
num_layer = sum(1 for item in Path(base_dir).iterdir()
...
for i in range(num_layer):
    ...
    if i != 0:
        images_dir = "{}/layer{}/layer{}_inpaint.png".format(
            base_dir, n_layer + 1, n_layer + 1)
    else:
        images_dir = "{}/rgb.png".format(base_dir)

    image = Image.open(images_dir).convert("RGB").resize((size[1], size[0]))
    mask_sharp = (Image.open(os.path.join(base_dir,
                                          "layer{}/layer{}_mask.png".format(n_layer,
                                                                            n_layer))) .convert("L") .resize((size[1],
                                                                                                              size[0])))

    mask_smooth = get_smooth_mask(np.asarray(mask_sharp))

    mask_image = Image.fromarray(mask_smooth)
    Image.fromarray(mask_smooth * 255).save(os.path.join(base_dir,
                                                         "layer{}/layer{}_mask_smooth.png".format(n_layer, n_layer)))
    w, h = image.size
    ...
    if i != 0:
        ...
        mask2 = extend_mask(
            image_array,
            np.array(mask_sharp) / np.array(mask_sharp).max(),
            sam_masks,
            cumulative_mask,
            base_dir,
            n_layer,
        ...
        color_masked_img = draw_mask_overlay(
            mask_sharp.astype(bool), image_array)
        color_masked_img = Image.fromarray(color_masked_img.astype(np.uint8))
        color_masked_img.save(os.path.join(
            base_dir, "layer{}/layer{}_maskedimg_color.png".format(n_layer, n_layer)))

        mask_sharp_img.save(os.path.join(
            base_dir, "layer{}/layer{}_mask_new.png".format(n_layer, n_layer)))
        mask_image.save(os.path.join(
            base_dir, "layer{}/layer{}_mask_smooth_new.png".format(n_layer, n_layer)))
    ...
    tmp_size = (256, 512)
    pano_lama = lama_model(
        cv2.resize(
            pano, (tmp_size[1], tmp_size[0])), cv2.resize(
            pano_mask, (tmp_size[1], tmp_size[0])))  # (1024, 2048, 3) uint8 255
    pano_lama = pano_lama.squeeze().astype(np.uint8)

    pano = cv2.resize(pano_lama, (size[1], size[0])) * \
        pano_mask_lama + (1 - pano_mask_lama) * pano
