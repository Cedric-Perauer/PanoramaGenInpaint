...
#
# For permission requests, please contact robot0321@snu.ac.kr,
# esw0116@snu.ac.kr, namhj28@gmail.com, jarin.lee@gmail.com.
import os
import numpy as np
import torch
import math
...
def generate_seed_360(viewangle, n_views, alternate=False):  # 360,10
    ...
    alternate_poses = (lambda l: [l[0]] + [i for j in zip(
        l[1: len(l) // 2], l[-1: len(l) // 2: -1]) for i in j] + [l[len(l) // 2]])
...
def generate_seed_preset(n_views, phi=0):
    ...
    for i in range(N):
        ...
        render_poses[i, :3, :3] = np.matmul(
            np.array([[np.cos(th), 0, -np.sin(th)], [0, 1, 0], [np.sin(th), 0, np.cos(th)]]),
            np.array([[1, 0, 0], [0, np.cos(ph), -np.sin(ph)], [0, np.sin(ph), np.cos(ph)]]),
        )
...
def generate_pano360(n_views):
    ...
    _render_poses, _render_poses_pc, _theta_list, _phi_list = generate_360(
        n_views, phi=0)
...
def generate_seed_360_half(viewangle, n_views):
    ...
    for i in range(N):
        ...
        render_poses[i, :3, :3] = np.array(
            [[np.cos(th), 0, np.sin(th)], [0, 1, 0], [-np.sin(th), 0, np.cos(th)]])
        render_poses[i, :3, 3:4] = np.random.randn(
            3, 1) * 0.0  # Transition vector
    for i in range(N):
        th = -(halfangle / N) * i / 180 * np.pi
        render_poses[i + N,
                     :3,
                     :3] = np.array([[np.cos(th),
                                      0,
                                      np.sin(th)],
                                     [0,
                                      1,
                                     ...
                                     [-np.sin(th),
                                      ...
                                      np.cos(th)]])
        # Transition vector
        render_poses[i + N, :3, 3:4] = np.random.randn(3, 1) * 0.0
    return render_poses
...
def generate_seed_hemisphere(center_depth, degree=5):
    ...
    for i in range(len(thlist)):
        ...
        # central point of (hemi)sphere / you can change this value
        d = center_depth
        ...
        render_poses[i,
                     :3,
                     3:4] = np.array([d * np.sin(th / 180 * np.pi),
                                      0,
                                      d - d * np.cos(th / 180 * np.pi)]).reshape(3,
                                                                                 1) + np.array([0,
                                                                                                d * np.sin(phi / 180 * np.pi),
                                                                                                d - d * np.cos(phi / 180 * np.pi)]).reshape(3,
                                                                                                                                            1)  # Transition vector
...
def generate_seed_lookdown():
    ...
    philist = np.concatenate(
        (np.linspace(0, 0, 7), np.linspace(-22.5, -22.5, 7)))
    assert len(thlist) == len(philist)
...
def generate_seed_llff(degree, nviews, round=4, d=2.3):
    ...
    for i in range(len(thlist)):
        ...
        ).reshape(3, 1) + np.array([0, d * np.sin(phi / 180 * np.pi), -z + d - d * np.cos(phi / 180 * np.pi)]).reshape(
            3, 1
        )  # Transition vector
    return render_poses
...
def generate_seed_headbanging(maxdeg, nviews_per_round, round=3, fullround=1):
    ...
    thlist = (2.66 * radius * np.sin(np.linspace(0,
                                                 2 * np.pi * (round + fullround + round),
                                                 nviews_per_round * (round + fullround + round))))
    philist = radius * np.cos(np.linspace(0,
                                          2 * np.pi * (round + fullround + round),
                                          nviews_per_round * (round + fullround + round)))
    assert len(thlist) == len(philist)
...
def generate_seed_around(
    viewangle, n_views, alternate=True, phi_tree=[
        0, -45, 45]):
...
def get_pcdGenPoses(pcdgenpath, argdict={}):
    if pcdgenpath == "rotate360":
        render_poses = generate_seed_360(
            360, argdict["n_views"], argdict["alternate"])
    elif pcdgenpath == "lookaround":
        render_poses = generate_seed_around(
            360, 10, argdict["alternate"], argdict["phi_tree"])
