...
def run(
    ...
    if input_path is not None:
        ...
        for index, image_name in enumerate(image_names):
            ...
            # output
            if output_path is not None:
                filename = os.path.join(output_path, os.path.splitext(
                    os.path.basename(image_name))[0] + "-" + model_type)
                if not side:
        ...
        with torch.no_grad():
            ...
            while True:
                ...
                if frame is not None:
                    ...
                    original_image_bgr = np.flip(
                        original_image_rgb, 2) if side else None
                    content = create_side_by_side(
                    ...
                    if output_path is not None:
                        filename = os.path.join(
                            output_path, "Camera" + "-" + model_type + "_" + str(frame_index))
                        cv2.imwrite(filename + ".png", content)

                    alpha = 0.1
                    if time.time() - time_start > 0:
                        # exponential moving average
                        fps = (1 - alpha) * fps + alpha * \
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-t", "--model_type", default="dpt_beit_large_512", help="Model type: "
