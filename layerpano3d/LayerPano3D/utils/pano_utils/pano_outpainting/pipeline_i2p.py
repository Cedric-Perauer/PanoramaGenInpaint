...
class StableDiffusionImage2PanoPipeline(
        DiffusionPipeline,
    ...
    # Copied from
    # diffusers.pipelines.stable_diffusion.pipeline_stable_diffusion_img2img.StableDiffusionImg2ImgPipeline.get_timesteps
    ...
    # Copied from
    # diffusers.pipelines.stable_diffusion.pipeline_stable_diffusion_img2img.StableDiffusionImg2ImgPipeline.prepare_latents
    def prepare_latents(
            self,
            image,
            timestep,
    ...
    def __call__(
        ...
        def controlnet_forward(
            ...
            if self.config.addition_embed_type is not None:
                if self.config.addition_embed_type == "text":
                    ...
                    time_embeds = time_embeds.reshape(
                        (text_embeds.shape[0], -1))

                    add_embeds = torch.concat(
                        [text_embeds, time_embeds], dim=-1)
            ...
            if mask is not None:
                sample = (1 - mask.to(sample.dtype)) * sample + \
                    mask.to(sample.dtype) * controlnet_cond
            else:
        ...
        self.controlnet.forward = controlnet_forward.__get__(
        ...
        control_image = torch.cat([control_image,
                                   control_image[:,
                                                 :,
                                                 :,
                                                 : self.blend_extend * self.vae_scale_factor]],
                                  dim=-1)
        if mask is not None:
            mask = torch.cat([mask] * batch_size, dim=0)
            mask = torch.cat(
                [mask, mask[:, :, :, : self.blend_extend]], dim=-1)
            ...
            for i, t in enumerate(timesteps):
                ...
                if mask is not None:
                    mask_input = torch.cat(
                        [mask] * 2) if do_classifier_free_guidance else mask
                else:
                    mask_input = None
