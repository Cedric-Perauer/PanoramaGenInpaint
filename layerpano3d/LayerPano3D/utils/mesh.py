...
class Mesh:
        ...
        if front_dir != "+z":
            ...
            if "-z" in front_dir:
                T = torch.tensor([[1, 0, 0], [0, 1, 0], [0, 0, -1]],
                                 device=mesh.device, dtype=torch.float32)
            elif "+x" in front_dir:
                ...
                T = torch.tensor([[1, 0, 0], [0, 0, 1], [0, 1, 0]],
                                 device=mesh.device, dtype=torch.float32)
            elif "-y" in front_dir:
                T = torch.tensor([[1, 0, 0], [0, 0, -1], [0, 1, 0]],
                                 device=mesh.device, dtype=torch.float32)
            ...
            if "1" in front_dir:
                T @= torch.tensor([[0, -1, 0], [1, 0, 0], [0, 0, 1]],
                                  device=mesh.device, dtype=torch.float32)
            elif "2" in front_dir:
                T @= torch.tensor([[1, 0, 0], [0, -1, 0], [0, 0, 1]],
                                  device=mesh.device, dtype=torch.float32)
    ...
    def load_obj(cls, path, albedo_path=None, device=None):
        ...
        # NOTE: we ignore usemtl, and assume the mesh ONLY uses one material
        # (first in mtl)
        ...
        for line in lines:
            ...
            # v/vn/vt
                ...
                for i in range(
                        nv - 2):  # triangulate (assume vertices are ordered)
        ...
        if not use_vertex_color:
                ...
                for line in lines:
                    ...
                    if "map_Kd" in prefix:
                        albedo_path = os.path.join(
                            os.path.dirname(path), split_line[1])
            ...
            if albedo_path is None or not os.path.exists(albedo_path):
                ...
                albedo = np.ones((1024, 1024, 3), dtype=np.float32) * \
                    np.array([0.5, 0.5, 0.5])  # default color
            else:
            ...
            mesh.albedo = torch.tensor(
                albedo, dtype=torch.float32, device=device)
    ...
    def load_trimesh(cls, path, device=None):
        ...
        if _mesh.visual.kind == "vertex":
            ...
            vertex_colors = np.array(
                vertex_colors[..., :3]).astype(np.float32) / 255
            ...
            if isinstance(_material, trimesh.visual.material.PBRMaterial):
                texture = np.array(
                    _material.baseColorTexture).astype(
                    np.float32) / 255
            elif isinstance(_material, trimesh.visual.material.SimpleMaterial):
                texture = np.array(
                    _material.to_pbr().baseColorTexture).astype(
                    np.float32) / 255
            else:
                raise NotImplementedError(
                    f"material type {type(_material)} not supported!")
            mesh.albedo = torch.tensor(
                texture, dtype=torch.float32, device=device)
            print(f"[load_trimesh] load texture: {texture.shape}")
        else:
            texture = np.ones((1024, 1024, 3), dtype=np.float32) * \
        ...
        mesh.vt = torch.tensor(
            texcoords,
            dtype=torch.float32,
            device=device) if texcoords is not None else None
        mesh.vn = torch.tensor(
            normals,
            dtype=torch.float32,
            device=device) if normals is not None else None
    ...
    def auto_normal(self):
        i0, i1, i2 = self.f[:, 0].long(
        ), self.f[:, 1].long(), self.f[:, 2].long()
    ...
    def auto_uv(self, cache_path=None, vmap=True):
        ...
        if vmap:
            # remap v/f to vt/ft, so each v correspond to a unique vt.
            # (necessary for gltf)
    ...
    def align_v_to_vt(self, vmapping=None):
        ...
        if vmapping is None:
            ...
            vmapping = torch.zeros(
                ...
                dtype=torch.long,
                device=self.device)
            # scatter, randomly choose one if index is not unique
            vmapping[ft] = f
