...
try:
    ...
    log.error(
        "depthmapAlign python module do not install, please build and install it reference the readme.")
...
def scale_shift_linear_disp(rendered_depth, predicted_depth, mask, fuse=True):
    ...
    X = torch.cat(
        [predicted_disparity, torch.ones_like(predicted_disparity)], dim=1)
...
def depth_alignment(depth1, depth2, mask_strict):
    ...
    depth2_fixed = scale_shift_linear(
        torch.from_numpy(depth1),
        torch.from_numpy(depth2),
        max_val.astype(float),
        torch.from_numpy(mask4align),
        fuse=True,
    )
...
def align_depth_maps_lsq(render_depth, predict_depth, mask):
    ...
    M = np.array([[1, 0, params[0]], [0, 1, params[1]], [0, 0, 1]])

    h, w = predict_depth.shape
    aligned_predict_depth = cv2.warpAffine(
        predict_depth, M[:2], (w, h), flags=cv2.INTER_LINEAR)
...
class DepthmapAlign:
    ...
    def __init__(self, opt, output_dir, subimages_rgb, debug=False):
        ...
        # save the normalized depth map for each resolution
        self.subimage_depthmap_aligning_filepath_expression = None
        ...
        # the original 20 rgb subimages, if is not warp depth map
        self.subimages_rgb = subimages_rgb
        ...
        # the grid height of the finest grid.
        self.align_coeff_grid_height_finest = 16
        ...
        # clear the alignment run time, call depthmapAlign.shutdown when the
        # interpreter exits
        import atexit
    ...
    def align_coeff_init(self):
        ...
        for ico_face_index in self.depthmap_original_ico_index:
            align_coeff_initial_scale = np.full(
                (self.align_coeff_grid_height, self.align_coeff_grid_width), 1.0, np.float64)
            align_coeff_initial_offset = np.full(
                (self.align_coeff_grid_height, self.align_coeff_grid_width), 0.0, np.float64)
            self.align_coeff_initial_scale_list.append(
                align_coeff_initial_scale)
            self.align_coeff_initial_offset_list.append(
                align_coeff_initial_offset)
    ...
    def report_cost(self, depthmap_list, pixel_corr_list):
        ...
        for src_idx in range(0, 20):
            for tar_idx in range(0, 20):
                ...
                src_depthmap_points = ndimage.map_coordinates(
                    src_depthmap, [src_y, src_x], order=1, mode="constant", cval=0
                )
                tar_depthmap_points = ndimage.map_coordinates(
                    tar_depthmap, [tar_y, tar_x], order=1, mode="constant", cval=0
                )
    ...
    def align_single_res(
        ...
        if self.align_method not in ["group", "enum"]:
            log.error(
                "The depth map alignment method {} specify error! ".format(
                    self.align_method))
        ...
        if False:
            ...
            for src_key, _ in enumerate(pixels_corresponding_list):
                ...
                info_str = info_str + "\nsource image {}:\n".format(src_key)
                for tar_key, _ in enumerate(
                        pixels_corresponding_list[src_key]):
                    # print("\t Target image {} , pixels corr number is {}".format(tar_key, pixels_corresponding_list[src_key][tar_key].size))
                    info_str = info_str + \
                        "{}:{}  ".format(tar_key, pixels_corresponding_list[src_key][tar_key].size)
                    pixels_corr_number = pixels_corr_number + \
                        pixels_corresponding_list[src_key][tar_key].size
            print(info_str)
        ...
        try:
            ...
            ceres_setting_result = depthmapAlign.ceres_solver_option(
                self.ceres_thread_number,
                self.ceres_max_num_iterations,
                self.ceres_max_linear_solver_iterations,
                self.ceres_min_linear_solver_iterations,
            )
    ...
    def align_multi_res(
            ...
            subimage_depthmap,
            padding_size,
            depthmap_original_ico_index=None):
        """
        ...
        if depthmap_original_ico_index is None and len(
                subimage_depthmap) == 20:
        ...
        # normalize the data
        log.debug(
            "Normalization the depth map with {} norm method".format(
                self.depthmap_norm_mothod))
        subimage_depthmap_norm_list = []
        for depthmap in subimage_depthmap:
            subimage_depthmap_norm = depthmap_utils.dispmap_normalize(
                depthmap, self.depthmap_norm_mothod)
        ...
        if self.multi_res_grid:
            depthmap_pryamid = [
                subimage_depthmap_norm_list] * self.pyramid_layer_number
            # pyramid_grid = [[4, 3], [8, 7], [16, 14]]   # Values reported in
            # paper
            pyramid_grid = [
                [self.align_coeff_grid_width * (2**level), self.align_coeff_grid_height * (2**level)]
                for level in range(0, self.pyramid_layer_number)
            ]
        else:
            depthmap_pryamid = depthmap_utils.depthmap_pyramid(
                subimage_depthmap_norm_list, self.pyramid_layer_number, self.pyramid_downscale)
        ...
        for pyramid_layer_index in range(0, self.pyramid_layer_number):
            ...
            log.info("Aligen the depth map in resolution {}".format(
                depthmap_pryamid[pyramid_layer_index][0].shape))
            ...
            if self.debug:
                ...
                if self.subimage_alignment_intermedia_filepath_expression is not None:
                    pickle_file_path = self.subimage_alignment_intermedia_filepath_expression.format(
                        image_size_str)
                    if os.path.exists(pickle_file_path) and os.path.getsize(
                            pickle_file_path) > 0:
                        log.warn(
                            "Load depthmap alignment data from {}".format(pickle_file_path))
                        alignment_data = None
            ...
            if pixel_corr_list is None or subimage_cam_param_list is None:
                _, subimage_cam_param_list, pixel_corr_list = subimage.erp_ico_proj(
                    erp_rgb_image_data, padding_size, tangent_image_width, self.downsample_pixelcorr_ratio, self.opt)

            # save intermedia data for debug output pixel corresponding
            # relationship and warped source image
            if self.debug:
                tangent_image_height = depthmap_pryamid[pyramid_layer_index][0].shape[0]
                image_size_str = "{}x{}".format(
                    tangent_image_height, tangent_image_width)
                ...
                if self.subimage_alignment_intermedia_filepath_expression is not None:
                    ...
                    with open(pickle_file_path, "wb") as file:
                        pickle.dump(
                            {
                                "subimage_depthmap": subimage_depthmap,
                        ...
                        log.warn(
                            "Save depth map alignment data to {}".format(pickle_file_path))

                # output the all subimages depth map corresponding relationship
                # to json
                if self.subimage_pixelcorr_filepath_expression is not None:
                    log.debug(
                        "output the all subimages corresponding relationship to {}".format(
                            self.subimage_pixelcorr_filepath_expression))
                    for subimage_index_src in range(0, 20):
                        for subimage_index_tar in range(0, 20):
                            ...
                            json_file_path = self.subimage_pixelcorr_filepath_expression.format(
                                subimage_index_src, subimage_index_tar, image_size_str)
                            serialization.pixel_corresponding_save(
                                json_file_path,
                                str(subimage_index_src),
                                None,
                                str(subimage_index_tar),
                                None,
                                pixel_corresponding,
                            )

                # draw the corresponding relationship in available subimage rgb
                # images
                if self.subimage_warpedimage_filepath_expression is not None and self.subimages_rgb is not None:
                    log.debug(
                        "draw the corresponding relationship in subimage rgb and output to {}".format(
                            self.subimage_warpedimage_filepath_expression))
                    for index_src in range(len(depthmap_original_ico_index)):
                        for index_tar in range(
                                len(depthmap_original_ico_index)):
                            ...
                            _, _, src_warp = subimage.draw_corresponding(
                                src_image_rgb, tar_image_rgb, pixel_corresponding
                            )
                            warp_image_filepath = self.subimage_warpedimage_filepath_expression.format(
                                face_index_src, face_index_tar, image_size_str)
                            image_io.image_save(src_warp, warp_image_filepath)

                # draw the corresponding relationship in available subimage
                # depth maps
                if self.subimage_warpeddepth_filename_expression is not None:
                    log.debug(
                        "draw the corresponding relationship in subimage depth map and output to {}".format(
                            self.subimage_warpeddepth_filename_expression))
                    for index_src in range(len(depthmap_original_ico_index)):
                        for index_tar in range(
                                len(depthmap_original_ico_index)):
                            ...
                            # visualize depth map
                            src_image_rgb = depthmap_utils.depth_visual(
                                src_image_data)
                            tar_image_rgb = depthmap_utils.depth_visual(
                                tar_image_data)
                            # draw relationship
                            face_index_src = depthmap_original_ico_index[index_src]
                            face_index_tar = depthmap_original_ico_index[index_tar]
                            ...
                            warp_image_filepath = self.subimage_warpeddepth_filename_expression.format(
                                face_index_src, face_index_tar, image_size_str)
                            image_io.image_save(src_warp, warp_image_filepath)
                ...
                if self.subimage_depthmap_aligning_filepath_expression is not None:
                    log.debug(
                        "output subimage's depth map of multi-layers: layer {}".format(pyramid_layer_index))
                    for index in range(len(depthmap_original_ico_index)):
                        ...
                        subimage_depthmap_filepath = self.subimage_depthmap_aligning_filepath_expression.format(
                            face_index, image_size_str)
                        depthmap_utils.depth_visual_save(
                            image_data, subimage_depthmap_filepath)

            # 1-1) align depth maps, to update align coeffs and subimages depth
            # maps.
            if self.multi_res_grid:
                if self.depthmap_aligned is not None:
                    self.align_single_res(
                        self.depthmap_aligned, pixel_corr_list)
                else:
                    self.align_single_res(
                        depthmap_pryamid[pyramid_layer_index], pixel_corr_list)
            else:
                self.align_single_res(
                    depthmap_pryamid[pyramid_layer_index],
                    pixel_corr_list)
            ...
            if self.multi_res_grid:
                if pyramid_layer_index < self.pyramid_layer_number - 1:
                    ...
                    for i in range(0,
                                   len(self.align_coeff_initial_scale_list)):
                        self.align_coeff_initial_scale_list[i] = cv2.resize(
                            self.align_coeff_initial_scale_list[i],
                            dsize=pyramid_grid[pyramid_layer_index + 1],
                            interpolation=cv2.INTER_LINEAR,
                        )

                        self.align_coeff_initial_offset_list[i] = cv2.resize(
                            self.align_coeff_initial_offset_list[i],
                            dsize=pyramid_grid[pyramid_layer_index + 1],
                            interpolation=cv2.INTER_LINEAR,
                        )
...
class Pano_depth_estimation:
    def __init__(
        ...
        # ['midas3', 'DepthAnythingv2'，’zoedepth‘]
        self.persp_monodepth = depth_model
        ...
        opt_dict = {
            ...
            # ['poisson', 'frustum', 'radial', 'nn', 'mean', 'all']
            "blending_method": "frustum",
        }
    ...
    def panorama_to_tangent_images(self):
        ...
        self.subimg_rgb_list, _, points_gnomocoord = erp2ico_image(
            self.pano_rgb, self.opt.tangent_img_width, padding_size=self.opt.subimage_padding_size, full_face_image=True)
        self.subimg_gnomo_xy = points_gnomocoord[1]

    @torch.no_grad()
    def depth_estimation(self):
        # estimate disparity map
        self.dispmap_persp_list = depthmap_utils.run_persp_monodepth(
            self.subimg_rgb_list, self.opt.persp_monodepth)

        erp_pred = depthmap_utils.run_persp_monodepth(
            [self.pano_rgb], self.opt.persp_monodepth)[0]
        self.erp_mask = np.abs(
            (erp_pred - erp_pred.min()) / (erp_pred.max() - erp_pred.min())) > 0
        ...
        for dispmap_persp in self.dispmap_persp_list:
            ...
            depthmap_erp = depthmap_utils.subdepthmap_tang2erp(
                depthmap_persp, self.subimg_gnomo_xy)
            dispmap_erp = depthmap_utils.depth2disparity(
                depthmap_erp).astype(np.float32)
        ...
        depthmap_utils.depth_visual_save(
            erp_pred, f"{self.depth_align_dir}/erp_pred.png")
        depthmap_utils.depth_visual_save(
            self.erp_mask, f"{self.depth_align_dir}/erp_mask_pred.png")

        depthmap_utils.depth_ico_visual_save(
            self.dispmap_persp_list,
            f"{self.depth_align_dir}/dispmap_persp.png")
        depthmap_utils.depth_ico_visual_save(
            self.depthmap_persp_list,
            f"{self.depth_align_dir}/depthmap_persp.png")
        depthmap_utils.depth_ico_visual_save(
            self.depthmap_erp_list,
            f"{self.depth_align_dir}/depthmap.png")
        depthmap_utils.depth_ico_visual_save(
            self.dispmap_erp_list,
            f"{self.depth_align_dir}/dispmap.png")

    @torch.no_grad()
    def depth_alignment(self):
        # from utils import depthmap_align
        depthmap_aligner = DepthmapAlign(
            self.opt,
            self.depth_align_dir,
            self.subimg_rgb_list,
            debug=True)

        subimage_available_list = list(range(len(self.dispmap_erp_list)))
        self.dispmap_aligned_list, coeffs_scale, coeffs_offset, self.subimg_cam_list = depthmap_aligner.align_multi_res(
            self.pano_rgb, self.dispmap_erp_list, self.opt.subimage_padding_size, subimage_available_list)

        blend_it = blending.BlendIt(
            self.opt.subimage_padding_size,
            len(subimage_available_list),
            self.opt.blending_method)
        blend_it.fidelity_weight = 0.1

        pano_rgb_height = self.pano_rgb.shape[0]
        blend_it.tangent_images_coordinates(
            pano_rgb_height, self.dispmap_aligned_list[0].shape)
        blend_it.erp_blendweights(
            self.subimg_cam_list,
            pano_rgb_height,
            self.dispmap_aligned_list[0].shape)
        blend_it.compute_linear_system_matrices(
            pano_rgb_height,
            pano_rgb_height * 2,
            blend_it.frustum_blendweights)

        erp_dispmap_blend = blend_it.blend(
            self.dispmap_aligned_list, pano_rgb_height)
        ...
        if self.persp_monodepth != "zoedepth":
            erp_depthmap_blend_save = np.full(
                erp_dispmap_blend_save.shape, 0, np.float64)
            bias = erp_dispmap_blend_save[self.erp_mask].max(
            ) - erp_dispmap_blend_save[self.erp_mask].min()

            erp_depthmap_blend_save[self.erp_mask] = (erp_dispmap_blend_save[self.erp_mask].max(
            ) - erp_dispmap_blend_save[self.erp_mask] + bias / 4)
            erp_depthmap_blend_save[self.erp_mask] = (
                erp_depthmap_blend_save[self.erp_mask] / erp_depthmap_blend_save[self.erp_mask].max()
            )
            ...
            erp_dispmap_blend_save = (
                erp_dispmap_blend_save - erp_dispmap_blend_save.min()) / (
                erp_dispmap_blend_save.max() - erp_dispmap_blend_save.min())
            np.save(
                os.path.join(
                    self.save_dir,
                    "erp_dispmap_blend_save.npy"),
                erp_dispmap_blend_save)
            print("============= Pass ============")
