...
def nearest_neighbor_fill(img, mask, erosion=0):
    ...
    if erosion > 0:
        eroded_mask = skimage.morphology.binary_erosion(
            mask.cpu().numpy(), footprint=skimage.morphology.disk(erosion))
    else:
        eroded_mask = mask.cpu().numpy()

    img_[eroded_mask <= 0] = np.nan

    distance_to_boundary = ndimage.distance_transform_bf(
        (~eroded_mask > 0), metric="cityblock")

    for current_dist in np.unique(distance_to_boundary)[1:]:
        ii, jj = np.where(distance_to_boundary == current_dist)

        ii_ = np.array([ii - 1, ii, ii + 1, ii - 1, ii, ii +
                       1, ii - 1, ii, ii + 1]).reshape(9, -1)
        jj_ = np.array([jj - 1, jj - 1, jj - 1, jj, jj, jj,
                       jj + 1, jj + 1, jj + 1]).reshape(9, -1)
...
def snap_high_gradients_to_nn(depth, threshold=20):
    ...
    grad = skimage.filters.rank.gradient(
        grad_depth, skimage.morphology.disk(1))
    return nearest_neighbor_fill(
        depth, torch.from_numpy(
            grad < threshold).to(
            depth.device), erosion=3)
...
def project_points(cameras, depth, use_pixel_centers=True):
    ...
    depth_t = torch.from_numpy(depth) if isinstance(
        depth, np.ndarray) else depth
    ...
    xy_depth_world = (cameras.get_world_to_view_transform().inverse(
    ).transform_points(directions.view(-1, 3)).unsqueeze(0))
...
def save_point_cloud_as_ply(points, filename="output.ply", colors=None):
    ...
    assert points.dim() == 2 and points.size(
        1) == 3, "Input tensor should be of shape [N, 3]."

    if colors is not None:
        assert colors.dim() == 2 and colors.size(
            1) == 3, "Color tensor should be of shape [N, 3]."
        assert points.size(0) == colors.size(
            0), "Points and colors tensors should have the same number of entries."
    ...
    if colors is not None:
        header.extend(["property uchar red",
    ...
    with open(filename, "w") as f:
        ...
        for i in range(points.size(0)):
            ...
            if colors is not None:
                # Scale color values from [0, 1] to [0, 255] and convert to
                # integers
