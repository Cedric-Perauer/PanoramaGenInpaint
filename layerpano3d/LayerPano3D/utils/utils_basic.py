...
def get_bagel_mask(mask_img):
    ...
    for i in range(0, h - filter_size, 5):
        for j in range(0, w - filter_size, 5):
            if np.sum(mask[i: i + filter_size,
                           j: j + filter_size]) < filter_size**2:
                small_mask = mask[i: i + filter_size, j: j + filter_size] > 0.5
                new_small_mask = new_mask[i: i +
                                          filter_size, j: j + filter_size]
                new_small_mask[small_mask] = 0.5
                new_mask[i: i + filter_size,
                         j: j + filter_size] = new_small_mask
...
def get_bagel_mask2(mask_smooth, mask_sharp):
    ...
    mask_diff = np.logical_xor(
        mask_smooth.astype(bool),
        mask_sharp.astype(bool))
...
def make_bagel_mask(mask_path, mask_smooth_path):
    ...
    mask_bagel[np.asarray(mask).astype(bool)] = mask_bagel_shape[np.asarray(
        mask).astype(bool)]  # (h, w) float64 [0-1]

    # mask_bagel = Image.fromarray((mask_bagel*255).astype(np.uint8)) #PIL
    # uint8 [0-255]
