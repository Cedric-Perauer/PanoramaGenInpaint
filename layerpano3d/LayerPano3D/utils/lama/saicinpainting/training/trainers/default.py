...
class DefaultInpaintingTrainingModule(BaseInpaintingTrainingModule):
    def __init__(
        ...
        **kwargs
        ...
        self.refine_mask_for_losses = (make_mask_distance_weighter(
            **distance_weighter_kwargs) if distance_weighter_kwargs is not None else None)
    ...
    def generator_loss(self, batch):
        ...
        if self.config.losses.feature_matching.weight > 0:
            need_mask_in_fm = OmegaConf.to_container(
                self.config.losses.feature_matching).get(
                "pass_mask", False)
