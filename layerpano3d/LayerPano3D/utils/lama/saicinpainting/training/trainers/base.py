...
# , make_discriminator
from utils.lama.saicinpainting.training.modules import make_generator

# from annotator.lama.saicinpainting.training.visualizers import make_visualizer
from utils.lama.saicinpainting.utils import (
    add_prefix_to_keys,
    average_dicts,
    set_requires_grad,
    flatten_dict,
...
class BaseInpaintingTrainingModule(ptl.LightningModule):
    ...
    def validation_epoch_end(self, outputs):
        ...
        test_evaluator_states = [s["test_evaluator_state"]
                                 for s in outputs if "test_evaluator_state" in s]
    ...
    def _do_step(
        ...
        if self.get_ddp_rank() in (
                None, 0) and (
                batch_idx %
                self.visualize_each_iters == 0 or mode == "test"):
            if self.config.losses.adversarial.weight > 0:
    ...
    def get_ddp_rank(self):
        return self.trainer.global_rank if (
            ...
            self.trainer.num_processes) > 1 else None
