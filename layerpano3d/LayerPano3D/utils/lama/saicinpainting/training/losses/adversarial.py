...
class NonSaturatingWithR1(BaseAdversarialLoss):
    def __init__(
        ...
        # otherwise we teach only the discr to pay attention to very small
        # difference
        ...
        # then it doesn't matter if the value of mask_as_fake_target is true or
        # false
...
class BCELoss(BaseAdversarialLoss):
    ...
    def generator_loss(self,
                       discr_fake_pred: torch.Tensor) -> Tuple[torch.Tensor,
                                                               Dict[str,
                                                                    torch.Tensor]]:
        real_mask_gt = torch.zeros(
            discr_fake_pred.shape).to(
