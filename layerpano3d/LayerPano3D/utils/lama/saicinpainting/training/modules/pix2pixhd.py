...
from utils.lama.saicinpainting.training.modules.base import (
    BaseDiscriminator,
    deconv_factory,
    get_conv_block_ctor,
    get_norm_layer,
...
class MultiDilatedGlobalGenerator(nn.Module):
    def __init__(
        ...
        resnet_conv_layer = functools.partial(
            get_conv_block_ctor("multidilated"),
            **multidilation_kwargs)
        ...
        for i in range(n_downsampling):
            ...
            model += [conv_layer(min(max_features,
                                     ngf * mult),
                                 min(max_features,
                                     ngf * mult * 2),
                                 kernel_size=3,
        ...
        for i in range(n_downsampling):
            ...
            model += deconv_factory(deconv_kind,
                                    ngf,
                                    mult,
                                    up_norm_layer,
                                    up_activation,
...
class ConfigGlobalGenerator(nn.Module):
    def __init__(
        ...
        conv_layer = get_conv_block_ctor(conv_kind)
        resnet_conv_layer = functools.partial(
            get_conv_block_ctor(resnet_conv_kind),
            **multidilation_kwargs)
        norm_layer = get_norm_layer(norm_layer)
        if affine is not None:
            norm_layer = partial(norm_layer, affine=affine)
        ...
        if len(manual_block_spec) == 0:
            manual_block_spec = [
                DotDict(
                    lambda: None, {
                        "n_blocks": n_blocks, "use_default": True})]
...
class GlobalGenerator(nn.Module):
    def __init__(
        ...
        for i in range(n_blocks):
            ...
            if ffc_positions is not None and i in ffc_positions:
                for _ in range(
                        ffc_positions[i]):  # same position can occur more than once
                    model += [
                        FFCResnetBlock(
                            feats_num_bottleneck,
                            padding_type,
