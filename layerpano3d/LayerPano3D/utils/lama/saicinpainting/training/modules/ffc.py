...
class FourierUnit(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.spectral_pos_encoding:
            ...
            coords_vert = torch.linspace(
                0, 1, height)[
                None, None, :, None].expand(
                batch, 1, height, width).to(ffted)
            coords_hor = torch.linspace(
                0, 1, width)[
                None, None, None, :].expand(
                batch, 1, height, width).to(ffted)
...
class FFCResnetBlock(nn.Module):
    ...
    def forward(self, x):
        if self.inline:
            x_l, x_g = x[:, : -
                         self.conv1.ffc.global_in_num], x[:, -
...
class FFCNLayerDiscriminator(BaseDiscriminator):
    def __init__(
        ...
        cur_model = [FFC_BN_ACT(nf_prev,
                                nf,
                                kernel_size=kw,
                                stride=1,
                                padding=padw,
                                norm_layer=norm_layer,
                                activation_layer=lambda *args,
                                **kwargs: nn.LeakyReLU(*args,
                                                       negative_slope=0.2,
                                                       **kwargs),
                                **conv_kwargs),
                     ConcatTupleLayer(),
