...
class DinoVisionTransformer(nn.Module):
    def __init__(
        ...
        if block_chunks > 0:
            ...
            for i in range(0, depth, chunksize):
                # this is to keep the block index consistent if we chunk the
                # block list
    ...
    def interpolate_pos_encoding(self, x, w, h):
        ...
        patch_pos_embed = nn.functional.interpolate(
            patch_pos_embed.reshape(
                1, int(sqrt_N), int(sqrt_N), dim).permute(
                0, 3, 1, 2),
            scale_factor=(sx, sy),
    ...
    def _get_intermediate_layers_not_chunked(self, x, n=1):
        ...
        blocks_to_take = range(
            total_block_len - n,
            total_block_len) if isinstance(
            n,
            int) else n
