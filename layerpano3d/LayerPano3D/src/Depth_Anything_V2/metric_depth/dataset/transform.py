...
def apply_min_size(sample, size, image_interpolation_method=cv2.INTER_AREA):
    ...
    # resize
    sample["image"] = cv2.resize(sample["image"], tuple(
        shape[::-1]), interpolation=image_interpolation_method)

    sample["disparity"] = cv2.resize(sample["disparity"], tuple(
            ...
            if "semseg_mask" in sample:
                ...
                sample["semseg_mask"] = F.interpolate(
                    torch.from_numpy(
                        ...
                        None, None, ...], (height, width), mode="nearest").numpy()[
                    0, 0]
