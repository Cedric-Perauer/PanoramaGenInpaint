...
def create_from_pcd(pcd, path):
    ...
    features = torch.zeros(
        (fused_color.shape[0], 3, (0 + 1) ** 2)).float().cuda()
    ...
    features_dc = nn.Parameter(features[:, :, 0:1].transpose(
        1, 2).contiguous().requires_grad_(True))
    features_rest = nn.Parameter(features[:, :, 1:].transpose(
        1, 2).contiguous().requires_grad_(True))
    ...
    f_dc = features_dc.detach().transpose(
        1, 2).flatten(
        start_dim=1).contiguous().cpu().numpy()
    f_rest = features_rest.detach().transpose(
        1, 2).flatten(
        start_dim=1).contiguous().cpu().numpy()
    ...
    dtype_full = [
        (attribute,
         "f4") for attribute in construct_list_of_attributes(
            features_dc,
            features_rest,
            scaling,
            rotation)]
...
def composition(
        ...
        rots_rgb,
        path_2):
    xyz_binary, features_dc_binary, features_extra_binary, opacities_binary, scales_binary, rots_binary = load_ply(
        path_2)
    ...
    features_rest = np.concatenate(
        (features_extra_rgb, features_extra_binary), axis=0)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="The path to the Original Gaussians.")
    parser.add_argument(
        "--supp_ply",
        type=str,
        required=True,
        help="The path to the Inpainting Gaussians")
    parser.add_argument(
        "--save_ply",
        type=str,
        required=True,
        help="The path to save the Gaussians.")
    parser.add_argument(
        "--temp_ply",
        type=str,
        default="temp.ply",
        help="The path to save the Temporary Gaussians.")
    parser.add_argument(
        "--nb_points",
        type=int,
        default=100,
        help="Number of points for the remove_radius_outlier function.")
    parser.add_argument(
        "--radius",
        type=float,
        default=0.1,
        help="Radius for the remove_radius_outlier function.")
    parser.add_argument(
        "--threshold",
        type=float,
        default=1.0,
        help="Threshold for the similar_points_tree function.")
    ...
    xyz, features_dc, features_extra, opacities, scales, rots = load_ply(
        processed_supp)
    print("There are {} points in the inpainted Gaussians.".format(len(xyz)))
    xyz2, features_dc2, features_extra2, opacities2, scales2, rots2 = load_ply(
        origin)
    ...
    print(
        "There are {} similar points in the two point clouds.".format(
            len(xyz_similar)))
    ...
    xyz_rgb, features_dc, features_rest, opacity, scaling, rotation = composition(
        xyz_rgb, features_dc, features_rest, opacity, scaling, rotation, processed_supp)
    xyz, features_dc, features_rest, opacity, scaling, rotation = convert_np_tensor(
        xyz_rgb, features_dc, features_rest, opacity, scaling, rotation)
    save_ply(
        xyz,
        features_dc,
        features_rest,
