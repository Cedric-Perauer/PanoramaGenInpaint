...
def training(
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        if mask_training:
            kernel_size = 10
            image_mask = cv2.dilate(viewpoint_cam.original_image_mask, np.ones(
                (kernel_size, kernel_size), dtype=np.uint8), iterations=1)

            image_m = image * \
                torch.tensor(1 - image_mask).cuda().repeat(3, 1, 1)
            gt_image_m = gt_image * \
                torch.tensor(1 - image_mask).cuda().repeat(3, 1, 1)
            ...
            Ll1 = l1_loss(
                image_m,
                gt_image_m,
                mask=viewpoint_cam.original_image_mask)
            loss = (1.0 - opt.lambda_dssim) * Ll1 + \
                opt.lambda_dssim * (1.0 - ssim(image_m, gt_image_m))
            loss.backward()
        else:
            Ll1 = l1_loss(
                image,
