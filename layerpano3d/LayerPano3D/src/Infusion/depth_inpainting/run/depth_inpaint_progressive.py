...
base_img = Image.open(
    os.path.join(
        base_path,
        "layer{}_inpaint.png".format(n_layers)))
...
for l in range(n_layers, 0, -1):
    input_rgb_path = os.path.join(
        base_path, "layer{}_inpaint.png".format(l - 1))
    input_mask_path = os.path.join(
        base_path, "layer{}_mask_smooth.png".format(l))
    input_depth_path = os.path.join(
        base_path, "layer{}_inpaint_depth.npy".format(l))
    intri = os.path.join(base_path, "intrinsics.npy")
    output_dir = base_path
    image_path = os.path.join(base_path, "layer{}_inpaint.png".format(l))
    os.system(
        f"python run_inference_inpainting.py --input_rgb_path {input_rgb_path} --input_mask {input_mask_path} --input_depth_path {input_depth_path} --output_dir {output_dir} --intri {intri} --use_mask  --blend"
    )
