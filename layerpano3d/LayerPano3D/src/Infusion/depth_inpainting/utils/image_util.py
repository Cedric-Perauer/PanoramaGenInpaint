...
def create_point_cloud(depth_map, camera_matrix, extrinsic_matrix):
    ...
    homogeneous_camera_coords = depth_map_reshaped * \
    ...
    homogeneous_world_coords = np.dot(
        homogeneous_camera_coords, extrinsic_matrix.T)

    # Divide by the fourth coordinate (homogeneous normalization)
    point_cloud = homogeneous_world_coords[:, :,
                                           :3] / homogeneous_world_coords[:, :, 3:]
...
def Disparity_Normalization(disparity):
    ...
    normalized_disparity = ((disparity - min_value) /
                            (max_value - min_value + 1e-5) - 0.5) * 2
