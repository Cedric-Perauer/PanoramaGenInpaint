...
def kitti_colormap(disparity, maxval=-1):
    ...
    cumsum = np.asarray([0, 0.114, 0.299, 0.413, 0.587,
                        0.701, 0.8859999999999999, 0.9999999999999999])

    colored_disp = np.zeros([disparity.shape[0], disparity.shape[1], 3])
    values = np.expand_dims(
        np.minimum(
            np.maximum(
                disparity / maxval, 0.0), 1.0), -1)
    bins = np.repeat(
        np.repeat(
            np.expand_dims(
                np.expand_dims(
    ...
    diffs = np.where((np.repeat(values, 8, axis=-1) - bins) >
                     0, -1000, (np.repeat(values, 8, axis=-1) - bins))
    index = np.argmax(diffs, axis=-1) - 1

    w = 1 - (values[:, :, 0] - cumsum[index]) * np.asarray(weights)[index]

    colored_disp[:, :, 2] = w * colormap[index][:, :, 0] + \
        (1.0 - w) * colormap[index + 1][:, :, 0]
    colored_disp[:, :, 1] = w * colormap[index][:, :, 1] + \
        (1.0 - w) * colormap[index + 1][:, :, 1]
    colored_disp[:, :, 0] = w * colormap[index][:, :, 2] + \
        (1.0 - w) * colormap[index + 1][:, :, 2]

    return (colored_disp * np.expand_dims((disparity > 0), -1)
            * 255).astype(np.uint8)
