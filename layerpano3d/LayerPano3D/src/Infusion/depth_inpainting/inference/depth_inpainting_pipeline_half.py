...
import sys
from src.Infusion.depth_inpainting.utils.image_util import (
    resize_max_res,
    chw2hwc,
    colorize_depth_maps,
    create_point_cloud,
...
class DepthEstimationInpaintPipeline(DiffusionPipeline):
    ...
    def __call__(
        ...
        depth_preds = torch.concat(
            depth_pred_ls, axis=0).squeeze()  # (10,224,768)
        ...
        # ----------------- Post processing -----------------
        depth_save = (depth_pred * (max_value - min_value) +
                      min_value) / downscale_factor
        ...
        depth_colored = colorize_depth_maps(
            depth_pred, 0, 1, cmap=color_map).squeeze()  # [3, H, W], value in (0, 1)
    ...
    def __encode_empty_text(self):
        ...
        self.empty_text_embed = self.text_encoder(
            text_input_ids)[0].to(torch.float16)  # [1,2,1024]
    ...
    def single_infer(
        ...
        # encode image
        # 1/8 Resolution with a channel nums of 4.
        ...
        left_disparity_resized, downscale_factor = resize_max_res_tensor(
            disparity.unsqueeze(0).unsqueeze(0), is_disp=True
        )
        min_value = torch.min(left_disparity_resized.squeeze()[
                              zero_indices[:, 0], zero_indices[:, 1]])
        max_value = torch.max(left_disparity_resized.squeeze()[
                              zero_indices[:, 0], zero_indices[:, 1]])
        left_disparity_resized_normalized = Disparity_Normalization_mask(
            left_disparity_resized, min_value, max_value)
        left_disparity_resized_normalized.squeeze()[mask == 1] = 0
        mask_disparity = left_disparity_resized_normalized.repeat(
            1, 3, 1, 1).half()
        mask_disparity = self.encode_depth(mask_disparity)
        mask = torch.nn.functional.interpolate(
            mask.unsqueeze(0).unsqueeze(0), size=(
                mask.shape[0] // 8, mask.shape[1] // 8)).half()
        mask_blend = mask

        # Initial depth map (Guassian noise)
        timesteps_add, _ = self.get_timesteps(
            num_inference_steps, 1.0, device, denoising_start=None)
        left_disparity_resized_normalized_no_mask = Disparity_Normalization_mask(
            left_disparity_resized, min_value, max_value)
        depth_latents_no_mask = left_disparity_resized_normalized_no_mask.repeat(
            1, 3, 1, 1)

        disp_latents = self.encode_depth(depth_latents_no_mask.half())
        noise = torch.randn_like(disp_latents)
        depth_latent = self.scheduler.add_noise(
            disp_latents, noise, timesteps_add[:1])
        ...
        for i, t in iterable:
            unet_input = torch.cat(
                [rgb_latent, depth_latent, mask, mask_disparity], dim=1)
            ...
            depth_latent = self.scheduler.step(
                noise_pred, t, depth_latent).prev_sample.to(
                torch.float16)
            if blend:
                ...
                if i < len(timesteps) - 1:
                    ...
                    depth_latent = (1 - init_mask) * \
                        disp_latent_step + init_mask * depth_latent
                    depth_latent = depth_latent.half()
