...
def render(
    ...
    if override_color is None:
        if opt.convert_SHs_python:
            ...
            shs_view = pc.get_features_render.transpose(1, 2).view(
                n_points, 3, -1)[:, :, : (pc.max_sh_degree + 1) ** 2]
            dir_pp = pc.get_xyz_render - \
                viewpoint_camera.camera_center.repeat(pc.get_features_render.shape[0], 1)
...
def render4d(
    ...
    time = torch.tensor(
        viewpoint_camera.time).to(
        means3D.device).repeat(
            means3D.shape[0],
        1)
