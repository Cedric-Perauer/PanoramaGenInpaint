...
class Gen_traindata:
    ...
    def count_layers(self, folder_path):
        subfolders = [f.path for f in os.scandir(
            folder_path) if f.is_dir() and f.name.startswith("layer")]
    ...
    def load_depth_inpainting_model(self):
        ...
        scheduler = DDIMScheduler.from_pretrained(
            self.model_path, subfolder="scheduler", torch_dtype=dtype)
        text_encoder = CLIPTextModel.from_pretrained(
            self.model_path, subfolder="text_encoder", torch_dtype=dtype)
        tokenizer = CLIPTokenizer.from_pretrained(
            self.model_path, subfolder="tokenizer", torch_dtype=dtype)
        ...
        pipe = DepthEstimationInpaintPipeline(
            unet=unet,
            vae=vae,
            scheduler=scheduler,
    ...
    def generate_panodepth(self, pano_rgb):
        ...
        panodepth_estimator = Pano_depth_estimation(
            pano_h, pano_w, self.save_dir, self.device, depth_model=self.depth_model)
        pano_depth = panodepth_estimator.get_panodepth(pano_rgb)  # [0-1]

        return pano_depth

    def generate_traindata(
        self, layerpano_rgbs, layerpano_bagel_masks, pano_align_masks, pano_align_masks_sharp  # N  # N  # N-1
    ):
        ...
        pano_rgb_base = cv2.resize(
            layerpano_rgbs[-1], (self.layerexp_W, self.layerexp_H))  # [h,w,3] [0-255] uint8
        # pano_depth : [1024,2048] [0-1] float64
        pano_depth_base = self.generate_panodepth(pano_rgb_base)
        layerpano_depths.append(pano_depth_base)
        depthmap_utils.depth_visual_save(
            pano_depth_base,
            f"{self.traindata_dir}/pano_depth_layer{N-1}.png")

        cmap = matplotlib.colormaps.get_cmap("Spectral_r")
        depth_color = (
            cmap(pano_depth_base)[
                :,
                :,
                :3] *
            255)[
            ...
            1].astype(
                np.uint8)
        cv2.imwrite(
            f"{self.traindata_dir}/pano_depth_layer_rgb{N-1}.png",
            depth_color)

        pano_depths_aligned = [
            (layerpano_depths[-1] * max_val).astype(np.int32)]  # [0-65535]
        ...
        for i in range(N - 1):
            ...
            pano_rgb = cv2.resize(
                layerpano_rgbs[-(id + 1)], (self.layerexp_W, self.layerexp_H))
            mask = pano_align_masks[-id].astype(float)
            mask[mask > 0.5] = 1
            mask[mask < 0.5] = 0
            pano_depth_base_i = layerpano_depths[-id]

            pipe_out = self.pipe_dp(
                input_image=pano_rgb,
            ...
            depthmap_utils.depth_visual_save(
                depth_pred, f"{self.traindata_dir}/pano_depth_layer{N-id-1}.png")
            ...
            depth_color = (
                ...
                np.uint8)
            cv2.imwrite(
                f"{self.traindata_dir}/pano_depth_layer_rgb{N-id-1}.png",
                depth_color)
        ...
        layerpano_rgbs = self.rgb_alignment(
            layerpano_rgbs_unaligned, pano_align_masks_sharp)
        ...
        for i in range(N):
            ...
            pano_depths_aligned[i] = cv2.resize(
                pano_depths_aligned[i].astype(
                    np.float64), (self.layer_rgb_raw_W, self.layer_rgb_raw_H)).astype(
                np.int32)

            pano_rgb = layerpano_rgbs[i]  # [h,w,3] [0-255] uint8

            pano_bagel_mask = cv2.resize(
                layerpano_bagel_masks[i],
                (self.layer_rgb_raw_W,
                 self.layer_rgb_raw_H))  # [h,w,3] [0-255] uint8
            pano_depth = pano_depths_aligned[i] / max_val

            depthmap_utils.depth_visual_save(
                pano_depth, f"{dir_now}/pano_RGBdepth_aligned_{i}.png")
            ...
            depth_color_pil.save(
                f"{dir_now}/pano_RGBdepth_aligned_light_{i}.png")
            ...
            pointcloud_utils.depthmap2pointcloud_erp(
                pano_depth * scale, pano_rgb, f"{dir_now}/pcd_rgb_layer{i}.ply")
            pointcloud_utils.depthmap2pointcloud_erp(
                pano_depth * scale, pano_bagel_mask, f"{dir_now}/pcd_mask_layer{i}.ply")
    ...
    def gen_frames_data(self, pano, pano_depth, save_dir, n=8):
        ...
        for i in range(len(theta_list)):
            ...
            pers_img = equ.GetPerspective(
                90, th, ph, pers_img_size, pers_img_size)
    ...
    def run(self):
        ...
        # "/mnt/petrelfs/yangshuai/4drender/PanFusion/outputs_inpaint_city1"     #"/mnt/petrelfs/yangshuai/4drender/PanFusion/outputs_inpaint_nature1"
        layerpano_dir = (self.layerpano_dir)
        # now defined a pano divided into [layer0, layer1, layer2(input pano/
        # outermost layer)]
        ...
        for i in range(self.num_layer):
            ...
            layer_rgb_path = (
                f"{layerpano_dir}/layer{i}/layer{i}_inpaint_sr.png"
                if self.sr
                else f"{layerpano_dir}/layer{i}/layer{i}_inpaint.png"
            )
            layer_mask_path = (
                f"{layerpano_dir}/layer{i}/layer{i}_mask.png"
                if i == self.num_layer - 1
                else f"{layerpano_dir}/layer{i}/layer{i}_mask_new.png"
            )
            layer_smoothmask_path = (
                f"{layerpano_dir}/layer{i}/layer{i}_mask_smooth.png"
                if i == self.num_layer - 1
                else f"{layerpano_dir}/layer{i}/layer{i}_mask_smooth_new.png"
            )
            ...
            layer_next_bagel_mask = make_bagel_mask(
                layer_mask_path, layer_smoothmask_path)
        ...
        for i in range(N):
            ...
            bagel_mask = np.repeat(
                bagel_mask, 3, axis=2)  # [h,w,3] [0-255] uint8
            layerpano_bagel_masks[i] = bagel_mask

        traindata = self.generate_traindata(
            layerpano_rgbs,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--depth_model",
        type=str,
        default="DepthAnythingv2")  # ['midas3', 'DepthAnythingv2']
    parser.add_argument("--save_dir", type=str, default=None)
    parser.add_argument("--sr", action="store_true")
    # outputs_traindata_kernelbig_depth
    parser.add_argument("--root", type=str, default="outputs")
    parser.add_argument("--layerpano_dir", type=str, default="")
    args = parser.parse_args()

    gen_traindata = Gen_traindata(
        depth_model=args.depth_model,
        layerpano_dir=args.layerpano_dir,
        # "/mnt/petrelfs/yangshuai/4drender/PanFusion/outputs_inpaint_city1"     #"/mnt/petrelfs/yangshuai/4drender/PanFusion/outputs_inpaint_nature1"
