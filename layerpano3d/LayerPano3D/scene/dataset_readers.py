...
def fetchPly(path):
    ...
    idx = np.random.choice(len(vertices["x"]), size=(
        min(len(vertices["x"]), 100_000),), replace=False)
    positions = np.vstack([vertices["x"][idx],
                           vertices["y"][idx],
                           vertices["z"][idx]]).T if "x" in vertices else None
    colors = (
        np.vstack([vertices["red"][idx], vertices["green"][idx], vertices["blue"][idx]]).T / 255.0
        if "red" in vertices
        else None
    )
    normals = np.vstack([vertices["nx"][idx],
                         vertices["ny"][idx],
                         vertices["nz"][idx]]).T if "nx" in vertices else None
...
def readColmapSceneInfo(path, images, eval, preset=None, llffhold=8):
    ...
    if eval:
        ...
        train_cam_infos = [
            c for idx,
            c in enumerate(cam_infos) if idx %
            5 == 2 or idx %
            5 == 0]
        test_cam_infos = [
            c for idx,
            c in enumerate(cam_infos) if not (
                idx %
                5 == 2 or idx %
                5 == 0)]
    else:
...
def readCamerasFromPreset(path, transformsfile):
    ...
    with open(os.path.join(path, transformsfile)) as json_file:
        ...
        for idx, frame in enumerate(frames):
            ...
            # get the world-to-camera transform and set R, T
            w2c = np.linalg.inv(np.concatenate(
                (c2w, np.array([0, 0, 0, 1]).reshape(1, 4)), axis=0))
            # R is stored transposed due to 'glm' in CUDA code
            R = np.transpose(w2c[:3, :3])
            T = w2c[:3, 3]
            # R = c2w[:3,:3]
...
def readNerfSyntheticInfo(
    ...
    if not os.path.exists(ply_path):
        ...
        # We create random points inside the bounds of the synthetic Blender
        # scenes
...
def loadCamerasFromData(traindata, white_background):
    ...
    for idx, frame in enumerate(frames):
        ...
        # change from OpenGL/Blender camera axes (Y up, Z back) to COLMAP (Y
        # down, Z forward)
        c2w[:3, 1:3] *= -1
...
def loadCameraPreset(traindata, presetdata):
    ...
    for camkey in presetdata:
        ...
        for idx, frame in enumerate(presetdata[camkey]["frames"]):
            ...
            world_view_transform = (torch.tensor(getWorld2View2(
                R, T, np.array([0.0, 0.0, 0.0]), 1.0)).transpose(0, 1).cuda())
