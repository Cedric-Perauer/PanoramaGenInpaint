...
class LayerGaussian:
    ...
    def capture_total(self):
        ...
        f_rest = torch.cat(
            (self._features_rest_static, self._features_rest), dim=0)
        ...
        xyz_gradient_accum_static = torch.zeros(
            (self._xyz_static.shape[0], 1), device="cuda")
        xyz_gradient_accum = torch.cat(
            [xyz_gradient_accum_static, self.xyz_gradient_accum], dim=0)
        max_radii2D_static = torch.zeros(
            (self._xyz_static.shape[0]), device="cuda")
        max_radii2D = torch.cat([max_radii2D_static, self.max_radii2D], dim=0)
        denom_static = torch.zeros(
            (self._xyz_static.shape[0], 1), device="cuda")
    ...
    def get_scaling_total(self):
        return self.scaling_activation(
            torch.cat([self._scaling_static, self._scaling], dim=0))

    @property
    def get_scaling_render(self):
        return self.scaling_activation(
            torch.cat([self._scaling_surface, self._scaling], dim=0))
    ...
    def get_rotation_total(self):
        return self.rotation_activation(
            torch.cat([self._rotation_static, self._rotation], dim=0))

    @property
    def get_rotation_render(self):
        return self.rotation_activation(
            torch.cat([self._rotation_surface, self._rotation], dim=0))
    ...
    def get_features_total(self):
        ...
        features_static = torch.cat(
            (features_dc_static, features_rest_static), dim=1)
    ...
    def get_features_render(self):
        ...
        features_surface = torch.cat(
            (features_dc_surface, features_rest_surface), dim=1)
    ...
    def get_opacity_total(self):
        return self.opacity_activation(
            torch.cat([self._opacity_static, self._opacity], dim=0))

    @property
    def get_opacity_render(self):
        return self.opacity_activation(
            torch.cat([self._opacity_surface, self._opacity], dim=0))
    ...
    def get_covariance_total(self, scaling_modifier=1):
        rotation_total = torch.cat(
            [self._rotation_static, self._rotation], dim=0)
        return self.covariance_activation(
            self.get_scaling_total, scaling_modifier, rotation_total)

    def get_covariance_render(self, scaling_modifier=1):
        rotation_total = torch.cat(
            [self._rotation_surface, self._rotation], dim=0)
        return self.covariance_activation(
            self.get_scaling_render,
            scaling_modifier,
            rotation_total)
    ...
    def create_from_pcd_and_masks(
        ...
        outlier_mask = self.get_outlier_filter(
            fused_point_cloud.detach().cpu().numpy(),
            bagel_temp.detach().cpu().numpy(),
            thresh=self.outlier_thresh)
        ...
        fused_color = RGB2SH(torch.tensor(np.asarray(pcd.colors), device="cuda")[
                             identity_mask][outlier_mask].float())
        features = torch.zeros(
            (fused_color.shape[0], 3, (self.max_sh_degree + 1) ** 2)).float().cuda()
        features[:, :3, 0] = fused_color
        features[:, 3:, 1:] = 0.0
        ...
        print(
            "Number of new pcd points at initialisation : ",
            fused_point_cloud.shape[0])
        ...
        _features_dc = features[:, :, 0:1].transpose(
            1, 2).contiguous().requires_grad_(True)
        _features_rest = features[:, :, 1:].transpose(
            1, 2).contiguous().requires_grad_(True)
        ...
        xyz_part = np.asarray(pcd.points)[identity_mask.detach(
        ).cpu().numpy()][outlier_mask.detach().cpu().numpy()]
        colors_part = np.asarray(pcd.colors)[identity_mask.detach(
        ).cpu().numpy()][outlier_mask.detach().cpu().numpy()]
        ...
        return (
            identity[identity_mask][outlier_mask],
            _xyz,
            _features_dc,
            _features_rest,
            _scaling,
            _rotation,
            _opacity,
            identity_mask.detach().cpu().numpy(),
            outlier_mask.detach().cpu().numpy(),
        )
    ...
    def load_ply_and_create_pcd(self, plydata, pcd, masks, spatial_lr_scale):
        ...
        if plydata is not None:
            xyz, opacities, features_dc, features_extra, scales, rots = (
                plydata["xyz"],
                plydata["opacities"],
                plydata["f_dc"],
                plydata["f_rest"],
        ...
        if plydata is not None:
            ...
            xyz2, active_mask, surface_mask = self.get_active_mask(
                plydata, pcd, _identity_mask, _outlier_mask)
            ...
            print(
                ...
                identity.shape,
                active_mask.shape)
            _xyz = torch.cat([_xyz, torch.tensor(
                xyz2[active_mask], dtype=torch.float, device="cuda")], dim=0)
            _features_dc = torch.cat([_features_dc, torch.tensor(
                features_dc[active_mask], dtype=torch.float, device="cuda").contiguous()], dim=0, )
            _features_rest = torch.cat([_features_rest, torch.tensor(
                features_extra[active_mask], dtype=torch.float, device="cuda").contiguous(), ], dim=0, )
            _scaling = torch.cat([_scaling, torch.tensor(
                scales[active_mask], dtype=torch.float, device="cuda")], dim=0)
            _rotation = torch.cat([_rotation, torch.tensor(
                rots[active_mask], dtype=torch.float, device="cuda")], dim=0)
            # opacities[active_mask] = -3

            _opacity = torch.cat([_opacity, torch.tensor(
                opacities[active_mask], dtype=torch.float, device="cuda")], dim=0)
            identity_active = torch.ones(
                (xyz[active_mask].shape[0]),
                dtype=torch.float,
                device="cuda")
            identity_active[:] = 2.0
        ...
        # Static Gaussians = Surface Gaussians + Occluded Gaussians (by new pcd
        # points)
        if plydata is not None:
            # print("Number of static points: ", xyz2[static_mask].shape[0])
            self._xyz_static = nn.Parameter(
                torch.tensor(
                    xyz2[static_mask],
                    dtype=torch.float,
                    device="cuda").requires_grad_(True))
            self._features_dc_static = nn.Parameter(
                torch.tensor(
                    features_dc[static_mask],
                    dtype=torch.float,
                    device="cuda") .contiguous() .requires_grad_(True))
            self._features_rest_static = nn.Parameter(
                torch.tensor(
                    features_extra[static_mask],
                    dtype=torch.float,
                    device="cuda") .contiguous() .requires_grad_(True))
            self._opacity_static = nn.Parameter(
                torch.tensor(
                    opacities[static_mask],
                    dtype=torch.float,
                    device="cuda").requires_grad_(True))
            self._scaling_static = nn.Parameter(
                torch.tensor(
                    scales[static_mask],
                    dtype=torch.float,
                    device="cuda").requires_grad_(True))
            self._rotation_static = nn.Parameter(
                torch.tensor(
                    rots[static_mask],
                    dtype=torch.float,
                    device="cuda").requires_grad_(True))

            self._xyz_surface = nn.Parameter(
                torch.tensor(
                    xyz2[surface_mask],
                    dtype=torch.float,
                    device="cuda").requires_grad_(True))
            self._features_dc_surface = nn.Parameter(
                torch.tensor(
                    features_dc[surface_mask],
                    dtype=torch.float,
                    device="cuda") .contiguous() .requires_grad_(True))
            self._features_rest_surface = nn.Parameter(
                torch.tensor(
                    features_extra[surface_mask],
                    dtype=torch.float,
                    device="cuda") .contiguous() .requires_grad_(True))
            self._opacity_surface = nn.Parameter(
                torch.tensor(
                    opacities[surface_mask],
                    dtype=torch.float,
                    device="cuda").requires_grad_(True))
            self._scaling_surface = nn.Parameter(
                torch.tensor(
                    scales[surface_mask],
                    dtype=torch.float,
                    device="cuda").requires_grad_(True))
            self._rotation_surface = nn.Parameter(
                torch.tensor(
                    rots[surface_mask],
                    dtype=torch.float,
                    device="cuda").requires_grad_(True))
            self.identity_surface = torch.zeros(
                (xyz2[surface_mask].shape[0]), device="cuda")
            self.identity = torch.cat([self.identity_surface, identity], dim=0)
        else:

            self._xyz_surface = self._xyz_surface.to(self._xyz.device)
            self._features_dc_surface = self._features_dc_surface.to(
                self._features_dc.device)
            self._features_rest_surface = self._features_rest_surface.to(
                self._features_rest.device)
            self._opacity_surface = self._opacity_surface.to(
                self._opacity.device)
            self._scaling_surface = self._scaling_surface.to(
                self._scaling.device)
            self._rotation_surface = self._rotation_surface.to(
                self._rotation.device)

            self._xyz_static = self._xyz_static.to(self._xyz.device)
            self._features_dc_static = self._features_dc_static.to(
                self._features_dc.device)
            self._features_rest_static = self._features_rest_static.to(
                self._features_rest.device)
            self._opacity_static = self._opacity_static.to(
                self._opacity.device)
            self._scaling_static = self._scaling_static.to(
                self._scaling.device)
            self._rotation_static = self._rotation_static.to(
                self._rotation.device)
        ...
        print(
            "Number of static and surface points at initialization:", len(
                self._xyz_static), len(
                self._xyz_surface))
    ...
    def load_gaussians(self, plydata, spatial_lr_scale):
        ...
        _features_dc = torch.tensor(
            features_dc[active_mask],
            dtype=torch.float,
            device="cuda").contiguous()
        _features_rest = torch.tensor(
            features_extra[active_mask],
            dtype=torch.float,
            device="cuda").contiguous()
        ...
        print(
            "Number of static and surface points at finetune:", len(
                self._xyz_static), len(
                self._xyz))
        self.active_sh_degree = self.max_sh_degree
    ...
    def save_ply(self, path):
        ...
        normals = np.zeros_like(xyz)
        f_dc = torch.cat((self._features_dc_static, self._features_dc), dim=0)
        f_dc = f_dc.detach().transpose(1, 2).flatten(
            start_dim=1).contiguous().cpu().numpy()
        f_rest = torch.cat(
            (self._features_rest_static, self._features_rest), dim=0)
        f_rest = f_rest.detach().transpose(1, 2).flatten(
            start_dim=1).contiguous().cpu().numpy()
        f_rest[:] = 0
        opacities = torch.cat([self._opacity_static, self._opacity], dim=0)
    ...
    def wrap_gaussian(self):
        # unlike save_ply, we don't transpose the sh degrees to the last dim
        # for feature dc and rest
        xyz = self.get_xyz_total.detach().cpu().numpy()
        normals = np.zeros_like(xyz)
        f_dc = torch.cat((self._features_dc_static, self._features_dc), dim=0)
        print(
            "DEBUG wrap gaussian shape,static,active",
            self._features_dc_static.shape,
            self._features_dc.shape)
    ...
    def prune_points(self, mask):
        ...
        self.identity = torch.cat(
            [self.identity_surface, self.identity[self.identity_mask][valid_points_mask]], dim=0)
    ...
    def densify_and_split(self, grads, grad_threshold, scene_extent, N=2):
        ...
        selected_pts_mask = torch.logical_and(selected_pts_mask, torch.max(
            self.get_scaling, dim=1).values > self.percent_dense * scene_extent)

        stds = self.get_scaling[selected_pts_mask].repeat(N, 1)
        means = torch.zeros((stds.size(0), 3), device="cuda")
        samples = torch.normal(mean=means, std=stds)
        rots = build_rotation(
            self._rotation[selected_pts_mask]).repeat(
            N, 1, 1)
        ...
        self.identity = torch.cat(
            (self.identity, self.identity[self.identity_mask][selected_pts_mask].repeat(N)), dim=0
        )
        self.update_identity_mask()
    ...
    def densify_and_clone(self, grads, grad_threshold, scene_extent):
        ...
        self.identity = torch.cat(
            (self.identity, self.identity[self.identity_mask][selected_pts_mask]), dim=0)
        self.update_identity_mask()
    ...
    def add_densification_stats(
        ...
        self.xyz_gradient_accum[update_filter] += torch.norm(
            viewspace_point_tensor.grad[identity_mask][update_filter, :2], dim=-1, keepdim=True
        )
    ...
    def get_active_mask(self, plydata, pcd, identity_mask0, outlier_mask):
        ...
        def scale_func(x): return np.log(x + 1) * 200
        # build_dictionary
        rays_d = []
        # z_table.fill(zmax)
        ...
        dist_voxel = np.zeros(
            (
                (np.ceil(x_range).astype("int")) + 1,
                np.ceil(y_range).astype("int") + 1,
                np.ceil(z_range).astype("int") + 1,
            )
        )
        dist_occupancy = np.zeros(
            (
                (np.ceil(x_range).astype("int")) + 1,
                np.ceil(y_range).astype("int") + 1,
                np.ceil(z_range).astype("int") + 1,
        ...
        for idx in tqdm(range(len(xyz))):
            dist = np.sqrt(xyz[idx, 0] ** 2 + xyz[idx, 1]
                           ** 2 + xyz[idx, 2] ** 2)
            ...
            if (
                x >= 0
                and x < np.ceil(x_range).astype("int") + 1
                and y >= 0
                and y < np.ceil(y_range).astype("int") + 1
                and z >= 0
                and z < np.ceil(z_range).astype("int") + 1
            ):
                ...
                # type 2 - out-of-domain / surface gaussians
                pcd_mask[idx] = False
