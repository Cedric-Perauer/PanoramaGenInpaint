...
class FluxControlNetInpaintingPipeline(DiffusionPipeline, FluxLoraLoaderMixin):
    ...
    def encode_prompt(
        ...
        text_ids = torch.zeros(
            batch_size, prompt_embeds.shape[1], 3).to(
    ...
    # Copied from
    # diffusers.pipelines.flux.pipeline_flux._prepare_latent_image_ids
    @staticmethod
    ...
    def prepare_image_with_mask(
        ...
        image_latents = self.vae.encode(
            masked_image.to(
                self.vae.dtype)).latent_dist.sample()
        image_latents = (
    ...
    def __call__(
        ...
        (prompt_embeds,
         ...
         text_ids) = (self.encode_prompt(prompt=prompt,
                                         prompt_2=prompt_2,
                                         prompt_embeds=prompt_embeds,
                                         pooled_prompt_embeds=pooled_prompt_embeds,
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                noise_pred = self.transformer(
                    ...
                    # by 1000 in the transforme rmodel (we should not keep it
                    ...
                    encoder_hidden_states=prompt_embeds,
                    controlnet_block_samples=[
                        sample.to(dtype=self.transformer.dtype) for sample in controlnet_block_samples
                    ],
                    controlnet_single_block_samples=(
