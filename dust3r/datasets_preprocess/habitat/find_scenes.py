...
def find_all_scenes(habitat_root, n_scenes=[100000]):
    ...
    def write_scene_list(scenes, n, fpath):
        sub_scenes = [os.path.join(scene, id)
                      for scene, ids in scenes for id in ids]
    ...
    for n in n_scenes:
        write_scene_list(train_scenes, n, os.path.join(
            habitat_root, f"Habitat_{n}_scenes_train.txt"))
        write_scene_list(
            val_scenes,
            n // 10,
            os.path.join(
                habitat_root,
                f"Habitat_{n//10}_scenes_val.txt"))
