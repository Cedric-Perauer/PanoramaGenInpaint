...
def preprocess_metadata(
    ...
    # Use 4 times the crop size as resolution for rendering the environment
    # map.
    max_res = max(crop_resolution)

    if equirectangular_resolution is None:
        # Use 4 times the crop size as resolution for rendering the environment
        # map.
        max_res = max(crop_resolution)
    ...
    for batch_label, batch in tqdm(metadata["view_batches"].items()):
        for view_label, view_params in batch.items():
            ...
            if fix_existing_dataset and os.path.isfile(
                    output_camera_params_filename):
                # Skip generation if we are fixing a dataset and the
            ...
            original_projection, position = multiview_crop_generator.dict_to_perspective_projection(
                view_params)
        ...
        for filename in glob.iglob(
            ...
            output_dir = os.path.join(
                args.output_dir, os.path.relpath(
                    os.path.dirname(filename), args.metadata_dir))
