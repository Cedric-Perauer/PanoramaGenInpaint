...
ViewpointData = collections.namedtuple(
    "ViewpointData", [
        "colormap", "distancemap", "pointmap", "position"])
...
class HabitatMultiviewCrops:
    def __init__(
        ...
        self.R_cam_to_world = np.asarray(
            self.lowres_envmap_renderer.R_cam_to_world())
        self.up_direction = np.asarray(
            self.lowres_envmap_renderer.up_direction())
        ...
        self.envmap_rays = projections.get_projection_rays(
            self.envmap_projection)
    ...
    def render_viewpoint_data(self, position):
        data = self.lowres_envmap_renderer.render_viewpoint(
            np.asarray(position))
        # Ignore the alpha channel
        colormap = data["observations"]["color_equirectangular"][..., :3]
    ...
    def extract_cropped_camera(
        ...
        cropped_pointmap = remapper.convert(
            ...
            single_map=True)
        cropped_voxelmap = (
            None if voxelmap is None else remapper.convert(
                voxelmap,
                interpolation=cv2.INTER_NEAREST,
                borderMode=cv2.BORDER_WRAP,
                single_map=True))
...
def perspective_projection_to_dict(persp_projection, position):
    ...
    camera_params = dict(
        camera_intrinsics=projections.colmap_to_opencv_intrinsics(
            persp_projection.base_projection.K).tolist(),
        size=(
...
def dict_to_perspective_projection(camera_params):
    ...
    projection = projections.PerspectiveProjection(
        K, height=size[1], width=size[0])
