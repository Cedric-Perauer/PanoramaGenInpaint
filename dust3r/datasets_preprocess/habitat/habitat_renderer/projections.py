...
class EquirectangularProjection:
    ...
    def unproject(self, u, v):
        ...
        x, z = np.sin(longitude) * \
            cos_latitude, np.cos(longitude) * cos_latitude
...
def get_projection_rays(projection, noise_level=0):
    ...
    """
    grid_u, grid_v = np.meshgrid(0.5 +
                                 np.arange(projection.width), 0.5 +
                                 np.arange(projection.height))
    if noise_level > 0:
        grid_u += np.clip(0, noise_level * np.random.uniform(-0.5,
                          0.5, size=grid_u.shape), projection.width)
        grid_v += np.clip(0, noise_level * np.random.uniform(-0.5,
                          0.5, size=grid_v.shape), projection.height)
