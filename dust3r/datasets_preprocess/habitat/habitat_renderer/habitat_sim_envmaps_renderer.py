...
class HabitatEnvironmentMapRenderer:
    def __init__(
        ...
        # so the values do not correspond to distance to the center and need
        # some scaling.
        self.equirectangular_depth_scale_factors = 1.0 / \
            np.max(np.abs(equirectangular_rays), axis=-1)
    ...
    def _lazy_initialization(self):
        ...
        if self.seed is None:
            ...
            if self.render_cubemap:
                for face_id, orientation in enumerate(
                        CUBEMAP_FACE_ORIENTATIONS_ROTVEC):
    ...
    def render_viewpoint(self, viewpoint_position):
        ...
        data = dict(
            observations=viewpoint_observations,
