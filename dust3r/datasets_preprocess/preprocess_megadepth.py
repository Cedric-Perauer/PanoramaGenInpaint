...
def _load_kpts_and_poses(
    ...
    if intrinsics:
        ...
        for camera in raw:
            camera = camera.split(" ")
            width, height, focal, cx, cy, k0 = [
                float(elem) for elem in camera[2:]]
            K = np.eye(3)
            K[0, 0] = focal
            K[1, 1] = focal
            K[0, 2] = cx
            ...
            camera_intrinsics[int(camera[0])] = (
                (int(width), int(height)), K, (k0, 0, 0, 0))
    ...
    if intrinsics:
        image_intrinsics = {
            im_id: camera_intrinsics[cam] for im_id,
            cam in zip(
                poses,
                camera)}
...
def colmap_raw_pose_to_principal_axis(image_pose):
    ...
    w, x, y, z = qvec
    z_axis = np.float32([2 * x * z - 2 * y * w, 2 * y *
                        z + 2 * x * w, 1 - 2 * x * x - 2 * y * y])
    return z_axis
