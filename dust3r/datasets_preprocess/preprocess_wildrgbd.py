...
def get_parser():
    ...
    parser.add_argument(
        "--output_dir",
        type=str,
        default="data/wildrgbd_processed")
    parser.add_argument("--wildrgbd_dir", type=str, required=True)
    parser.add_argument(
        "--train_num_sequences_per_object",
        type=int,
        default=50)
    parser.add_argument(
        "--test_num_sequences_per_object",
        type=int,
        default=10)
...
def get_set_list(category_dir, split):
    ...
    sequences_all = {s: {k: set() for k in listfiles}
                     for s in ["train", "val"]}
    ...
    train_intersection = set.intersection(
        *list(sequences_all["train"].values()))
    if split == "train":
        return train_intersection
    else:
        all_seqs = set.union(*
                             list(sequences_all["train"].values()), *
                             list(sequences_all["val"].values()))
...
def prepare_sequences(
    ...
    if len(sequences_all) <= max_num_sequences_per_object:
        ...
        selected_sequences = random.sample(
            sequences_all, max_num_sequences_per_object)
    ...
    for seq_name in tqdm(selected_sequences, leave=False):
        ...
        selected_frames = np.round(
            np.linspace(
                0,
                num_frames - 1,
                output_num_frames)).astype(int).tolist()
        selected_sequences_numbers_dict[seq_name] = selected_frames

        for frame_id in tqdm(selected_frames):
            depth_path = os.path.join(
                scene_dir, "depth", f"{frame_id:0>5d}.png")
            masks_path = os.path.join(
                scene_dir, "masks", f"{frame_id:0>5d}.png")
            ...
            # the new window will be a rectangle of size (2*min_margin_x,
            # 2*min_margin_y) centered on (cx,cy)
            ...
            # try to set the lower dimension to img_size * 3/4 -> img_size=512
            # => 384
            scale_final = ((img_size * 3 // 4) / min(H, W)) + 1e-8
            output_resolution = np.floor(
                np.array([W, H]) * scale_final).astype(int)
