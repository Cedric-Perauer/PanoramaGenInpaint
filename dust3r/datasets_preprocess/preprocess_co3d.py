...
def prepare_sequences(
    ...
    sequences_all = get_set_list(
        category_dir, split, is_single_sequence_subset)
    sequences_numbers = sorted(
        set(seq_name for seq_name, _, _ in sequences_all))
    ...
    for seq_name, frame_number, filepath in tqdm(sequences_all):
        ...
        with PIL.Image.open(depth_path) as depth_pil:
            ...
            # we cast it to uint16, then reinterpret as float16, then cast to
            # float32
            input_depthmap = (
                np.frombuffer(
                    np.array(
