...
def get_up_vectors(pose_device_to_world):
    return np.matmul(pose_device_to_world, np.array(
        [[0.0], [-1.0], [0.0], [0.0]]))
...
def read_traj(traj_path):
    ...
    with open(traj_path) as f:
        ...
        for line in traj_lines:
            ...
            r_p_to_w_as_quat = quaternion.from_rotation_matrix(
                pose_p_to_w[:3, :3])
...
def main(rootdir, pairsdir, outdir):
    ...
    for subdir in subdirs:
        ...
        for scene_subdir in scene_dirs:
            ...
            selected_sky_direction_scene = str(
                selected_npz["sky_direction_scene"][0])
            if len(selection) == 0 or len(pairs) == 0:
                # not a valid scene
                continue
            valid_scenes.append(scene_subdir)

            # STEP 3: parse the scene and export the list of valid (K, pose,
            # rgb, depth) and convert images
            scene_metadata_path = osp.join(
                out_scene_subdir, "scene_metadata.npz")
            if osp.isfile(scene_metadata_path):
                ...
                timestamps_selected = [
                    float(frame_id) for _,
                    frame_id in selected_images]
                ...
                for basename in images:
                    ...
                    depth_out = os.path.join(
                        out_scene_subdir, "lowres_depth", basename)
                    ...
                    if sky_direction_scene == "RIGHT":
                        ...
                        depth = cv2.rotate(
                            depth, cv2.ROTATE_90_COUNTERCLOCKWISE)
                    ...
                    if not osp.isfile(
                            depth_out):  # avoid destroying the base dataset when you mess up the paths
...
def convert_scene_metadata(
    ...
    sky_direction_scene, rotated_to_cam = find_scene_orientation(
        poses_cam_to_world)
    ...
    for i, (basename, frame_id) in enumerate(selected_images):
        intrinsic_fn = osp.join(
            intrinsics_dir,
            f"{scene_subdir}_{frame_id}.pincam")
        if not osp.exists(intrinsic_fn):
            intrinsic_fn = osp.join(
                intrinsics_dir,
                f"{scene_subdir}_{float(frame_id) - 0.001:.3f}.pincam")
        if not osp.exists(intrinsic_fn):
            intrinsic_fn = osp.join(
                intrinsics_dir,
                f"{scene_subdir}_{float(frame_id) + 0.001:.3f}.pincam")
...
def find_scene_orientation(poses_cam_to_world):
    if len(poses_cam_to_world) > 0:
        up_vector = sum(get_up_vectors(p)
                        for p in poses_cam_to_world) / len(poses_cam_to_world)
        right_vector = sum(get_right_vectors(p)
                           for p in poses_cam_to_world) / len(poses_cam_to_world)
    ...
    device_up_to_world_up_angle = (
        np.arccos(
            np.clip(
                np.dot(
                    np.transpose(up_world), up_vector), -1.0, 1.0)).item() * 180.0 / np.pi)
    device_right_to_world_up_angle = (
        np.arccos(
            np.clip(
                np.dot(
                    np.transpose(up_world), right_vector), -1.0, 1.0)).item() * 180.0 / np.pi)

    up_closest_to_90 = abs(
        device_up_to_world_up_angle -
        90.0) < abs(
        device_right_to_world_up_angle -
        90.0)
