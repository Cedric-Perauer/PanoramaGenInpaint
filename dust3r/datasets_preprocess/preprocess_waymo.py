...
def extract_frames_one_seq(filename):
    ...
    for data in tqdm(dataset, leave=False):
        ...
        for i, image in enumerate(frame.images):
            ...
            pix = cp_points_msk_tensor[...,
                                       1:3].numpy().round().astype(np.int16)
        ...
        if not "show full point cloud":
            show_raw_pointcloud([v["pts3d"] for v in views.values()], [
...
def crop_one_seq(input_dir, output_dir, seq, resolution=512):
    ...
    try:
        ...
        print(
            f"/!\\ Error: Missing calib.json in sequence {seq} /!\\",
            file=sys.stderr)
        return
    ...
    for cam_idx, cam_info in calib:
        ...
        cam_to_car[cam_idx] = np.asarray(
            cam_info["extrinsics"]).reshape(
            4, 4)  # cam-to-vehicle
    ...
    for frame in tqdm(frames, leave=False):
        ...
        pos2d = geotrf(
            intrinsics2 @ inv(cam_K[cam_idx]), pos2d).round().astype(np.int16)
        x, y = pos2d.T
        depthmap[y.clip(min=0, max=H - 1),
                 x.clip(min=0, max=W - 1)] = pts3d[:, 2]
        cv2.imwrite(osp.join(out_dir, frame + "exr"), depthmap)

        # save camera parametes
        cam2world = car_to_world @ cam_to_car[cam_idx] @ inv(
            axes_transformation)
        np.savez(
