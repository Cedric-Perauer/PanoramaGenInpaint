...
class RandomCropPair(torchvision.transforms.RandomCrop):
    # the crop will be intentionally different for the two images with this
    # class
    def forward(self, img1, img2):
        img1 = super().forward(img1)
        img2 = super().forward(img2)
...
class ColorJitterPair(torchvision.transforms.ColorJitter):
    # can be symmetric (same for both images) or assymetric (different jitter
    # params for each image) depending on assymetric_prob
...
def get_pair_transforms(transform_str, totensor=True, normalize=True):
    ...
    for s in transform_str.split("+"):
        if s.startswith("crop"):
            ...
            trfs.append(
                ColorJitterPair(
                    assymetric_prob=1.0, brightness=(
                        0.6, 1.4), contrast=(
                        0.6, 1.4), saturation=(
                        0.6, 1.4), hue=0.0))
