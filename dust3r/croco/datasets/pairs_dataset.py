...
def load_pairs_from_cache_file(fname, root=""):
    assert os.path.isfile(
        fname), "cannot parse pairs from {:s}, file does not exist".format(fname)
    with open(fname, "r") as fid:
        lines = fid.read().strip().splitlines()
    pairs = [
        (os.path.join(
            root, l.split()[0]), os.path.join(
            root, l.split()[1])) for l in lines]
    return pairs
...
def load_pairs_from_list_file(fname, root=""):
    ...
    with open(fname, "r") as fid:
        lines = fid.read().strip().splitlines()
    pairs = [(os.path.join(root, l + "_1.jpg"), os.path.join(root, l + "_2.jpg"))
...
def parse_and_cache_all_pairs(dname, data_dir="./data/"):
    if dname == "habitat_release":
        ...
        assert os.path.isdir(
            dirname), "cannot find folder for habitat_release pairs: " + dirname
        cache_file = os.path.join(dirname, "pairs.txt")
        assert not os.path.isfile(
            cache_file), "cache file already exists: " + cache_file
        ...
        for root, dirs, files in os.walk(dirname):
            ...
            dirs.sort()
            pairs += [
                (os.path.join(root, f), os.path.join(root, f[: -len("_1.jpeg")] + "_2.jpeg"))
...
def dnames_to_image_pairs(dnames, data_dir="./data/"):
    ...
    for dname in dnames.split("+"):
        if dname == "habitat_release":
            ...
            assert os.path.isdir(
                dirname), "cannot find folder for {:s} pairs: {:s}".format(dname, dirname)
            list_file = os.path.join(dirname, "listing.txt")
            assert os.path.isfile(list_file), "cannot find list file for {:s} pairs, see instructions. {:s}".format(
                dname, list_file
            )
