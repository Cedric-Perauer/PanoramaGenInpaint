...
def generate_multiview_images_from_metadata(
    ...
    """
    # Reorder paths by decreasing label length, to avoid collisions when
    # testing if a string by such label
    if scene_datasets_paths is not None:
        scene_datasets_paths = dict(
            sorted(
                scene_datasets_paths.items(),
                key=lambda x: len(
                    x[0]),
                reverse=True))
    ...
    generation_entries = dict([(key, value) for key, value in metadata.items(
    ...
    for idx_label, data in tqdm(metadata["multiviews"].items()):
        ...
        for oidx in range(n):
            observation = generator.render_viewpoint(
                positions[oidx], quaternion.from_float_array(
                    orientations[oidx]))
            # Leonid is indexing starting from 1
            ...
            if generate_depth:
                ...
                cv2.imwrite(
                    filename, observation["depth"], [
                        cv2.IMWRITE_EXR_TYPE, cv2.IMWRITE_EXR_TYPE_HALF])
                # Camera parameters
                camera_params = dict([(key, observation[key].tolist()) for key in (
                    "camera_intrinsics", "R_cam2world", "t_cam2world")])
                filename = os.path.join(
