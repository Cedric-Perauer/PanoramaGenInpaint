...
R_OPENCV2HABITAT = np.stack((habitat_sim.geo.RIGHT, -
                             habitat_sim.geo.UP, habitat_sim.geo.FRONT), axis=0)
...
def compute_camera_pose_opencv_convention(camera_position, camera_orientation):
    R_cam2world = quaternion.as_rotation_matrix(
        camera_orientation) @ R_OPENCV2HABITAT
...
def compute_pointcloud_overlaps_scikit(
    ...
    nbrs = NearestNeighbors(n_neighbors=1,
                            algorithm="kd_tree").fit(pointcloud2)
    ...
    if compute_symmetric:
        nbrs = NearestNeighbors(
            n_neighbors=1,
            algorithm="kd_tree").fit(pointcloud1)
        distances, indices = nbrs.kneighbors(pointcloud2)
        intersection2 = np.count_nonzero(
            distances.flatten() < distance_threshold)
...
class MultiviewHabitatSimGenerator:
    def __init__(
        ...
        # Minimum fraction of the scene which should be valid (well defined
        # depth)
        self.minimum_valid_fraction = 0.7

        # Distance threshold to see  to select pairs
        self.distance_threshold = 0.05
        # Minimum IoU of a view point cloud with respect to the reference view
        # to be kept.
    ...
    def _lazy_initialization(self):
        # Lazy random seeding and instantiation of the simulator to deal with
        # multiprocessing properly
        if self.seed is None:
            ...
            if self.navmesh is not None and self.navmesh != "":
                # Use pre-computed navmesh when available (usually better than
            ...
            if not self.sim.pathfinder.is_loaded:
                ...
                self.sim.recompute_navmesh(
                    self.sim.pathfinder, navmesh_settings, True)
    ...
    def sample_other_random_viewpoint(self, observed_point, nav_point):
        ...
        other_viewpoint_position = other_nav_point + \
            other_viewpoint_height * habitat_sim.geo.UP

        # Set viewing direction towards the central point
        rotation, position = look_at_for_habitat(
            eye=other_viewpoint_position, center=observed_point, up=habitat_sim.geo.UP, return_cam2world=True)
        rotation = rotation * \
            generate_orientation_noise(self.pan_range, self.tilt_range, self.roll_range)
        return position, rotation, other_nav_point

    def is_other_pointcloud_overlapping(
            self, ref_pointcloud, other_pointcloud):
        ...
        covisibility = min(
            overlap["intersection1"] /
            pixels_count,
            overlap["intersection2"] /
            pixels_count)
        is_valid = (
            valid_fraction >= self.minimum_valid_fraction) and (
    ...
    def is_other_viewpoint_overlapping(
        ...
        return self.is_other_pointcloud_overlapping(
    ...
    def __getitem__(self, useless_idx):
        ...
        if ref_valid_fraction < self.minimum_valid_fraction:
            # This should produce a recursion error at some point when
        ...
        for _ in range(self.views_count - 1):
            ...
            for sampling_attempt in range(self.max_attempts_count):
                ...
                # Observation
                other_viewpoint_observations = self.render_viewpoint(
                    position, rotation)
                other_pointcloud = compute_pointcloud(
                    other_viewpoint_observations["depth"], self.hfov, position, rotation)

                is_valid, valid_fraction, covisibility = self.is_other_pointcloud_overlapping(
                    ref_pointcloud, other_pointcloud)
                if is_valid:
        ...
        for i in range(len(viewpoints_observations)):
            ...
            for j in range(i + 1, len(viewpoints_observations)):
                overlap = compute_pointcloud_overlaps_scikit(
                    ...
                    self.distance_threshold,
                    compute_symmetric=True)
                pairwise_visibility_ratios[i,
                                           j] = overlap["intersection1"] / pixels_count
                pairwise_visibility_ratios[j,
                                           i] = overlap["intersection2"] / pixels_count
    ...
    def generate_random_spiral_trajectory(
        ...
        if len(ref_pointcloud) / pixels_count < self.minimum_valid_fraction:
            # Dirty hack: ensure that the valid part of the image is
            # significant
        ...
        for i, alpha in enumerate(np.linspace(0, 1, images_count)):
            ...
            position = ref_position + \
                (ref_R @ np.asarray([x, y, z]).reshape(3, 1)).flatten()
            if use_constant_orientation:
                orientation = ref_orientation
            else:
                # trajectory looking at a mean point in front of the ref
                # observation
