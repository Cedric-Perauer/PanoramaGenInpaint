...
SceneData = collections.namedtuple(
    "SceneData", [
        "scene_dataset_config_file", "scene", "navmesh", "output_dir"])


def list_replicacad_scenes(
        base_output_dir,
        base_path=SCENES_DATASET["replica_cad"]):
    ...
    navmeshes = [
        f"navmeshes/apt_{i}_static_furniture.navmesh" for i in range(6)] + ["empty_stage.navmesh"]
    scenes_data = []
...
def list_replica_cad_baked_lighting_scenes(
        base_output_dir,
        base_path=SCENES_DATASET["replica_cad_baked_lighting"]):
    scene_dataset_config_file = os.path.join(
...
def list_scenes(base_output_dir, base_path):
    ...
    for root, dirs, files in os.walk(base_path, followlinks=True):
        ...
        # When two meshesxxxx.basis.glb and xxxx.glb are present, use the
        # 'basis' version.
        basis_scenes = [data.scene[: -len(".basis.glb")]
                        for data in folder_scenes_data if data.scene.endswith(".basis.glb")]
        if len(basis_scenes) != 0:
            folder_scenes_data = [data for data in folder_scenes_data if not (
                data.scene[: -len(".glb")] in basis_scenes)]

        scenes_data.extend(folder_scenes_data)
    return scenes_data


def list_scenes_available(
        base_output_dir,
        scenes_dataset_paths=SCENES_DATASET):
    scenes_data = []
