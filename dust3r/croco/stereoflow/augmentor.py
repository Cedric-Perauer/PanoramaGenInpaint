...
class StereoAugmentor(object):
    ...
    def _random_scale(self, img1, img2, disp):
        ...
        if self.scale_prob > 0.0 and np.random.rand() < self.scale_prob:
            min_scale, max_scale = (
                (self.lminscale, self.lmaxscale) if min(
                    h, w) < self.lhth else (
            ...
            disp = (
                cv2.resize(
                    ...
                    fx=scale_x,
                    fy=scale_y,
                    interpolation=cv2.INTER_LINEAR if not self.scale_interp_nearest else cv2.INTER_NEAREST,
                ) *
                scale_x)
    ...
    def _random_rotate_shift_right(self, img2):
        if self.rightjitterprob > 0.0 and np.random.rand() < self.rightjitterprob:
            ...
            image_center = (
                np.random.uniform(
                    0, img2.shape[0]), np.random.uniform(
                    0, img2.shape[1]))
...
class FlowAugmentor:
    ...
    def _resize_flow(self, flow, scale_x, scale_y, factor=1.0):
        if np.all(np.isfinite(flow)):
            ...
            # invalid value every where, before we fill it with the correct
            # ones
