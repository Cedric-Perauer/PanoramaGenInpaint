...
class StereoDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        Ldispname = self.pairname_to_Ldispname(
            pairname) if self.pairname_to_Ldispname is not None else None

        # load images and disparities
        Limg = _read_img(Limgname)
        Rimg = _read_img(Rimgname)
        disp = self.load_disparity(
            Ldispname) if Ldispname is not None else None

        # sanity check
        if disp is not None:
            assert np.all(
                disp > 0) or self.name == "Spring", (self.name, pairname, Ldispname)
...
class SceneFlowDataset(StereoDataset):
    ...
    def _prepare_data(self):
        ...
        self.pairname_to_Limgname = lambda pairname: osp.join(
            self.root, pairname)
        self.pairname_to_Rimgname = lambda pairname: osp.join(
            self.root, pairname).replace("/left/", "/right/")
    ...
    def _build_cache(self):
        ...
        pairs = sorted(
            glob(
                self.root +
                "Driving/frames_finalpass/*/*/*/left/*.png"))
        ...
        pairs = sorted(
            glob(
                self.root +
                "Monkaa/frames_finalpass/*/left/*.png"))
        pairs = list(map(lambda x: x[len(self.root):], pairs))
        ...
        pairs = sorted(
            glob(
                self.root +
                "FlyingThings/frames_finalpass/TRAIN/*/*/left/*.png"))
        pairs = list(map(lambda x: x[len(self.root):], pairs))
        ...
        testpairs = sorted(
            glob(
                self.root +
                "FlyingThings/frames_finalpass/TEST/*/*/left/*.png"))
        ...
        assert len(
            test1of100pairs) == 44, "incorrect parsing of pairs in SceneFlow"
        # all
        tosave = {
        ...
        tosave["test_allpass"] = tosave["test_finalpass"] + \
            tosave["test_cleanpass"]
        return tosave
...
class Md21Dataset(StereoDataset):
    ...
    def _build_cache(self):
        ...
        trainpairs = []
        for s in seqs:
            # trainpairs += [s+'/im0.png'] # we should remove it, it is
            # included as such in other lightings
            trainpairs += [
                s + "/ambient/" + b + "/" + a
        ...
        subtrainpairs = [p for p in trainpairs if any(
            p.startswith(s + "/") for s in seqs[:-2])]
        subvalpairs = [p for p in trainpairs if any(
            p.startswith(s + "/") for s in seqs[-2:])]
        assert len(subtrainpairs) == 335 and len(
            subvalpairs) == 20, "incorrect parsing of pairs in Middlebury 2021"
        tosave = {
            "train": trainpairs,
...
class Md14Dataset(StereoDataset):
    ...
    def _build_cache(self):
        ...
        subtrainpairs = [
            p for p in trainpairs if not any(
                p.startswith(
                    s + "/") for s in valseqs)]
        subvalpairs = [
            p for p in trainpairs if any(
                p.startswith(
                    s + "/") for s in valseqs)]
        assert len(subtrainpairs) == 132 and len(
            subvalpairs) == 6, "incorrect parsing of pairs in Middlebury 2014"
        tosave = {
            "train": trainpairs,
            "subtrain": subtrainpairs,
            "subval": subvalpairs}
    ...
    def _build_cache(self):
        ...
        subvalpairs = [
            p for p in trainpairs if any(
                p.startswith(
                    s + "/") for s in valseqs)]
        assert len(subtrainpairs) == 171 and len(
            subvalpairs) == 18, "incorrect parsing of pairs in Middlebury 2006"
        tosave = {
            "train": trainpairs,
            "subtrain": subtrainpairs,
            "subval": subvalpairs}
    ...
    def _build_cache(self):
        ...
        for s in seqs:
            for i in ["Illum1", "Illum2", "Illum3"]:
                for e in ["Exp0", "Exp1", "Exp2"]:
                    trainpairs.append(osp.join(s, i, e, "view1.png"))
        assert len(
            trainpairs) == 54, "incorrect parsing of pairs in Middlebury 2005"
        valseqs = ["Reindeer"]
        assert all(s in seqs for s in valseqs)
        subtrainpairs = [
            p for p in trainpairs if not any(
        ...
        subvalpairs = [
            p for p in trainpairs if any(
                p.startswith(
                    s + "/") for s in valseqs)]
        assert len(subtrainpairs) == 45 and len(
            subvalpairs) == 9, "incorrect parsing of pairs in Middlebury 2005"
        tosave = {
            "train": trainpairs,
            "subtrain": subtrainpairs,
            "subval": subvalpairs}
...
class MdEval3Dataset(StereoDataset):
    ...
    def _prepare_data(self):
        ...
        self.pairname_to_Ldispname = lambda pairname: (
            None if pairname.startswith("test") else osp.join(
                self.root, pairname, "disp0GT.pfm"))
        self.pairname_to_str = lambda pairname: pairname
        self.load_disparity = _read_middlebury_disp
        # for submission only
        self.submission_methodname = "CroCo-Stereo"
        self.submission_sresolution = (
            "F" if self.split.endswith("_full") else (
    ...
    def submission_save_pairname(self, pairname, prediction, outdir, time):
        ...
        timefile = os.path.join(
            ...
            "time" + self.submission_methodname + ".txt")
    ...
    def finalize_submission(self, outdir):
        ...
        os.system(cmd)
        print(
            f"Done. Submission file at {outdir}/{self.submission_methodname}.zip")
...
class ETH3DLowResDataset(StereoDataset):
    ...
    def _prepare_data(self):
        ...
        self.pairname_to_Ldispname = (
            ...
            if self.split == "test"
                ...
                else osp.join(self.root, pairname.replace("train/", "train_gt/"), "disp0GT.pfm")
            )
        )
        self.pairname_to_str = lambda pairname: pairname
    ...
    def _build_cache(self):
        ...
        testpairs = [
            ...
            s for s in sorted(
                os.listdir(
                    self.root +
                    "test/"))]
        assert len(trainpairs) == 27 and len(
            testpairs) == 20, "incorrect parsing of pairs in ETH3D Low Res"
        subvalpairs = [
        ...
        assert len(subvalpairs) == 3 and len(
            subtrainpairs) == 24, "incorrect parsing of pairs in ETH3D Low Res"
        tosave = {
            "train": trainpairs,
            "test": testpairs,
            "subtrain": subtrainpairs,
    ...
    def finalize_submission(self, outdir):
        ...
        os.system(cmd)
        print(
            f"Done. Submission file at {outdir}/eth3d_low_res_two_view_results.zip")
...
class BoosterDataset(StereoDataset):
    ...
    def _build_cache(self):
        ...
        trainpairs = [
            ...
            for s in trainseqs
            for imname in sorted(os.listdir(self.root + "train/balanced/" + s + "/camera_00/"))
        ]
        testpairs = [
            "test/balanced/" + s + "/camera_00/" + imname
            for s in sorted(os.listdir(self.root + "test/balanced"))
            for imname in sorted(os.listdir(self.root + "test/balanced/" + s + "/camera_00/"))
        ]
        assert len(trainpairs) == 228 and len(testpairs) == 191
        subtrainpairs = [p for p in trainpairs if any(
            s in p for s in trainseqs[:-2])]
        subvalpairs = [p for p in trainpairs if any(
            s in p for s in trainseqs[-2:])]
...
class SpringDataset(StereoDataset):
    ...
    def _build_cache(self):
        ...
        testpairs += [p.replace("frame_left", "frame_right")
                      for p in testpairs]
        ...
        assert (len(trainpairs) == 5000 and len(testpairs) == 2000 and len(
            subtrainpairs) == 4904 and len(subvalpairs) == 96), "incorrect parsing of pairs in Spring"
        tosave = {
            "train": trainpairs,
            "test": testpairs,
            "subtrain": subtrainpairs,
...
class Kitti15Dataset(StereoDataset):
    ...
    def _build_cache(self):
        ...
        assert (len(trainseqs) == 200 and len(subtrainseqs) == 195 and len(
            subvalseqs) == 5 and len(testseqs) == 200), "incorrect parsing of pairs in Kitti15"
        tosave = {
            "train": trainseqs,
    ...
    def submission_save_pairname(self, pairname, prediction, outdir, time):
        ...
        assert prediction.dtype == np.float32
        outfile = os.path.join(
            outdir, "disp_0", pairname.split("/")[-1] + "_10.png")
        os.makedirs(os.path.dirname(outfile), exist_ok=True)
        img = (prediction * 256).astype("uint16")
        Image.fromarray(img).save(outfile)
