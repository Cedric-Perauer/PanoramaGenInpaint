...
def get_args_parser():
    parser = argparse.ArgumentParser(
        "Test CroCo models on stereo/flow", add_help=False)
    # important argument
    ...
    # save (it will automatically go to
    # <model_path>_<dataset_str>/<tile_str>_<save>)
    parser.add_argument(
        "--save",
        type=str,
        nargs="+",
...
def _save_batch(
    ...
    for i in range(len(pairnames)):
        ...
        pairname = eval(pairnames[i]) if pairnames[i].startswith(
            "(") else pairnames[i]  # unbatch pairname
        ...
        if "visu" in save:
            if task == "stereo":
                ...
                # normalize flowToColor according to the maxnorm of gt (or
                # prediction if not available)
                flowNorm = torch.sqrt(
                    torch.sum(
                        (gti if gt is not None else predi) ** 2,
                        dim=0)).max().item()
                imgflow = flowToColor(
                    predi.permute(
                        (1, 2, 0)).cpu().numpy(), maxflow=flowNorm)
        ...
        if "submission" in save:
            ...
            dataset.submission_save_pairname(
...
def main(args):
    ...
    model, metrics, cropsize, with_conf, task, tile_conf_mode = _load_model_and_criterion(
        args.model, "metrics" in args.save, device)
    if args.tile_conf_mode == "":
        args.tile_conf_mode = tile_conf_mode

    # load the datasets
    datasets = (get_test_datasets_stereo if task ==
                "stereo" else get_test_datasets_flow)(args.dataset)
    dataloaders = [
        DataLoader(
            dataset,
            batch_size=1,
            ...
            pin_memory=True,
            drop_last=False) for dataset in datasets]
    ...
    for i, dataloader in enumerate(dataloaders):
        ...
        for data_iter_step, (image1, image2, gt,
                             pairnames) in enumerate(tqdm(dataloader)):

            do_flip = (
                task == "stereo" and dstr.startswith("Spring") and any("right" in p for p in pairnames)
            )  # we flip the images and will flip the prediction after as we assume img1 is on the left

            image1 = image1.to(device, non_blocking=True)
            image2 = image2.to(device, non_blocking=True)
            gt = gt.to(device, non_blocking=True) if gt.numel(
            ) > 0 else None  # special case for test time
            if do_flip:
                assert all("right" in p for p in pairnames)
                # this is already the right frame, let's flip it
                image1 = image1.flip(dims=[3])
