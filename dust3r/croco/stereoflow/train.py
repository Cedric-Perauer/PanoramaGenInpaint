...
def get_args_parser():
    ...
    parser = argparse.ArgumentParser(
        "Finetuning CroCo models on stereo or flow",
        add_help=False)
    subparsers = parser.add_subparsers(
        title="Task (stereo or flow)",
        dest="task",
        required=True)
    parser_stereo = subparsers.add_parser(
        "stereo", help="Training stereo model")
    ...
    add_arg("--min_lr", type=float, default=0.0, metavar="LR",
    ...
    add_arg(
        ...
        default=0.7,
        type=float,
        help="Overlap value for the tiling")
    # others
    add_arg("--num_workers", default=8, type=int)
    add_arg("--eval_every", type=int, default=1,
            help="Val loss evaluation frequency")
    add_arg(
        "--save_every",
        type=int,
        default=1,
...
def main(args):
    ...
    metrics = (StereoMetrics if args.task ==
               "stereo" else FlowMetrics)().to(device)
    ...
    if criterion.with_conf:
        num_channels += 1
    print(
        f"Building head PixelwiseTaskWithDPT() with {num_channels} channel(s)")
    ...
    if global_rank == 0 and args.output_dir is not None:
        log_writer = SummaryWriter(
            ...
            purge_step=args.start_epoch * 1000)
    ...
    if args.val_dataset == "":
        ...
        val_datasets = (
            get_test_datasets_stereo if args.task == "stereo" else get_test_datasets_flow)(
            args.val_dataset)
        ...
        bestmetric = ("AVG_" if len(data_loaders_val) > 1 else str(
            data_loaders_val[0].dataset) + "_") + args.bestmetric
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # Validation (current naive implementation runs the validation on every
        # gpu ... not smart ...)
        if data_loaders_val is not None and args.eval_every > 0 and (
        ...
        if args.output_dir and (
            (epoch + 1) %
                args.save_every == 0 or epoch + 1 == args.epochs):
            misc.save_model(
                args=args,
                model_without_ddp=model_without_ddp,
                optimizer=optimizer,
