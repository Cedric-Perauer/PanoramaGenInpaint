...
def train_one_epoch(
    ...
    if args.img_per_epoch:
        iter_per_epoch = args.img_per_epoch // args.batch_size + \
            int(args.img_per_epoch % args.batch_size > 0)
        assert len(
            data_loader) >= iter_per_epoch, "Dataset is too small for so many iterations"
    ...
    for data_iter_step, (image1, image2, gt, pairname) in enumerate(
        ...
        with torch.cuda.amp.autocast(enabled=bool(args.amp)):
            ...
            prediction, conf = split_prediction_conf(
                prediction, criterion.with_conf)
            batch_metrics = metrics(prediction.detach(), gt)
            loss = criterion(
                prediction, gt) if conf is None else criterion(
                prediction, gt, conf)
        ...
        if log_writer is not None and time_to_log:
            epoch_1000x = int(
                (data_iter_step / len_data_loader + epoch) * 1000)
            # We use epoch_1000x as the x-axis in tensorboard. This calibrates
            # different curves when batch size changes.
...
def validate_one_epoch(
    ...
    for didx, data_loader in enumerate(data_loaders):
        ...
        for data_iter_step, (image1, image2, gt, pairname) in enumerate(
            ...
            if dname.startswith("Spring"):
                assert gt.size(2) == image1.size(
                    2) * 2 and gt.size(3) == image1.size(3) * 2
                gt = (
                    gt[:, :, 0::2, 0::2] + gt[:, :, 0::2, 1::2] + gt[:, :, 1::2, 0::2] + gt[:, :, 1::2, 1::2]
    ...
    results = {
        k: meter.global_avg for ml in metric_loggers for k,
        meter in ml.meters.items()}
    if len(dnames) > 1:
        for k in batch_metrics.keys():
            results["AVG_" + k] = sum(results[dname + "_" + k]
                                      for dname in dnames) / len(dnames)
...
def tiled_pred(
    ...
    if conf_mode.startswith("conf_expsigmoid_"):  # conf_expsigmoid_30_10
        beta, betasigmoid = map(
            float, conf_mode[len("conf_expsigmoid_"):].split("_"))
    ...
    # accumulate the weighted sum of confidences ; not so useful except for
    # computing some losses
    ...
    for sy1, sx1, sy2, sx2, aligned in crop_generator():
        ...
        if criterion is not None and gt is not None:
            tiled_losses.append(
                criterion(
                    pred,
                    gtcrop).item() if predconf is None else criterion(
                    pred,
                    gtcrop,
                    predconf).item())

        if conf_mode.startswith("conf_expsigmoid_"):
            conf = torch.exp(-beta * 2 * (torch.sigmoid(predconf /
                             betasigmoid) - 0.5)).view(B, win_height, win_width)
...
def _crop(img, sy, sx):
    ...
    return img[:, :, slice(sy.start + t, sy.stop + t),
               slice(sx.start + l, sx.stop + l)]
