...
dataset_to_root.update(**{"TartanAir": "./data/stereoflow/TartanAir",
                          "FlyingChairs": "./data/stereoflow/FlyingChairs/",
                          "FlyingThings": osp.join(dataset_to_root["SceneFlow"],
                                                   "FlyingThings") + "/",
...
class FlowDataset(data.Dataset):
    ...
    def __init__(self, split, augmentor=False, crop_size=None, totensor=True):
        ...
        self.has_constant_resolution = (
            # whether the dataset has constant resolution or not (=> don't use
            # batch_size>1 at test time)
            True
        )
        self._prepare_data()
    ...
    def __getitem__(self, index):
        ...
        flowname = self.pairname_to_flowname(
            pairname) if self.pairname_to_flowname is not None else None
    ...
    def _set_root(self):
        ...
        assert os.path.isdir(
            self.root), f"could not find root directory for dataset {self.name}: {self.root}"
...
class TartanAirDataset(FlowDataset):
    ...
    def _prepare_data(self):
        ...
        self.pairname_to_flowname = lambda pairname: osp.join(
            self.root, pairname[0], "flow/{:06d}_{:06d}_flow.npy".format(pairname[1], pairname[2])
        )
        self.pairname_to_str = lambda pairname: os.path.join(pairname[0][pairname[0].find(
            "/") + 1:], "{:06d}_{:06d}".format(pairname[1], pairname[2]))
    ...
    def _build_cache(self):
        ...
        pairs = [
            ...
            for a in sorted(os.listdir(osp.join(self.root, s, s, difficulty, Pxxx, "image_left/")))[:-1]
        ]
...
class FlyingChairsDataset(FlowDataset):
    ...
    def _build_cache(self):
        ...
        trainpairs = ["{:05d}".format(i)
                      for i in np.where(split_list == 1)[0] + 1]
        valpairs = ["{:05d}".format(i)
                    for i in np.where(split_list == 2)[0] + 1]
        assert len(trainpairs) == 22232 and len(
            valpairs) == 640, "incorrect parsing of pairs in MPI-Sintel"
    ...
    def _build_cache(self):
        ...
        for set_ in ["train", "test"]:
            ...
            pp = [
                ...
                for fname in sorted(os.listdir(osp.join(sroot, d, s, "into_future/left")))[:-1]
            ]
            pairs = [(a, i, i + 1) for a, i in pp]
            pairs += [(a.replace("into_future", "into_past"), i + 1, i)
                      for a, i in pp]
            assert len(pairs) == {"train": 40302, "test": 7866}[
                set_], "incorrect parsing of pairs Flying Things"
        ...
        # see
        # https://github.com/autonomousvision/unimatch/blob/master/dataloader/flow/datasets.py#L229
        ...
        for cam in ["left", "right"]:
            camstr = "" if cam == "left" else f"_{cam}cam"
            for pass_ in ["final", "clean"]:
                tosave[f"test1024_{pass_}pass{camstr}"] = sorted(tosave[f"test_{pass_}pass{camstr}"])[:-remove][
                    ::stride
                ]  # warning, it was not sorted before
            assert len(tosave["test1024_cleanpass"]
                       ) == 1024, "incorrect parsing of pairs in Flying Things"
...
class MPISintelDataset(FlowDataset):
    ...
    def _build_cache(self):
        ...
        subvalpairs = [
            (p, i) for p, i in trainpairs if any(
                s in p for s in subvalseqs)]
        subtrainpairs = [
            (p, i) for p, i in trainpairs if any(
                s in p for s in subtrainseqs)]
        ...
        assert (len(trainpairs) == 1041 and len(testpairs) == 552 and len(subvalpairs) ==
                98 and len(subtrainpairs) == 943), "incorrect parsing of pairs in MPI-Sintel"
        tosave = {}
        ...
        for t in ["train", "test", "subval", "subtrain"]:
            tosave[t + "_finalpass"] = [(p.replace("/clean/", "/final/"), i)
                                        for p, i in tosave[t + "_cleanpass"]]
            tosave[t + "_allpass"] = tosave[t + "_cleanpass"] + \
                tosave[t + "_finalpass"]
        return tosave
    ...
    def finalize_submission(self, outdir):
        ...
        # eg <bundle_exe> <path_to_results_for_clean>
        # <path_to_results_for_final> <output/bundled.lzma>
        ...
        if os.path.isfile(bundle_exe):
            ...
            os.system(cmd)
            print(
                f'Done. Submission file at: "{outdir}/submission/bundled.lzma"')
        else:
...
class SpringDataset(FlowDataset):
    ...
    def _prepare_data(self):
        ...
        self.pairname_to_img2name = lambda pairname: osp.join(
            ...
            pairname[1],
            "frame_" + pairname[3],
            "frame_{:s}_{:04d}.png".format(pairname[3], pairname[4] + (1 if pairname[2] == "FW" else -1)),
        )
        self.pairname_to_flowname = lambda pairname: (
            None
            if pairname[0] == "test"
    ...
    def _build_cache(self):
        ...
        for leftright in ["left", "right"]:
            for fwbw in ["FW", "BW"]:
                trainpairs += [
                    ("train", s, fwbw, leftright, int(f[len(f"flow_{fwbw}_{leftright}_"): -len(".flo5")]))
                    for s in trainseqs
                    for f in sorted(os.listdir(osp.join(self.root, "train", s, f"flow_{fwbw}_{leftright}")))
                ]
        # test
        ...
        for leftright in ["left", "right"]:
            testpairs += [
                ("test", s, "FW", leftright, int(f[len(f"frame_{leftright}_"): -len(".png")]))
                for s in testseqs
                for f in sorted(os.listdir(osp.join(self.root, "test", s, f"frame_{leftright}")))[:-1]
            ]
            testpairs += [
                ("test", s, "BW", leftright, int(f[len(f"frame_{leftright}_"): -len(".png")]) + 1)
                for s in testseqs
                for f in sorted(os.listdir(osp.join(self.root, "test", s, f"frame_{leftright}")))[:-1]
            ]
        # subtrain / subval
    ...
    def finalize_submission(self, outdir):
        ...
        if os.path.isfile(exe):
            ...
            os.system(cmd)
            print(
                f"Done. Submission file at {outdir}/test/flow_submission.hdf5")
        else:
...
class Kitti12Dataset(FlowDataset):
    ...
    def _prepare_data(self):
        ...
        self.pairname_to_flowname = (
            None if self.split == "test" else lambda pairname: osp.join(
                self.root, pairname.replace(
                    "/colored_0/", "/flow_occ/") + "_10.png"))
        self.pairname_to_str = lambda pairname: pairname.replace(
            "/colored_0/", "/")
    ...
    def _build_cache(self):
        ...
        assert len(trainseqs) == 194 and len(
            testseqs) == 195, "incorrect parsing of pairs in Kitti12"
...
class Kitti15Dataset(FlowDataset):
    ...
    def _build_cache(self):
        ...
        assert (len(trainseqs) == 200 and len(subtrainseqs) == 190 and len(
            subvalseqs) == 10 and len(testseqs) == 200), "incorrect parsing of pairs in Kitti15"
        tosave = {
            "train": trainseqs,
    ...
    def submission_save_pairname(self, pairname, prediction, outdir, time):
        ...
        assert prediction.shape[2] == 2
        outfile = os.path.join(
            outdir, "flow", pairname.split("/")[-1] + "_10.png")
        os.makedirs(os.path.dirname(outfile), exist_ok=True)
        writeFlowKitti(outfile, prediction)
...
def readFlowFile(filename):
    ...
    with open(filename, "rb") as f:
        ...
        if tag != TAG_FLOAT:
            raise Exception(
                "flow_utils.readFlowFile({:s}): wrong tag".format(filename))
        # read dimension
        w, h = struct.unpack("ii", f.read(8))
        if w < MIN_WIDTH or w > MAX_WIDTH:
            raise Exception(
                "flow_utils.readFlowFile({:s}: illegal width {:d}".format(
                    filename, w))
        if h < MIN_HEIGHT or h > MAX_HEIGHT:
            raise Exception(
                "flow_utils.readFlowFile({:s}: illegal height {:d}".format(
                    filename, h))
        flow = np.fromfile(f, "float32")
        if not flow.shape == (h * w * 2,):
            raise Exception(
                "flow_utils.readFlowFile({:s}: illegal size of the file".format(filename))
        flow.shape = (h, w, 2)
        return flow
...
def writeFlowFile(flow, filename):
    ...
    if not filename.endswith(".flo"):
        raise Exception(
            "flow_utils.writeFlowFile(<flow>,{:s}): filename must finish with '.flo'".format(filename))

    if not flow.shape[2:] == (2,):
        raise Exception(
            "flow_utils.writeFlowFile(<flow>,{:s}): <flow> must have 2 bands".format(filename))
...
def colorTest():
    ...
    img = _computeColor(np.concatenate(
        (u[:, :, np.newaxis], v[:, :, np.newaxis]), 2) / trange / np.sqrt(2))
    plt.imshow(img)
    plt.axis("off")
...
def _computeColor(flow, saturate=True):
    ...
    for i in range(nchans):
        ...
        img[:, :, i] = (
            255 * col * (1 - nanidx.astype("float"))).astype("uint8")
