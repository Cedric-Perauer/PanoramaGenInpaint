...
class L1Loss(nn.Module):
    ...
    def forward(self, predictions, gt, inspect=False):
        ...
        if self.max_gtnorm is not None:
            mask *= _get_gtnorm(gt).expand(-1, gt.size(1), -
                                           1, -1) < self.max_gtnorm
        if inspect:
...
class LaplacianLoss(nn.Module):  # used for CroCo-Stereo on ETH3D, d'=exp(d)
    ...
    def forward(self, predictions, gt, conf):
        ...
        return (
            ...
            torch.exp(
                conf[mask]) +
            conf[mask]).mean()  # + torch.log(2) => which is a constant


class LaplacianLossBounded(
        nn.Module):  # used for CroCo-Flow ; in the equation of the paper, we have a=1/b
    ...
    def forward(self, predictions, gt, conf):
        ...
        return (
            torch.abs(
                ...
                dim=1)[mask] /
            conf[mask] +
            torch.log(conf)[mask]).mean()  # + torch.log(2) => which is a constant
...
class StereoMetrics(nn.Module):
    ...
    def forward(self, predictions, gt):
        ...
        for ths in self.bad_ths:
            metrics["bad@{:.1f}".format(ths)] = (((L1error > ths)
                                                  * mask.view(B, -1)).sum(dim=1) / Npx).mean(dim=0) * 100
        return metrics
...
class FlowMetrics(nn.Module):
    ...
    def forward(self, predictions, gt):
        ...
        Npx = mask.view(B, -1).sum(dim=1)
        # to compute L1/L2 error, we need to have non-infinite value, the error
        # computed at this locations will be ignored
        gtcopy = (gt.clone())
        gtcopy[:, 0, :, :][~mask] = 999999.0
        gtcopy[:, 1, :, :][~mask] = 999999.0
        L1error = (torch.abs(gtcopy - predictions).sum(dim=1)
                   * mask).view(B, -1)
        L2error = (torch.sqrt(torch.sum(torch.square(
            gtcopy - predictions), dim=1)) * mask).view(B, -1)
        metrics["L1err"] = torch.mean(L1error.sum(dim=1) / Npx)
        metrics["EPE"] = torch.mean(L2error.sum(dim=1) / Npx)
        for ths in self.bad_ths:
            metrics["bad@{:.1f}".format(ths)] = (((L2error > ths)
                                                  * mask.view(B, -1)).sum(dim=1) / Npx).mean(dim=0) * 100
        return metrics
...
class StereoDatasetMetrics(nn.Module):
    ...
    def add_batch(self, predictions, gt):
        ...
        if (gt.size(2) == predictions.size(2) *
            2 and gt.size(3) == predictions.size(3) *
                2):  # special case for Spring ...
            L1err = torch.minimum(
                torch.minimum(
                    torch.minimum(
            ...
            # both x and y would be infinite
            valid = torch.isfinite(gt[:, 0, :, :])
            L1err = torch.sum(torch.abs(gt - predictions), dim=1)
        N = valid.sum()
        Nnew = self.agg_N + N
        self.agg_L1err = float(self.agg_N) / Nnew * self.agg_L1err + \
    ...
    def _compute_metrics(self):
        ...
        for i, th in enumerate(self.bad_ths):
            out["bad@{:.1f}".format(th)] = (float(self.agg_Nbad[i]
                                                  ) / self.agg_N).item() * 100.0
        self._metrics = out
...
class FlowDatasetMetrics(nn.Module):
    ...
    def reset(self):
        ...
        self.agg_EPEspeed = [torch.tensor(0.0) for _ in self.speed_ths]
        # N pixels per speed bin so far
    ...
    def add_batch(self, predictions, gt):
        ...
        self.agg_L1err = float(self.agg_N) / Nnew * self.agg_L1err + \
            L1err[valid].mean().cpu() * float(N) / Nnew
        self.agg_L2err = float(self.agg_N) / Nnew * self.agg_L2err + \
    ...
    def _compute_metrics(self):
        ...
        for i, (th1, th2) in enumerate(self.speed_ths):
            out["s{:d}{:s}".format(
                th1, "-" + str(th2) if th2 < torch.inf else "+")] = self.agg_EPEspeed[i].item()
        self._metrics = out
