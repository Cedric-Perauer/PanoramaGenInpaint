...
class CroCoNet(nn.Module):
    ...
    def __init__(
        ...
        # whether to apply normalization of the 'memory' = (second image) in
        # the decoder
        ...
        if pos_embed == "cosine":
            ...
            enc_pos_embed = get_2d_sincos_pos_embed(enc_embed_dim, int(
                self.patch_embed.num_patches**0.5), n_cls_token=0)
            self.register_buffer(
                "enc_pos_embed",
                torch.from_numpy(enc_pos_embed).float())
            # positional embedding of the decoder
            dec_pos_embed = get_2d_sincos_pos_embed(dec_embed_dim, int(
                self.patch_embed.num_patches**0.5), n_cls_token=0)
            self.register_buffer(
                "dec_pos_embed",
                torch.from_numpy(dec_pos_embed).float())
        ...
        self.enc_blocks = nn.ModuleList(
            [
                Block(
                    ...
                    rope=self.rope) for i in range(enc_depth)])
