    ...
    def setup(self, croconet):
        ...
        if self.hooks_idx is None:
            if hasattr(croconet, "dec_blocks"):  # encoder + decoder
                ...
                hooks_idx = [croconet.dec_depth + croconet.enc_depth -
                             1 - i * step for i in range(3, -1, -1)]
            else:  # encoder only
                step = croconet.enc_depth // 4
                hooks_idx = [croconet.enc_depth - 1 -
                             i * step for i in range(3, -1, -1)]
            self.hooks_idx = hooks_idx
            print(
        ...
        dim_tokens = [croconet.enc_embed_dim if hook <
                      croconet.enc_depth else croconet.dec_embed_dim for hook in self.hooks_idx]
