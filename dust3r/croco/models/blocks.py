...
class Attention(nn.Module):
    ...
    def forward(self, x, xpos):
        ...
        # q,k,v = qkv.unbind(2)  # make torchscript happy (cannot use tensor as
        # tuple)
...
class DecoderBlock(nn.Module):
    ...
    def forward(self, x, y, xpos, ypos):
        ...
        y_ = self.norm_y(y)
        x = x + \
            self.drop_path(self.cross_attn(self.norm2(x), y_, y_, xpos, ypos))
...
class PositionGetter(object):
    ...
    def __call__(self, b, h, w, device):
        if not (h, w) in self.cache_positions:
            ...
            self.cache_positions[h, w] = torch.cartesian_prod(
                y, x)  # (h, w, 2)
        pos = self.cache_positions[h, w].view(
            1, h * w, 2).expand(b, -1, 2).clone()
        return pos
