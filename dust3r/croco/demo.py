...
def main():
    ...
    imagenet_mean_tensor = torch.tensor(imagenet_mean).view(
        1, 3, 1, 1).to(device, non_blocking=True)
    imagenet_std = [0.229, 0.224, 0.225]
    imagenet_std_tensor = torch.tensor(imagenet_std).view(
        1, 3, 1, 1).to(device, non_blocking=True)
    trfs = Compose([ToTensor(), Normalize(
        mean=imagenet_mean, std=imagenet_std)])
    image1 = trfs(Image.open("assets/Chateau1.png").convert("RGB")
                  ).to(device, non_blocking=True).unsqueeze(0)
    image2 = trfs(Image.open("assets/Chateau2.png").convert("RGB")
                  ).to(device, non_blocking=True).unsqueeze(0)

    # load model
    ckpt = torch.load(
        "pretrained_models/CroCo_V2_ViTLarge_BaseDecoder.pth",
        "cpu")
    ...
    # the output is normalized, thus use the mean/std of the actual image to
    ...
    image_masks = model.unpatchify(model.patchify(
        torch.ones_like(ref_image)) * mask[:, :, None])
