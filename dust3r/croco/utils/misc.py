...
class MetricLogger(object):
    ...
    def log_every(self, iterable, print_freq, header=None, max_iter=None):
        ...
        len_iterable = min(
            len(iterable),
            max_iter) if max_iter else len(iterable)
        ...
        print("{} Total time: {} ({:.4f} s / it)".format(header,
              total_time_str, total_time / len_iterable))
...
def load_model(args, model_without_ddp, optimizer, loss_scaler):
    ...
    if args.resume is not None:
        ...
        if "best_so_far" in checkpoint:
            ...
            print("")
        print(
            "With optim & sched! start_epoch={:d}".format(
                args.start_epoch), end="")
    return best_so_far
...
def _replace(text, src, tgt, rm=""):
    ...
    if len(tgt) == 1:
        tgt = tgt * len(src)
    assert len(src) == len(
        tgt), f"'{src}' and '{tgt}' should have the same len"
...
def get_parameter_groups(
    ...
    if layer_decay < 1.0:
        ...
        layer_decay_values = list(layer_decay ** (num_layers + 1 - i)
    ...
    for name, param in model.named_parameters():
        ...
        if group_name not in parameter_group_names:
            ...
            parameter_group_names[group_name] = {
                "weight_decay": this_weight_decay, "params": [], "lr_scale": scale}
...
def adjust_learning_rate(optimizer, epoch, args):
    ...
    if epoch < args.warmup_epochs:
        ...
        lr = args.min_lr + (args.lr - args.min_lr) * 0.5 * (1.0 + math.cos(
