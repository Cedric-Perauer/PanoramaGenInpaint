...
def get_args_parser():
    ...
    parser.add_argument(
        ...
        type=str,
        help="training set")
    # in the paper, we also use some homography and rotation, but find later
    # that they were not useful or even harmful
    parser.add_argument(
        "--transforms",
        default="crop224+acolor",
        type=str,
        help="transforms to apply")
    # training
    ...
    parser.add_argument(
        ...
        default=400,
        type=int,
        help="Stop training at this epoch")
    parser.add_argument(
        "--accum_iter",
        default=1,
        type=int,
...
def main(args):
    ...
    print("Loading criterion: MaskedMSE(norm_pix_loss={:s})".format(
...
def train_one_epoch(
    ...
    for data_iter_step, (image1, image2) in enumerate(
        ...
        if log_writer is not None and (
            (data_iter_step + 1) %
                (accum_iter * args.print_freq)) == 0:
            # x-axis is based on epoch_1000x in the tensorboard, calibrating
            # differences curves when batch size changes
            epoch_1000x = int(
