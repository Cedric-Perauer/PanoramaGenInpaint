...
def aggregate_stats(info_str, pose_errors, angular_errors):
    ...
    for trl_thr, ang_thr in [(0.1, 1), (0.25, 2), (0.5, 5), (5, 10)]:
        for pose_error, angular_error in zip(pose_errors, angular_errors):
            correct_for_this_threshold = (
                pose_error < trl_thr) and (
                angular_error < ang_thr)
            stats[trl_thr, ang_thr] += correct_for_this_threshold
    stats = {f"acc@{key[0]:g}m,{key[1]}deg": 100 * val /
             len(pose_errors) for key, val in stats.items()}
...
def get_pose_error(pr_camtoworld, gt_cam_to_world):
    abs_transl_error = torch.linalg.norm(torch.tensor(
        pr_camtoworld[:3, 3]) - torch.tensor(gt_cam_to_world[:3, 3]))
    abs_angular_error = (roma.rotmat_geodesic_distance(torch.tensor(
        pr_camtoworld[:3, :3]), torch.tensor(gt_cam_to_world[:3, :3])) * 180 / np.pi)
