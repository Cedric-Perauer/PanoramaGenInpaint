...
ratios_resolutions = {224: {1.0: [224, 224]}, 512: {4 /
                                                    3: [512, 384], 32 /
                                                    21: [512, 336], 16 /
                                                    9: [512, 288], 2 /
                                                    1: [512, 256], 16 /
...
def get_HW_resolution(H, W, maxdim, patchsize=16):
    ...
    # check patchsize and make sure output resolution is a multiple of
    # patchsize
    if isinstance(patchsize, tuple):
        assert (
            len(patchsize) == 2 and isinstance(patchsize[0], int) and isinstance(patchsize[1], int)
...
def get_resize_function(maxdim, patch_size, H, W, is_mask=False):
    if [max(H, W), min(H, W)] in ratios_resolutions[maxdim].values():
        ...
        target_HW = get_HW_resolution(
            H, W, maxdim=maxdim, patchsize=patch_size)
...
def rescale_points3d(pts2d, pts3d, to_resize, HR, WR):
    ...
    pts3d_rescaled = np.full((HR, WR, 3), np.nan, dtype=np.float32)
