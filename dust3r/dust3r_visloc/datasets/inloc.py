...
class VislocInLoc(BaseVislocDataset):
    def __init__(self, root, pairsfile, topk=1):
        ...
        try:
            self.pairs = get_ordered_pairs_from_file(os.path.join(
                self.root, "pairfiles/query", pairsfile + ".txt"))
        ...
        self.aligns_DUC1 = read_alignments(os.path.join(
            self.root, "mapping/DUC1_alignment/all_transformations.txt"))
        self.aligns_DUC2 = read_alignments(os.path.join(
            self.root, "mapping/DUC2_alignment/all_transformations.txt"))
