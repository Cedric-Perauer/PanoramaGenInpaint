...
def geotrf(Trf, pts, ncol=None, norm=False):
    ...
    if isinstance(
            ...
            pts,
            torch.Tensor) and Trf.ndim == 3 and pts.ndim == 4:
        d = pts.shape[3]
        if Trf.shape[-1] == d:
            pts = torch.einsum("bij, bhwj -> bhwi", Trf, pts)
        elif Trf.shape[-1] == d + 1:
            pts = torch.einsum("bij, bhwj -> bhwi",
                               Trf[:, :d, :d], pts) + Trf[:, None, None, :d, d]
        else:
            raise ValueError(
...
def normalize_pointcloud(
    ...
    if norm_mode == "avg":
        ...
        nan_pts2, nnz2 = invalid_to_zeros(
            pts2, valid2, ndim=3) if pts2 is not None else (
            None, 0)
        all_pts = torch.cat(
        ...
        nan_pts2 = invalid_to_nans(
            pts2, valid2, ndim=3) if pts2 is not None else None
        all_pts = torch.cat(
            (nan_pts1, nan_pts2), dim=1) if pts2 is not None else nan_pts1
...
@torch.no_grad()
def get_joint_pointcloud_depth(
        z1,
        z2,
        valid_mask1,
        valid_mask2=None,
