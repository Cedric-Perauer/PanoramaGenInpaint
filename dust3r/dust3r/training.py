...
def train(args):
    ...
    data_loader_test = {
        dataset.split("(")[0]: build_dataset(
            ...
            args.batch_size,
            args.num_workers,
            test=True) for dataset in args.test_dataset.split("+")}
...
def save_final_model(args, epoch, model_without_ddp, best_so_far=None):
    ...
    to_save = {
        "args": args,
        "model": model_without_ddp if isinstance(
            model_without_ddp,
            dict) else model_without_ddp.cpu().state_dict(),
        "epoch": epoch,
    }
...
def train_one_epoch(
    ...
    for data_iter_step, batch in enumerate(
        ...
        if (data_iter_step + 1) % accum_iter == 0 and ((data_iter_step + 1) %
...
def test_one_epoch(
    ...
    results = {
        f"{k}_{tag}": getattr(
            meter,
            attr) for k,
        meter in metric_logger.meters.items() for tag,
        attr in aggs}
