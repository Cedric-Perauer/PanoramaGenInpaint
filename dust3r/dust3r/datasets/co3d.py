...
class Co3d(BaseStereoViewDataset):
    def __init__(self, mask_bg=True, *args, ROOT, **kwargs):
        ...
        # we prepare all combinations such that i-j = +/- [5, 10, .., 90]
        # degrees
        self.combinations = [
            (i,
             j) for i,
    ...
    def _read_depthmap(self, depthpath, input_metadata):
        ...
        depthmap = (depthmap.astype(np.float32) / 65535) * \
            np.nan_to_num(input_metadata["maximum_depth"])
        return depthmap
    ...
    def _get_views(self, idx, resolution, rng):
        ...
        views = []
        imgs_idxs = [max(0, min(im_idx + rng.integers(-4, 5), last))
        ...
        while len(imgs_idxs) > 0:  # some images (few) have zero depth
            ...
            if self.invalidate[obj, instance][resolution][im_idx]:
                ...
                for offset in range(1, len(image_pool)):
                    ...
                    if not self.invalidate[obj,
                                           instance][resolution][tentative_im_idx]:
            ...
            if mask_bg:
                ...
                maskmap = imread_cv2(
                    maskpath, cv2.IMREAD_UNCHANGED).astype(
                    np.float32)
