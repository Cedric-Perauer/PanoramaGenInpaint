...
class ModularPointCloudOptimizer(BasePCOptimizer):
    ...
    def __init__(
        ...
        default_focals = [self.focal_brake *
                          np.log(max(H, W)) for H, W in self.imshapes]
    ...
    def preset_intrinsics(self, known_intrinsics, msk=None):
        ...
        for K in known_intrinsics:
            assert K.shape == (3, 3)
        self.preset_focal([K.diagonal()[:2].mean()
                          for K in known_intrinsics], msk)
    ...
    def get_principal_points(self):
        return torch.stack([pp.new((W / 2, H / 2)) + 10 *
                           pp for pp, (H, W) in zip(self.im_pp, self.imshapes)])
    ...
    def depth_to_pts3d(self):
        ...
        rel_ptmaps = [depthmap_to_pts3d(depth[i][None], focal_ex(
            i), pp=pp[i: i + 1])[0] for i in range(im_poses.shape[0])]
        # project to world frame
        return [geotrf(pose, ptmap)
                for pose, ptmap in zip(im_poses, rel_ptmaps)]
