...
def init_from_known_poses(self, niter_PnP=10, min_conf_thr=3):
    ...
    for e, (i, j) in enumerate(tqdm(self.edges, disable=not self.verbose)):
        ...
        msk = self.conf_i[i_j] > min(
            min_conf_thr, self.conf_i[i_j].min() - 0.1)
        ...
        s, R, T = align_multiple_poses(
            torch.stack((P1, P2)), known_poses[[i, j]])
...
def minimum_spanning_tree(
    ...
    n_imgs = len(imshapes)
    sparse_graph = - \
        dict_to_sparse_graph(compute_edge_scores(map(i_j_ij, edges), conf_i, conf_j))
    ...
    if has_im_poses:
        ...
        pair_scores = list(sparse_graph.values())
...
def align_multiple_poses(src_poses, target_poses):
    ...
    def center_and_z(poses):
        ...
        return torch.cat(
            (poses[:, :3, 3], poses[:, :3, 3] + eps * poses[:, :3, 2]))

    R, T, s = roma.rigid_points_registration(center_and_z(
        src_poses), center_and_z(target_poses), compute_scaling=True)
    return s, R, T
