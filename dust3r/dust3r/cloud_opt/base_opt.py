...
from dust3r.dust3r.cloud_opt.commons import (
    edge_str,
    ALL_DISTS,
    NoGradParamDict,
    get_imshapes,
...
class BasePCOptimizer(nn.Module):
    ...
    def _init_from_views(
        ...
        self.edges = [(int(i), int(j))
                      for i, j in zip(view1["idx"], view2["idx"])]
        self.is_symmetrized = set(self.edges) == {
    ...
    def _check_edges(self):
        ...
        assert indices == list(
            range(len(indices))), "bad pair indices: missing values "
        return len(indices)
    ...
    def get_pw_norm_scale_factor(self):
        if self.norm_pw_scale:
            ...
            return (np.log(self.base_scale) -
                    self.pw_poses[:, -1].mean()).exp()
        else:
    ...
    def show(
        ...
        if show_pw_cams:
            ...
            if show_pw_pts3d:
                pts = [geotrf(pw_poses[e], self.pred_i[edge_str(i, j)])
                       for e, (i, j) in enumerate(self.edges)]
...
def clean_pointcloud(
    ...
    """
    assert len(im_confs) == len(cams) == len(
        K) == len(depthmaps) == len(all_pts3d)
    assert 0 <= tol < 1
