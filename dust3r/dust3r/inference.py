...
def loss_of_one_batch(
        ...
        ret=None):
    view1, view2 = batch
    ignore_keys = set(["depthmap", "dataset", "label",
                      "instance", "idx", "true_shape", "rng"])
    for view in batch:
...
def check_if_same_size(pairs):
    ...
    return all(
        shapes1[0] == s for s in shapes1) and all(
...
def find_opt_scaling(
    ...
    nan_gt_pts2 = invalid_to_nans(
        gt_pts2, valid2).flatten(
        1, 2) if gt_pts2 is not None else None

    pr_pts1 = invalid_to_nans(pr_pts1, valid1).flatten(1, 2)
    pr_pts2 = invalid_to_nans(
        pr_pts2, valid2).flatten(
        1, 2) if pr_pts2 is not None else None

    all_gt = torch.cat((nan_gt_pts1, nan_gt_pts2),
                       dim=1) if gt_pts2 is not None else nan_gt_pts1
    all_pr = torch.cat(
    ...
    if fit_mode.startswith("avg"):
        ...
        for iter in range(10):
            ...
            scaling = (w * dot_gt_pr).nanmean(dim=1) / \
                (w * dot_gt_gt).nanmean(dim=1)
    else:
