    ...
    for idx in tqdm(range(len(dataset))):
        ...
        for map_view in map_views:
            ...
            for idx, img in enumerate(
                ...
                imgs.append(dict(img=img.unsqueeze(0), true_shape=np.int32(
                    [img.shape[1:]]), idx=idx, instance=str(idx)))
            output = inference([tuple(imgs)], model, device,
            ...
            valid_pts3d = map_view["pts3d_rescaled"][matches_im1[:,
                                                                 1], matches_im1[:, 0]]
            ...
            if viz_matches > 0:
                ...
                img0 = np.pad(
                    viz_imgs[0], ((0, max(
                        H1 - H0, 0)), (0, 0), (0, 0)), "constant", constant_values=0)
                img1 = np.pad(
                    viz_imgs[1], ((0, max(
                        H0 - H1, 0)), (0, 0), (0, 0)), "constant", constant_values=0)
                ...
                for i in range(n_viz):
                    ...
                    pl.plot([x0, x1 + W0], [y0, y1], "-+", color=cmap(i /
        ...
        if len(query_pts2d) == 0:
            ...
            query_pts2d = np.concatenate(
                query_pts2d, axis=0).astype(
                np.float32)
            ...
            if reprojection_error_diag_ratio is not None:
                reprojection_error_img = reprojection_error_diag_ratio * \
                    math.sqrt(W**2 + H**2)
            else:
