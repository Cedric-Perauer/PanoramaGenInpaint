...
def image_to_equirectangular(image, h_fov, output_width=2048, output_height=1024, wrap_around=False):
    ...
    if wrap_around:
        ...
        left_edge = (theta <= -np.pi + h_fov_rad/2) & (phi >= -
                                                       v_fov_rad/2) & (phi <= v_fov_rad/2)
        right_edge = (theta >= np.pi - h_fov_rad/2) & (phi >= -
                                                       v_fov_rad/2) & (phi <= v_fov_rad/2)

        # Left edge maps to RIGHT half of image
        u_left = (theta[left_edge] + np.pi) / \
            (h_fov_rad/2)  # Corrected mapping
        v_left = phi[left_edge] / (v_fov_rad/2)

        # Right edge maps to LEFT half of image
        u_right = (theta[right_edge] - np.pi) / \
            (h_fov_rad/2)  # Corrected mapping
        ...
        equirect[valid_y_left,
                 valid_x_left] = image[valid_img_y_left, valid_img_x_left]
        equirect[valid_y_right,
                 valid_x_right] = image[valid_img_y_right, valid_img_x_right]
...
def render_perspective(equirect_image, yaw_deg,
                       pitch_deg, h_fov_deg,
                       v_fov_deg, output_size=1024,
                       wrap_x=True):                       # ← new flag
    """
...
def project_mask_onto_equirect(
    ...
    in_view_mask = (persp_u >= 0) & (persp_u < persp_w) & \
        (persp_v >= 0) & (persp_v < persp_h) & \
        valid_mask_geom
    ...
    original_colors = original_equirect[eq_y_to_highlight, eq_x_to_highlight].astype(
        np.float32)
    highlight_color_float = highlight_color.astype(np.float32)
    blended_colors = original_colors * \
        (1.0 - alpha) + highlight_color_float * alpha

    equirect_to_highlight[eq_y_to_highlight, eq_x_to_highlight] = np.clip(
        blended_colors, 0, 255).astype(np.uint8)
...
def visualize_all_inpainting_masks(initial_panorama, side_pano, output_size=1024):
    ...
    for view_type, config in view_sets.items():
        ...
        for i, (yaw, pitch) in enumerate(config["params"]):
            ...
            perspective_render = render_perspective(
                initial_panorama, yaw, -pitch, fov, v_fov, output_size, wrap_x=wrap
            )

            mask_persp = render_perspective(
                initial_panorama, yaw, pitch, fov, v_fov, output_size, wrap_x=wrap
            )
            ...
            if np.sum(mask) > 0:
                ...
                raise ValueError(
                    f"Mask for {label} is empty. Check the input image or mask generation.")
...
def project_perspective_to_equirect(
    ...
    equirect_target[fixed_eq_valid,
                    eq_x_valid] = perspective_image[persp_v_int, persp_u_int]

    print(
        f"Projected {len(eq_y_valid)} pixels from perspective to equirect with yaw={yaw_deg}, pitch={pitch_deg}")
